...
def _mode() -> str:
    raw = os.getenv("CODEFLOW_SAMPLING_MODE", "mock").strip().lower()
...
def _resolve_provider(payload: dict[str, Any], mode: str) -> str:
    if mode == "gemini":
        return mode
    provider = (
        os.getenv("CODEFLOW_SAMPLING_PROVIDER", "").strip().lower()
...
def _resolve_model(payload: dict[str, Any], provider: str) -> str:
    ...
    env_model = os.getenv("CODEFLOW_SAMPLING_MODEL", "").strip()
    if env_model:
        return env_model
