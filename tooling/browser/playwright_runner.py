...
class BrowserRunner:
    def __init__(
        ...
        if headless is None:
            env = os.getenv("AGENTCODER_HEADLESS", "true").lower()
            headless = env not in {"0", "false", "no"}
        self._headless = headless
        timeout_ms = os.getenv("AGENTCODER_PLAYWRIGHT_TIMEOUT_MS", "30000")
        try:
    ...
    def _allowlist(self) -> list[str]:
        raw = os.getenv("AGENTCODER_BROWSER_ALLOWLIST", "").strip()
        if not raw:
            return []
