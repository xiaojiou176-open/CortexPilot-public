...
DEFAULT_PROFILE_DISPLAY_NAME = "codeflow"
...
def _machine_cache_root() -> Path:
    explicit = _first_non_empty(os.getenv("CODEFLOW_MACHINE_CACHE_ROOT"))
    if explicit:
...
def default_cdp_host() -> str:
    return _first_non_empty(os.getenv("CODEFLOW_BROWSER_CDP_HOST"), DEFAULT_CDP_HOST)


def default_cdp_port() -> int:
    raw = _first_non_empty(os.getenv("CODEFLOW_BROWSER_CDP_PORT"), str(DEFAULT_CDP_PORT))
    try:
        value = int(raw)
    except ValueError:
