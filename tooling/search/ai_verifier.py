...
def verify_search_results_ai(
    ...
    provider = str(requested_provider or os.getenv("CODEFLOW_SEARCH_PROVIDER", "") or "gemini").strip().lower()
    if provider != "gemini":
        return {
            "ok": False,
            "status": "SKIPPED",
    ...
    base_url = os.getenv("CODEFLOW_AGENTS_BASE_URL", "").strip()
    ...
    api_mode = os.getenv("CODEFLOW_AGENTS_API", "").strip()
    if api_mode:
        set_default_openai_api(api_mode)

    resolved_model = (model or os.getenv("CODEFLOW_SEARCH_VERIFY_MODEL", "")).strip()
