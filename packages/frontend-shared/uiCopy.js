    ...
    en: {
        ...
        dashboard: {
            homePhase2: {
                ...
                heroSubtitle: "Stop babysitting AI coding work. AI coding does not lack models. It lacks a command tower. Codeflow gives teams one governed operator path to plan, delegate, track, resume, and prove long-running engineering work across Codex and Claude Code.",
                ...
                publicTemplatesDescription: "Start with `news_digest` first. It is the official public baseline. `topic_brief` and `page_brief` now follow release-proven secondary paths, but `news_digest` remains the only official first public baseline.",
                ...
                publicAdvantagesDescription: "Codeflow is not just one more prompt wrapper. It turns prompt, context, and harness decisions into explicit product surfaces so long-running work keeps moving when humans step away.",
                ...
                aiSurfacesDescription: "AI in Codeflow is not a floating chat box. It already shows up as a pre-run advisor, a workflow-level explainer, and a run/compare operator brief.",
        ...
        desktop: {
            ...
            overview: {
                ...
                subtitle: "Codeflow starts with one loop: start the task, watch Command Tower, confirm the Workflow Case, then verify Proof & Replay. Only open approvals when the flow asks for one.",
                refreshData: "Refresh data",
    ...
    "zh-CN": {
        ...
        dashboard: {
            homePhase2: {
                ...
                heroSubtitle: "别再盯着 AI 编码一步一步催了。AI 编码不缺模型，缺的是指挥塔。Codeflow 会围绕 Codex / Claude Code 去规划、派工、追踪、续跑和验真，把长期工程任务收进一条可治理、可续跑、可验真的操作路径。",
                ...
                publicTemplatesDescription: "先从 `news_digest` 开始。它是官方公开基线。`topic_brief` 和 `page_brief` 现在都属于已发布验证的次级路径，但 `news_digest` 仍是唯一官方第一公开基线。",
                ...
                publicTemplateCards: [
                    ...
                    },
                    {
                        ...
                        example: "https://example.com + focused summary request",
                        proof: "Proof 状态：已发布验证的浏览器次级路径",
                        fields: ["url", "focus"],
                    },
                ],
                publicAdvantagesTitle: "指挥塔背后的三层工程能力",
                publicAdvantagesDescription: "Codeflow 不是再包一层花哨 prompt，而是把 prompt、context 和 harness 这三类最容易失控的东西，都拉进显式、可检查、可演进的系统里。",
                publicAdvantageCards: [
                    {
                        href: "/pm",
                        title: "提示词工程",
                ...
                aiSurfacesDescription: "Codeflow 里的 AI 不是漂浮聊天框。它已经分别出现在执行前建议、工作流解释，以及运行/对比的操作摘要里。",
        ...
        desktop: {
            ...
            overview: {
                title: "指挥面总览",
                subtitle: "Codeflow 的第一条主循环是：发起任务、观察指挥塔、确认工作流案例、再检查证明与回放。只有在流程要求时才进入审批。",
                refreshData: "刷新数据",
