...
test("dashboard client uses contract-backed role-config paths", async () => {
  ...
  assert.equal(calls[1].init.headers["x-agentcoder-role"], "TECH_LEAD");
  assert.equal(calls[2].init.headers["x-agentcoder-role"], "TECH_LEAD");
});
...
test("dashboard client attaches operator role only for mutation requests", async () => {
  ...
  assert.equal(calls[0].init.headers["x-agentcoder-role"], undefined);
  assert.equal(calls[1].init.method, "POST");
  assert.equal(calls[1].init.headers["x-agentcoder-role"], "TECH_LEAD");
  assert.equal(calls[1].init.headers["x-agentcoder-run-id"], "run-1");
