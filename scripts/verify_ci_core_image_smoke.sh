...
IMAGE="${AGENTCODER_CI_CORE_IMAGE_SMOKE_IMAGE:-agentcoder-ci-core:local}"
...
DOCKER_VERIFY_TIMEOUT_SEC="${AGENTCODER_CI_CORE_IMAGE_SMOKE_VERIFY_TIMEOUT_SEC:-180}"
...
if command -v docker >/dev/null 2>&1; then
  ...
  if ! docker_cmd_with_timeout "${DOCKER_PRECHECK_TIMEOUT_SEC}" docker image inspect "$IMAGE" >/dev/null 2>&1; then
    ...
    AGENTCODER_DOCKER_CI_STAGE_CONTEXT=ci-core-image-smoke bash scripts/docker_ci.sh lane ci-smoke
  ...
  docker_cmd_with_timeout "${DOCKER_VERIFY_TIMEOUT_SEC}" docker run --rm "$IMAGE" bash -lc '
    ...
    cargo audit --version
  '
elif [[ "${AGENTCODER_CI_CONTAINER:-0}" == "1" ]]; then
