...
APP_DIR="${AGENTCODER_DASHBOARD_APP_DIR:-$ROOT_DIR/apps/dashboard}"
source "$ROOT_DIR/scripts/lib/toolchain_env.sh"
source "$ROOT_DIR/scripts/lib/machine_cache_retention.sh"

STORE_DIR="${AGENTCODER_PNPM_STORE_DIR:-$(agentcoder_pnpm_store_dir "$ROOT_DIR")/dashboard}"
STATE_ROOT="$ROOT_DIR/.runtime-cache/agentcoder/temp/install-dashboard-deps-state"
if [[ "${AGENTCODER_CI_CONTAINER:-0}" == "1" && -n "${RUNNER_TEMP:-}" ]]; then
  STATE_ROOT="${RUNNER_TEMP}/install-dashboard-deps-state"
fi
INSTALL_NODE_LINKER="${AGENTCODER_DASHBOARD_PNPM_NODE_LINKER:-hoisted}"
INSTALL_PACKAGE_IMPORT_METHOD="${AGENTCODER_DASHBOARD_PNPM_IMPORT_METHOD:-copy}"
INSTALL_SHAMEFULLY_HOIST="${AGENTCODER_DASHBOARD_PNPM_SHAMEFULLY_HOIST:-1}"
...
LOCK_HELD=0
MIN_ENOSPC_RECOVERY_HEADROOM_GIB="${AGENTCODER_DASHBOARD_ENOSPC_MIN_HEADROOM_GIB:-3}"
WORKSPACE_RETRY_STORE_ACTIVE=0

agentcoder_maybe_auto_prune_machine_cache "$ROOT_DIR" "install_dashboard_deps"
...
resolve_writable_store_dir() {
  ...
  agentcoder_pnpm_local_retry_dir "$ROOT_DIR" "dashboard"
}
...
cleanup_stale_retry_stores() {
  local retry_prefix
  retry_prefix="$(agentcoder_pnpm_local_retry_prefix "$ROOT_DIR" "dashboard")"
  local candidate=""
  shopt -s nullglob
...
recover_with_workspace_store() {
  ...
  INSTALL_PACKAGE_IMPORT_METHOD="${AGENTCODER_DASHBOARD_ENOSPC_IMPORT_METHOD:-hardlink}"
