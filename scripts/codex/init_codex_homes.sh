...
MODEL_NAME="${CODEFLOW_CODEX_MODEL:-gpt-5.2-codex}"
BASE_CONFIG="${CODEFLOW_CODEX_BASE_CONFIG:-$HOME/.codex/config.toml}"
BASE_REQUIREMENTS="${CODEFLOW_CODEX_BASE_REQUIREMENTS:-$HOME/.codex/requirements.toml}"
...
create_home() {
  local role="$1"
  local role_file="$2"
  local home_dir="$HOME/.codex-homes/codeflow-$role"
  mkdir -p "$home_dir"
  local tmp_file
  tmp_file="$(mktemp)"
  awk '
