...
MODEL_NAME="${AGENTCODER_CODEX_MODEL:-gpt-5.2-codex}"
BASE_CONFIG="${AGENTCODER_CODEX_BASE_CONFIG:-$HOME/.codex/config.toml}"
BASE_REQUIREMENTS="${AGENTCODER_CODEX_BASE_REQUIREMENTS:-$HOME/.codex/requirements.toml}"
...
create_home() {
  local role="$1"
  local role_file="$2"
  local home_dir="$HOME/.codex-homes/agentcoder-$role"
  mkdir -p "$home_dir"
  local tmp_file
  tmp_file="$(mktemp)"
  awk '
