...
if [ "${CODEFLOW_NIGHTLY_DEAD_CODE_SCAN:-1}" = "1" ]; then
  echo "🌙 [nightly-cleanup] running archived dead-code full scan"
  bash scripts/dead_code_nightly.sh
else
  echo "⚠️ [nightly-cleanup] CODEFLOW_NIGHTLY_DEAD_CODE_SCAN=0, skip dead-code nightly scan"
fi
echo "🌙 [nightly-cleanup] applying nightly profile cleanup"
CODEFLOW_CLEANUP_PROFILE=nightly bash scripts/cleanup_runtime.sh
