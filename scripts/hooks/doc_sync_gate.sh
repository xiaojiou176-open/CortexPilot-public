...
collect_target_files() {
  ...
  if [[ "$mode" == "ci-diff" ]]; then
    ...
    if [[ -z "$base_sha" || -z "$head_sha" ]]; then
      echo "❌ [doc-sync-gate] ci-diff mode requires AGENTCODER_DOC_GATE_BASE_SHA and AGENTCODER_DOC_GATE_HEAD_SHA." >&2
      return 1
    fi
  ...
  echo "❌ [doc-sync-gate] unsupported AGENTCODER_DOC_GATE_MODE=$mode (expected: staged|ci-diff)." >&2
  return 1
}
...
is_major_logic_file() {
  ...
  case "$file" in
    .pre-commit-config.yaml|package.json)
      return 0
      ;;
    apps/orchestrator/src/agentcoder_orch/*.py|apps/orchestrator/src/agentcoder_orch/**/*.py)
      return 0
      ;;
