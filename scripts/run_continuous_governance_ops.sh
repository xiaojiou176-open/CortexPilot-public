...
HARD_FAIL=0
ENV_FLAKY_MAX_RETRIES="${CODEFLOW_CONT_GOV_ENV_FLAKY_MAX_RETRIES:-1}"

if ! [[ "$ENV_FLAKY_MAX_RETRIES" =~ ^[0-9]+$ ]]; then
  echo "❌ [continuous-governance] CODEFLOW_CONT_GOV_ENV_FLAKY_MAX_RETRIES must be non-negative integer" >&2
  exit 2
fi
...
if [[ -f ".runtime-cache/codeflow/reports/ci/current_run/source_manifest.json" ]]; then
  ...
  run_step \
    ...
    "1" \
    "python3 scripts/check_ci_current_run_sources.py --source-manifest .runtime-cache/codeflow/reports/ci/current_run/source_manifest.json --out-json \"$CURRENT_RUN_CONSISTENCY_JSON\" --out-markdown \"$CURRENT_RUN_CONSISTENCY_MD\""
fi
...
payload = {
    "report_type": "codeflow_continuous_governance_summary",
    "schema_version": 1,
