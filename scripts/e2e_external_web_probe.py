...
def _is_mainline_context() -> bool:
    if _is_truthy(os.environ.get("CI")):
        return True
    if str(os.environ.get("CODEFLOW_CI_PROFILE", "")).strip().lower() == "strict":
        return True
...
def _resolve_provider_probe_target() -> dict[str, str]:
    provider = str(os.environ.get("CODEFLOW_PROVIDER", "")).strip().lower()
    base_url = str(os.environ.get("CODEFLOW_PROVIDER_BASE_URL", "")).strip()
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--url",
        default=os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_URL", "https://example.com"),
        help="External URL to probe.",
    )
    parser.add_argument(
        "--title-regex",
        default=os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_TITLE_REGEX", r"Example\s+Domain"),
        help="Expected page title regex.",
    )
    parser.add_argument(
        "--run-id",
        default=os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_RUN_ID", "").strip(),
        help="Optional run id.",
    )
    parser.add_argument(
        "--timeout-ms",
        type=int,
        default=int(os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_TIMEOUT_MS", "45000")),
        help="Page navigation timeout in milliseconds.",
    )
    parser.add_argument(
        "--provider-api-mode",
        choices=("off", "auto", "require"),
        default=os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_PROVIDER_API_MODE", "auto"),
        help="Whether to probe real provider API with resolved key.",
    )
    parser.add_argument(
        "--provider-api-timeout-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_PROVIDER_TIMEOUT_SEC", "15")),
        help="Timeout for provider API probe.",
    )
    parser.add_argument(
        "--heartbeat-interval-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_HEARTBEAT_SEC", "10")),
        help="Heartbeat emission interval in seconds.",
    )
    parser.add_argument(
        "--hard-timeout-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_EXTERNAL_WEB_PROBE_HARD_TIMEOUT_SEC", "180")),
