...
DEFAULT_PROVIDER_FILE = ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "runners" / "provider_resolution.py"
...
def main() -> int:
    ...
    ci_image = next((entry for entry in upstreams if entry.get("id") == "ci-image:agentcoder-ci-core"), None)
    if isinstance(ci_image, dict):
        recorded_pin = str(ci_image.get("pin") or "").strip()
        if dockerfile_from and recorded_pin != dockerfile_from:
            errors.append(
                "ci-image:agentcoder-ci-core pin drift vs infra/ci/Dockerfile.core "
