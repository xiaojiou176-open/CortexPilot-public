...
LOCK_DIR="${ROOT_DIR}/.runtime-cache/agentcoder/locks/ui_regression_flake_gate.lock"
...
detect_lingering_ui_e2e_processes() {
  local patterns=(
    ...
    '.runtime-cache/cache/toolchains/python/current/bin/python -m agentcoder_orch.cli serve --host 127.0.0.1 --port 18500'
    '.runtime-cache/cache/toolchains/python/current/bin/python -m agentcoder_orch.cli serve --host 127.0.0.1 --port 18600'
  )
  local -a matches=()
  local pid=""
  local command=""
...
env = os.environ.copy()
env["AGENTCODER_E2E_ARTIFACT_SUFFIX"] = artifact_suffix
