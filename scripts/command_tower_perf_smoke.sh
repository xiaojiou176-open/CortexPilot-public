...
resolve_python() {
  agentcoder_python_bin "$ROOT_DIR"
}
...
if mode == "testclient" and not token:
    os.environ.setdefault("AGENTCODER_API_TOKEN", "local-dev-token")
    token = os.environ["AGENTCODER_API_TOKEN"]
...
if mode == "http":
    ...
    from agentcoder_orch.api.main import app
