...
if mode == "testclient" and not token:
    os.environ.setdefault("CODEFLOW_API_TOKEN", "local-dev-token")
    token = os.environ["CODEFLOW_API_TOKEN"]
...
if mode == "http":
    ...
    from codeflow_orch.api.main import app
