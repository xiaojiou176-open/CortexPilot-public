...
BASE_REF="${CODEFLOW_TEST_QUICK_BASE:-}"
...
map_orchestrator_source_to_tests() {
  ...
  if [[ -n "$module_dir" && "$module_dir" != "." && "$module_dir" != "codeflow_orch" ]]; then
    while IFS= read -r test_path; do
      append_line_if_exists "$ORCH_TARGETS" "$test_path"
...
run_pytest_with_parallel_fallback() {
  ...
  local log_file="$2"
  local parallel_args
  parallel_args="${CODEFLOW_PYTEST_PARALLEL_ARGS:--n auto --dist loadscope}"
  local targets=()
  while IFS= read -r line; do
    [[ -n "$line" ]] && targets+=("$line")
  done < "$target_file"
