...
MODE="gate"
SCOPE="${AGENTCODER_DEAD_CODE_SCOPE:-incremental}"
BASE_REF="${AGENTCODER_DEAD_CODE_BASE_REF:-}"
WARN_SYMBOLS="${AGENTCODER_DEAD_CODE_WARN_SYMBOLS:-5}"
WARN_LINES="${AGENTCODER_DEAD_CODE_WARN_LINES:-50}"
FAIL_SYMBOLS="${AGENTCODER_DEAD_CODE_FAIL_SYMBOLS:-5}"
FAIL_LINES="${AGENTCODER_DEAD_CODE_FAIL_LINES:-50}"
SEVERE_LINES="${AGENTCODER_DEAD_CODE_SEVERE_LINES:-500}"
OUT_DIR="${AGENTCODER_DEAD_CODE_OUT_DIR:-.runtime-cache/test_output/dead_code_gate}"
...
def parse_vulture_findings(changed: Set[str], added: Dict[str, Set[int]]) -> Tuple[List[Finding], str]:
    venv_python_raw = os.environ.get("AGENTCODER_PYTHON", "").strip()
    if venv_python_raw:
...
def parse_depcheck_findings(changed_all: Set[str], added: Dict[str, Set[int]], app_dir: str) -> Tuple[List[Finding], str]:
    depcheck_enabled = os.environ.get("AGENTCODER_DEAD_CODE_ENABLE_DEPCHECK", "0").strip().lower() in {"1", "true", "yes"}
    ...
    ignore_raw = os.environ.get(
        "AGENTCODER_DEAD_CODE_DEPCHECK_IGNORE",
...
ci_slice = str(os.environ.get("AGENTCODER_CI_SLICE") or "").strip()
ci_route_id = str(os.environ.get("AGENTCODER_CI_ROUTE_ID") or "").strip()
