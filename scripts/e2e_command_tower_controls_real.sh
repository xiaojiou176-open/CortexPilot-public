...
TMP_RUNTIME_DIR="${RUNNER_TEMP:-$ROOT_DIR/.runtime-cache/codeflow/temp}/playwright-artifacts"
...
API_PORT="$(codeflow_env_get CODEFLOW_E2E_API_PORT "19200")"
DASHBOARD_PORT="$(codeflow_env_get CODEFLOW_E2E_DASHBOARD_PORT "19300")"
BROWSER_HOST="$(codeflow_env_get CODEFLOW_E2E_BROWSER_HOST "$HOST")"
...
PLAYWRIGHT_TIMEOUT_SEC="$(codeflow_env_get CODEFLOW_E2E_PLAYWRIGHT_TIMEOUT_SEC "2400")"
PLAYWRIGHT_ATTEMPTS="$(codeflow_env_get CODEFLOW_E2E_PLAYWRIGHT_ATTEMPTS "2")"
PLAYWRIGHT_RETRY_BACKOFF_SEC="$(codeflow_env_get CODEFLOW_E2E_PLAYWRIGHT_RETRY_BACKOFF_SEC "2")"
...
ARTIFACT_SUFFIX="${CODEFLOW_E2E_ARTIFACT_SUFFIX:-}"
...
if [[ -n "${CODEFLOW_E2E_ARTIFACT_SUFFIX:-}" ]]; then
  echo "ℹ️ [ct-controls] skip fast gate under flake iteration context (artifact_suffix=${CODEFLOW_E2E_ARTIFACT_SUFFIX})"
else
...
start_dashboard_dev() {
  ...
  NEXT_PUBLIC_CODEFLOW_API_BASE="http://$BROWSER_HOST:$API_PORT" \
  NEXT_PUBLIC_CODEFLOW_API_TOKEN="$API_TOKEN" \
  CODEFLOW_API_TOKEN="$API_TOKEN" \
  CODEFLOW_E2E_API_TOKEN="$API_TOKEN" \
  PORT="$DASHBOARD_PORT" \
...
try:
    ...
    with sync_playwright() as p:
        ...
        context = browser.new_context(
            ...
            extra_http_headers={"x-codeflow-role": "OWNER"},
        )
        context.add_cookies(
            [
                {"name": "codeflow_api_token", "value": api_token, "url": dash_base},
        ...
        project_input_candidates = filter_card.get_by_placeholder("codeflow")
        if project_input_candidates.count() == 0:
            ensure_home_drawer_open()
            try:
                filter_card.get_by_placeholder("codeflow").first.wait_for(state="visible", timeout=3000)
            except Exception:
                pass
        project_input = filter_card.get_by_placeholder("codeflow").first
        ...
        project_input.fill("codeflow-temp")
        if apply_btn.is_disabled():
