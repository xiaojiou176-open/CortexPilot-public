...
TMP_RUNTIME_DIR="${RUNNER_TEMP:-$ROOT_DIR/.runtime-cache/agentcoder/temp}/playwright-artifacts"
...
API_PORT="$(agentcoder_env_get AGENTCODER_E2E_API_PORT "19200")"
DASHBOARD_PORT="$(agentcoder_env_get AGENTCODER_E2E_DASHBOARD_PORT "19300")"
BROWSER_HOST="$(agentcoder_env_get AGENTCODER_E2E_BROWSER_HOST "$HOST")"
...
PLAYWRIGHT_TIMEOUT_SEC="$(agentcoder_env_get AGENTCODER_E2E_PLAYWRIGHT_TIMEOUT_SEC "2400")"
PLAYWRIGHT_ATTEMPTS="$(agentcoder_env_get AGENTCODER_E2E_PLAYWRIGHT_ATTEMPTS "2")"
PLAYWRIGHT_RETRY_BACKOFF_SEC="$(agentcoder_env_get AGENTCODER_E2E_PLAYWRIGHT_RETRY_BACKOFF_SEC "2")"
...
ARTIFACT_SUFFIX="${AGENTCODER_E2E_ARTIFACT_SUFFIX:-}"
...
if [[ -n "${AGENTCODER_E2E_ARTIFACT_SUFFIX:-}" ]]; then
  echo "ℹ️ [ct-controls] skip fast gate under flake iteration context (artifact_suffix=${AGENTCODER_E2E_ARTIFACT_SUFFIX})"
else
...
start_dashboard_dev() {
  ...
  NEXT_PUBLIC_AGENTCODER_API_BASE="http://$BROWSER_HOST:$API_PORT" \
  NEXT_PUBLIC_AGENTCODER_API_TOKEN="$API_TOKEN" \
  AGENTCODER_API_TOKEN="$API_TOKEN" \
  AGENTCODER_E2E_API_TOKEN="$API_TOKEN" \
  PORT="$DASHBOARD_PORT" \
...
try:
    ...
    with sync_playwright() as p:
        ...
        context = browser.new_context(
            ...
            extra_http_headers={"x-agentcoder-role": "OWNER"},
        )
        context.add_cookies(
            [
                {"name": "agentcoder_api_token", "value": api_token, "url": dash_base},
        ...
        project_input_candidates = filter_card.get_by_placeholder("agentcoder")
        if project_input_candidates.count() == 0:
            ensure_home_drawer_open()
            try:
                filter_card.get_by_placeholder("agentcoder").first.wait_for(state="visible", timeout=3000)
            except Exception:
                pass
        project_input = filter_card.get_by_placeholder("agentcoder").first
        ...
        project_input.fill("agentcoder-temp")
        if apply_btn.is_disabled():
