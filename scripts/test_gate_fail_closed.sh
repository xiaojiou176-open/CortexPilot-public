...
set +e
AGENTCODER_GITHUB_ALERTS_MODE=off bash scripts/check_repo_hygiene.sh >/dev/null 2>&1
...
if [[ $baseline_hygiene_status -eq 0 ]]; then
  AGENTCODER_GITHUB_ALERTS_MODE=off bash scripts/check_repo_hygiene.sh >/dev/null
else
  set +e
  post_cleanup_hygiene_output="$(AGENTCODER_GITHUB_ALERTS_MODE=off bash scripts/check_repo_hygiene.sh 2>&1)"
...
if AGENTCODER_UI_MATRIX_FILE="$tmpdir/matrix.md" \
  AGENTCODER_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
  AGENTCODER_UI_FULL_AUDIT_REPORT_ROOT="$tmpdir/full" \
  AGENTCODER_UI_TRUTH_GATE_REPORT="$tmpdir/truth_default.json" \
...
if AGENTCODER_UI_MATRIX_FILE="$tmpdir/matrix.md" \
  AGENTCODER_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
  AGENTCODER_UI_TRUTH_GATE_REPORT="$tmpdir/truth_forged.json" \
  AGENTCODER_UI_P0_REPORT="$tmpdir/forged_p0.json" \
  AGENTCODER_UI_P1_REPORT="$tmpdir/forged_p1.json" \
...
JSON
AGENTCODER_UI_MATRIX_FILE="$tmpdir/matrix.md" \
AGENTCODER_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
AGENTCODER_UI_FULL_AUDIT_REPORT_ROOT="$tmpdir/full" \
AGENTCODER_UI_TRUTH_GATE_REPORT="$tmpdir/truth_break_glass.json" \
AGENTCODER_UI_LATEST_MANIFEST_PATH="$tmpdir/latest_manifest.json" \
AGENTCODER_UI_TRUTH_DISABLE_AUTO_LATEST=0 \
AGENTCODER_UI_TRUTH_BREAK_GLASS=1 \
AGENTCODER_UI_TRUTH_BREAK_GLASS_REASON="test override" \
AGENTCODER_UI_TRUTH_BREAK_GLASS_TICKET="TEST-123" \
AGENTCODER_UI_TRUTH_SKIP_LOCK=1 \
...
if AGENTCODER_UI_MATRIX_FILE="$tmpdir/matrix.md" \
  AGENTCODER_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
  AGENTCODER_UI_FULL_AUDIT_REPORT_ROOT="$tmpdir/full" \
  AGENTCODER_UI_TRUTH_GATE_REPORT="$tmpdir/truth_strict_break_glass_blocked.json" \
  AGENTCODER_UI_P0_REPORT="$tmpdir/flake/p0_case/flake_report.json" \
  AGENTCODER_UI_P1_REPORT="$tmpdir/flake/p1_case/flake_report.json" \
  AGENTCODER_UI_CLICK_INVENTORY_REPORT="$tmpdir/full/full_case/click_inventory_report.json" \
  AGENTCODER_UI_TRUTH_GATE_STRICT=1 \
  AGENTCODER_UI_TRUTH_DISABLE_AUTO_LATEST=1 \
  AGENTCODER_UI_TRUTH_REQUIRE_RUN_ID_MATCH=1 \
  AGENTCODER_UI_TRUTH_BREAK_GLASS=1 \
  AGENTCODER_UI_TRUTH_BREAK_GLASS_REASON="strict-check-override-test" \
  AGENTCODER_UI_TRUTH_BREAK_GLASS_TICKET="TEST-STRICT-OVERRIDE-001" \
  AGENTCODER_UI_TRUTH_SKIP_LOCK=1 \
  bash scripts/ui_e2e_truth_gate.sh >/dev/null 2>&1; then
...
JSON
if AGENTCODER_INCIDENT_DIR="$tmpdir/incidents" \
  AGENTCODER_INCIDENT_MAP_PATH="$tmpdir/incident_map_bad_type.json" \
...
JSON
if AGENTCODER_INCIDENT_DIR="$tmpdir/incidents" \
  AGENTCODER_INCIDENT_MAP_PATH="$tmpdir/incident_map_empty_string.json" \
  bash scripts/check_incident_regression_gate.sh >/dev/null 2>&1; then
...
JSON
if AGENTCODER_INCIDENT_DIR="$tmpdir/incidents" \
  AGENTCODER_INCIDENT_MAP_PATH="$tmpdir/incident_map_missing_target.json" \
  bash scripts/check_incident_regression_gate.sh >/dev/null 2>&1; then
...
JSON
AGENTCODER_INCIDENT_DIR="$tmpdir/incidents" \
AGENTCODER_INCIDENT_MAP_PATH="$tmpdir/incident_map_ok.json" \
...
if env -i PATH="$PATH" HOME="${HOME:-/tmp}" \
  AGENTCODER_CI_BREAK_GLASS_VALIDATE_ONLY_GATE=AGENTCODER_CI_MUTATION_GATE \
  AGENTCODER_CI_MUTATION_GATE=0 \
...
AGENTCODER_CI_BREAK_GLASS_VALIDATE_ONLY_GATE=AGENTCODER_CI_MUTATION_GATE \
AGENTCODER_CI_MUTATION_GATE=0 \
AGENTCODER_CI_MUTATION_GATE_BREAK_GLASS=1 \
AGENTCODER_CI_MUTATION_GATE_BREAK_GLASS_REASON="test break glass" \
AGENTCODER_CI_MUTATION_GATE_BREAK_GLASS_TICKET="TEST-456" \
...
AGENTCODER_CI_BREAK_GLASS_AUDIT_LOG="$ci_validate_only_audit_log" \
AGENTCODER_CI_BREAK_GLASS=1 \
AGENTCODER_CI_BREAK_GLASS_REASON="ci validate-only break-glass test" \
AGENTCODER_CI_BREAK_GLASS_TICKET="TEST-789" \
AGENTCODER_CI_BREAK_GLASS_VALIDATE_ONLY_GATE=AGENTCODER_CI_MUTATION_GATE \
bash scripts/ci.sh >/dev/null
assert_file_contains "$ci_validate_only_audit_log" "\"scope\": \"validate_only_early_exit\""
assert_file_contains "$ci_validate_only_audit_log" "\"enabled_var\": \"AGENTCODER_CI_BREAK_GLASS\""
...
assert_file_contains "scripts/security_scan.sh" "agentcoder-trufflehog.jsonl"
...
assert_file_contains ".pre-commit-config.yaml" "agentcoder-public-sensitive-surface-gate"
...
assert_file_contains ".pre-commit-config.yaml" "agentcoder-github-security-alerts-gate"
assert_file_contains "scripts/pre_push_quality_gate.sh" "scripts/check_github_security_alerts.py"
assert_file_contains ".github/workflows/ci.yml" "scripts/check_github_security_alerts.py --repo xiaojiou176-open/Agentcoder"
...
#!/usr/bin/env bash
set -euo pipefail
suffix="${AGENTCODER_E2E_ARTIFACT_SUFFIX:-missing}"
