...
set +e
CODEFLOW_GITHUB_ALERTS_MODE=off bash scripts/check_repo_hygiene.sh >/dev/null 2>&1
...
if [[ $baseline_hygiene_status -eq 0 ]]; then
  CODEFLOW_GITHUB_ALERTS_MODE=off bash scripts/check_repo_hygiene.sh >/dev/null
else
  set +e
  post_cleanup_hygiene_output="$(CODEFLOW_GITHUB_ALERTS_MODE=off bash scripts/check_repo_hygiene.sh 2>&1)"
...
if CODEFLOW_UI_MATRIX_FILE="$tmpdir/matrix.md" \
  CODEFLOW_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
  CODEFLOW_UI_FULL_AUDIT_REPORT_ROOT="$tmpdir/full" \
  CODEFLOW_UI_TRUTH_GATE_REPORT="$tmpdir/truth_default.json" \
...
if CODEFLOW_UI_MATRIX_FILE="$tmpdir/matrix.md" \
  CODEFLOW_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
  CODEFLOW_UI_TRUTH_GATE_REPORT="$tmpdir/truth_forged.json" \
  CODEFLOW_UI_P0_REPORT="$tmpdir/forged_p0.json" \
  CODEFLOW_UI_P1_REPORT="$tmpdir/forged_p1.json" \
...
JSON
CODEFLOW_UI_MATRIX_FILE="$tmpdir/matrix.md" \
CODEFLOW_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
CODEFLOW_UI_FULL_AUDIT_REPORT_ROOT="$tmpdir/full" \
CODEFLOW_UI_TRUTH_GATE_REPORT="$tmpdir/truth_break_glass.json" \
CODEFLOW_UI_LATEST_MANIFEST_PATH="$tmpdir/latest_manifest.json" \
CODEFLOW_UI_TRUTH_DISABLE_AUTO_LATEST=0 \
CODEFLOW_UI_TRUTH_BREAK_GLASS=1 \
CODEFLOW_UI_TRUTH_BREAK_GLASS_REASON="test override" \
CODEFLOW_UI_TRUTH_BREAK_GLASS_TICKET="TEST-123" \
CODEFLOW_UI_TRUTH_SKIP_LOCK=1 \
...
if CODEFLOW_UI_MATRIX_FILE="$tmpdir/matrix.md" \
  CODEFLOW_UI_FLAKE_REPORT_ROOT="$tmpdir/flake" \
  CODEFLOW_UI_FULL_AUDIT_REPORT_ROOT="$tmpdir/full" \
  CODEFLOW_UI_TRUTH_GATE_REPORT="$tmpdir/truth_strict_break_glass_blocked.json" \
  CODEFLOW_UI_P0_REPORT="$tmpdir/flake/p0_case/flake_report.json" \
  CODEFLOW_UI_P1_REPORT="$tmpdir/flake/p1_case/flake_report.json" \
  CODEFLOW_UI_CLICK_INVENTORY_REPORT="$tmpdir/full/full_case/click_inventory_report.json" \
  CODEFLOW_UI_TRUTH_GATE_STRICT=1 \
  CODEFLOW_UI_TRUTH_DISABLE_AUTO_LATEST=1 \
  CODEFLOW_UI_TRUTH_REQUIRE_RUN_ID_MATCH=1 \
  CODEFLOW_UI_TRUTH_BREAK_GLASS=1 \
  CODEFLOW_UI_TRUTH_BREAK_GLASS_REASON="strict-check-override-test" \
  CODEFLOW_UI_TRUTH_BREAK_GLASS_TICKET="TEST-STRICT-OVERRIDE-001" \
  CODEFLOW_UI_TRUTH_SKIP_LOCK=1 \
  bash scripts/ui_e2e_truth_gate.sh >/dev/null 2>&1; then
...
JSON
if CODEFLOW_INCIDENT_DIR="$tmpdir/incidents" \
  CODEFLOW_INCIDENT_MAP_PATH="$tmpdir/incident_map_bad_type.json" \
...
JSON
if CODEFLOW_INCIDENT_DIR="$tmpdir/incidents" \
  CODEFLOW_INCIDENT_MAP_PATH="$tmpdir/incident_map_empty_string.json" \
  bash scripts/check_incident_regression_gate.sh >/dev/null 2>&1; then
...
JSON
if CODEFLOW_INCIDENT_DIR="$tmpdir/incidents" \
  CODEFLOW_INCIDENT_MAP_PATH="$tmpdir/incident_map_missing_target.json" \
  bash scripts/check_incident_regression_gate.sh >/dev/null 2>&1; then
...
JSON
CODEFLOW_INCIDENT_DIR="$tmpdir/incidents" \
CODEFLOW_INCIDENT_MAP_PATH="$tmpdir/incident_map_ok.json" \
...
if env -i PATH="$PATH" HOME="${HOME:-/tmp}" \
  CODEFLOW_CI_BREAK_GLASS_VALIDATE_ONLY_GATE=CODEFLOW_CI_MUTATION_GATE \
  CODEFLOW_CI_MUTATION_GATE=0 \
...
CODEFLOW_CI_BREAK_GLASS_VALIDATE_ONLY_GATE=CODEFLOW_CI_MUTATION_GATE \
CODEFLOW_CI_MUTATION_GATE=0 \
CODEFLOW_CI_MUTATION_GATE_BREAK_GLASS=1 \
CODEFLOW_CI_MUTATION_GATE_BREAK_GLASS_REASON="test break glass" \
CODEFLOW_CI_MUTATION_GATE_BREAK_GLASS_TICKET="TEST-456" \
...
CODEFLOW_CI_BREAK_GLASS_AUDIT_LOG="$ci_validate_only_audit_log" \
CODEFLOW_CI_BREAK_GLASS=1 \
CODEFLOW_CI_BREAK_GLASS_REASON="ci validate-only break-glass test" \
CODEFLOW_CI_BREAK_GLASS_TICKET="TEST-789" \
CODEFLOW_CI_BREAK_GLASS_VALIDATE_ONLY_GATE=CODEFLOW_CI_MUTATION_GATE \
bash scripts/ci.sh >/dev/null
assert_file_contains "$ci_validate_only_audit_log" "\"scope\": \"validate_only_early_exit\""
assert_file_contains "$ci_validate_only_audit_log" "\"enabled_var\": \"CODEFLOW_CI_BREAK_GLASS\""
...
assert_file_contains "scripts/security_scan.sh" "codeflow-trufflehog.jsonl"
...
assert_file_contains ".pre-commit-config.yaml" "codeflow-public-sensitive-surface-gate"
...
assert_file_contains ".pre-commit-config.yaml" "codeflow-github-security-alerts-gate"
assert_file_contains "scripts/pre_push_quality_gate.sh" "scripts/check_github_security_alerts.py"
assert_file_contains ".github/workflows/ci.yml" "scripts/check_github_security_alerts.py --repo xiaojiou176-open/Codeflow"
...
#!/usr/bin/env bash
set -euo pipefail
suffix="${CODEFLOW_E2E_ARTIFACT_SUFFIX:-missing}"
