...
STRICT_ALL_LINES_TARGETS = [
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "contract" / "compiler.py",
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "planning" / "intake_policy_helpers.py",
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "scheduler" / "preflight_gate_runtime_helpers.py",
    ROOT / "scripts" / "resolve_perf_smoke_env.sh",
    ...
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "reviewer" / "reviewer.py",
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "scheduler" / "task_execution_review_helpers.py",
]
TARGETED_DIAGNOSTIC_PATTERNS = {
    ...
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "services" / "orchestration_service.py": (
        re.compile(r'failure_summary_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
        re.compile(r'action_hint_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
        re.compile(r'outcome_label_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
    ),
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "services" / "rollback_service.py": (
        re.compile(r'failure_summary_zh"\]\s*=\s*f?".*[\u4e00-\u9fff]'),
        re.compile(r'action_hint_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
        re.compile(r'outcome_label_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
    ),
    ROOT / "apps" / "orchestrator" / "src" / "agentcoder_orch" / "api" / "main_runs_handlers.py": (
        re.compile(r'failure_summary_zh"\]\s*=\s*f?".*[\u4e00-\u9fff]'),
        re.compile(r'action_hint_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
        re.compile(r'outcome_label_zh"\s*:\s*".*[\u4e00-\u9fff]'),
        re.compile(r'outcome_label_zh"\]\s*=\s*".*[\u4e00-\u9fff]'),
