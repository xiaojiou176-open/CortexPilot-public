...
run_ci_step85_86_desktop_real_gates() {
  ...
  if [ "${CODEFLOW_CI_DESKTOP_FIRST_ENTRY_E2E:-1}" = "1" ]; then
    ...
    (
      set -o pipefail
      CODEFLOW_E2E_API_PORT=18500 CODEFLOW_E2E_WEB_PORT=4173 \
        npm run desktop:e2e:first-entry:real:full 2>&1 | tee "$DESKTOP_E2E_FULL_LOG"
    ) &
    desktop_full_pid=$!
    (
      set -o pipefail
      CODEFLOW_E2E_API_PORT=18600 CODEFLOW_E2E_WEB_PORT=4174 \
    ...
    require_skip_gate_break_glass_or_fail "CODEFLOW_CI_DESKTOP_FIRST_ENTRY_E2E" "desktop_first_entry_e2e_skip"
    echo "⚠️ [WARN] CODEFLOW_CI_DESKTOP_FIRST_ENTRY_E2E=0, skip desktop first-entry e2e gate (break-glass)"
  fi
  echo "✅ [STEP 8.5/12] Completed"
  echo "🚀 [STEP 8.6/12] Start: Tauri real-shell E2E (nightly full gate)"
  if [ "${CODEFLOW_CI_NIGHTLY_FULL:-0}" = "1" ] && [ "${CODEFLOW_CI_TAURI_REAL_E2E:-1}" = "1" ]; then
    ...
    run_with_timeout_heartbeat_and_cleanup \
      ...
      "${STEP8_6_TIMEOUT_SEC}" \
      bash -lc "set -o pipefail; CODEFLOW_E2E_API_PORT='${CODEFLOW_CI_TAURI_REAL_API_PORT:-18700}' CODEFLOW_E2E_TAURI_PORT='${CODEFLOW_CI_TAURI_REAL_WEB_PORT:-1430}' npm run desktop:e2e:tauri:real 2>&1 | tee '$DESKTOP_TAURI_REAL_LOG'"
    ...
    echo "⚠️ [WARN] skip tauri real shell e2e (need CODEFLOW_CI_NIGHTLY_FULL=1 and CODEFLOW_CI_TAURI_REAL_E2E=1)"
  fi
