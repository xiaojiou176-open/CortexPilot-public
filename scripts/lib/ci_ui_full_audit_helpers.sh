...
run_ci_ui_full_gemini_audit() {
  ...
  local ui_full_audit_parallel="${AGENTCODER_CI_UI_FULL_AUDIT_PARALLEL:-1}"
  local ui_full_budget_profile="${AGENTCODER_CI_UI_FULL_AUDIT_BUDGET_PROFILE:-auto}"
  local ci_mode="${CI:-}"
  local ci_default_budget="0"
  if [ "$ui_full_budget_profile" = "auto" ]; then
    if [ "${AGENTCODER_CI_NIGHTLY_FULL:-0}" = "1" ]; then
  ...
  if [ -n "$ci_mode" ]; then
    ...
    if [ "$ui_full_budget_profile" = "nightly_full" ]; then
      ui_full_audit_default_shards="${AGENTCODER_CI_UI_FULL_AUDIT_NIGHTLY_SHARDS:-$ui_full_audit_shard_baseline}"
      ui_full_audit_default_max_routes="${AGENTCODER_CI_UI_FULL_AUDIT_NIGHTLY_MAX_ROUTES:-32}"
      ui_full_audit_default_max_buttons_per_page="${AGENTCODER_CI_UI_FULL_AUDIT_NIGHTLY_MAX_BUTTONS_PER_PAGE:-120}"
      ui_full_audit_default_max_interactions="${AGENTCODER_CI_UI_FULL_AUDIT_NIGHTLY_MAX_INTERACTIONS:-300}"
      ui_full_audit_default_max_runtime_sec="${AGENTCODER_CI_UI_FULL_AUDIT_NIGHTLY_MAX_RUNTIME_SEC:-2400}"
    elif [ "$ui_full_budget_profile" = "pr" ]; then
      ui_full_audit_default_shards="${AGENTCODER_CI_UI_FULL_AUDIT_PR_SHARDS:-$ui_full_audit_shard_baseline}"
      ui_full_audit_default_max_routes="${AGENTCODER_CI_UI_FULL_AUDIT_PR_MAX_ROUTES:-16}"
      ui_full_audit_default_max_buttons_per_page="${AGENTCODER_CI_UI_FULL_AUDIT_PR_MAX_BUTTONS_PER_PAGE:-120}"
      ui_full_audit_default_max_interactions="${AGENTCODER_CI_UI_FULL_AUDIT_PR_MAX_INTERACTIONS:-120}"
      ui_full_audit_default_max_runtime_sec="${AGENTCODER_CI_UI_FULL_AUDIT_PR_MAX_RUNTIME_SEC:-1200}"
    else
      echo "❌ [ci] unsupported AGENTCODER_CI_UI_FULL_AUDIT_BUDGET_PROFILE=${ui_full_budget_profile}. expected: auto|pr|nightly_full"
      return 1
    fi
  ...
  if ! is_ci_environment && [ "$CI_PROFILE" = "prepush" ]; then
    ui_full_audit_default_shards="${AGENTCODER_CI_UI_FULL_AUDIT_PREPUSH_SHARDS:-$ui_full_audit_shard_baseline}"
    ui_full_audit_default_max_routes="${AGENTCODER_CI_UI_FULL_AUDIT_PREPUSH_MAX_ROUTES:-8}"
    ui_full_audit_default_max_buttons_per_page="${AGENTCODER_CI_UI_FULL_AUDIT_PREPUSH_MAX_BUTTONS_PER_PAGE:-120}"
    ui_full_audit_default_max_interactions="${AGENTCODER_CI_UI_FULL_AUDIT_PREPUSH_MAX_INTERACTIONS:-80}"
    ui_full_audit_default_max_runtime_sec="${AGENTCODER_CI_UI_FULL_AUDIT_PREPUSH_MAX_RUNTIME_SEC:-900}"
  fi
  local ui_full_audit_shards="${AGENTCODER_CI_UI_FULL_AUDIT_SHARDS:-$ui_full_audit_default_shards}"
  local ui_full_audit_reuse_running_services="${AGENTCODER_CI_UI_FULL_AUDIT_REUSE_RUNNING_SERVICES:-1}"
  local ui_full_audit_max_routes="${AGENTCODER_CI_UI_FULL_AUDIT_MAX_ROUTES:-$ui_full_audit_default_max_routes}"
  local ui_full_audit_max_buttons_per_page="${AGENTCODER_CI_UI_FULL_AUDIT_MAX_BUTTONS_PER_PAGE:-$ui_full_audit_default_max_buttons_per_page}"
  local ui_full_audit_max_interactions="${AGENTCODER_CI_UI_FULL_AUDIT_MAX_INTERACTIONS:-$ui_full_audit_default_max_interactions}"
  local ui_full_audit_max_runtime_sec="${AGENTCODER_CI_UI_FULL_AUDIT_MAX_RUNTIME_SEC:-$ui_full_audit_default_max_runtime_sec}"
  local ui_full_audit_max_duplicate_targets="${AGENTCODER_CI_UI_FULL_AUDIT_MAX_DUPLICATE_TARGETS:-3}"
  local ui_full_route_sampling_mode_raw="${AGENTCODER_CI_UI_FULL_AUDIT_ROUTE_SAMPLING_MODE:-auto}"
  ...
  if [ "$ui_full_route_sampling_mode_raw" = "auto" ]; then
    ...
    echo "❌ [ci] unsupported AGENTCODER_CI_UI_FULL_AUDIT_ROUTE_SAMPLING_MODE=${ui_full_route_sampling_mode_raw}. expected: auto|off|stratified"
    return 1
  fi
  local ui_full_route_priority_file="${AGENTCODER_CI_UI_FULL_AUDIT_ROUTE_PRIORITY_FILE:-}"
  local ui_full_route_priority_profile="${AGENTCODER_CI_UI_FULL_AUDIT_ROUTE_PRIORITY_PROFILE:-$ui_full_budget_profile}"
  local ui_full_route_sample_size_default="0"
  if ! is_ci_environment && [ "$CI_PROFILE" = "prepush" ]; then
    ui_full_route_sample_size_default="${AGENTCODER_CI_UI_FULL_AUDIT_PREPUSH_ROUTE_SAMPLE_SIZE:-4}"
  elif [ "$ui_full_budget_profile" = "pr" ]; then
    ui_full_route_sample_size_default="${AGENTCODER_CI_UI_FULL_AUDIT_PR_ROUTE_SAMPLE_SIZE:-8}"
  else
    ui_full_route_sample_size_default="${AGENTCODER_CI_UI_FULL_AUDIT_NIGHTLY_ROUTE_SAMPLE_SIZE:-0}"
  fi
  local ui_full_route_sample_size="${AGENTCODER_CI_UI_FULL_AUDIT_ROUTE_SAMPLE_SIZE:-$ui_full_route_sample_size_default}"
  local ui_full_audit_ab_matrix="${AGENTCODER_CI_UI_FULL_AUDIT_EXPERIMENT_MATRIX:-0}"
  local ui_full_audit_ab_profiles="${AGENTCODER_CI_UI_FULL_AUDIT_EXPERIMENT_PROFILES:-}"
  local ui_full_audit_ab_iterations="${AGENTCODER_CI_UI_FULL_AUDIT_EXPERIMENT_ITERATIONS:-3}"
  local ui_full_audit_auto_bootstrap_shared_services="${AGENTCODER_CI_UI_FULL_AUDIT_AUTO_BOOTSTRAP_SHARED_SERVICES:-1}"
  local ui_full_audit_model="${AGENTCODER_CI_UI_FULL_AUDIT_MODEL:-gemini-3.0-flash}"
  local run_id="${run_id_prefix}_$(date +%Y%m%d_%H%M%S)"
  local external_api_base="${AGENTCODER_E2E_EXTERNAL_API_BASE:-}"
  local external_dashboard_base="${AGENTCODER_E2E_EXTERNAL_DASHBOARD_BASE:-}"
  ...
  local serial_break_glass_var="AGENTCODER_CI_UI_FULL_AUDIT_SERIAL_BREAK_GLASS"
  local serial_break_glass_reason_var="AGENTCODER_CI_UI_FULL_AUDIT_SERIAL_BREAK_GLASS_REASON"
  local serial_break_glass_ticket_var="AGENTCODER_CI_UI_FULL_AUDIT_SERIAL_BREAK_GLASS_TICKET"
...
api_token = (
    os.environ.get("AGENTCODER_API_TOKEN", "").strip()
    or os.environ.get("AGENTCODER_E2E_API_TOKEN", "").strip()
    or "agentcoder-e2e-token"
)
...
PY
  ...
  ui_full_audit_bootstrap_shared_services() {
    ...
    local shared_api_token="${AGENTCODER_API_TOKEN:-${AGENTCODER_E2E_API_TOKEN:-agentcoder-e2e-token}}"
    ...
    (
      ...
      export AGENTCODER_API_TOKEN="$shared_api_token"
      exec "$PYTHON" -m agentcoder_orch.cli serve --host "$host" --port "$api_port"
    ) >"$api_log" 2>&1 &
    ui_full_audit_bootstrap_api_pid=$!
    (
      cd apps/dashboard
      export NEXT_PUBLIC_AGENTCODER_API_BASE="http://${host}:${api_port}"
      export NEXT_PUBLIC_AGENTCODER_API_TOKEN="$shared_api_token"
      export AGENTCODER_API_TOKEN="$shared_api_token"
      export AGENTCODER_E2E_API_TOKEN="$shared_api_token"
  ...
  if [ "$ui_full_audit_parallel" = "1" ] && { [ -z "$external_api_base" ] || [ -z "$external_dashboard_base" ]; }; then
    if [ "$ui_full_audit_reuse_running_services" = "1" ]; then
      local shared_host="${AGENTCODER_E2E_HOST:-127.0.0.1}"
      local shared_api_port="${AGENTCODER_UI_FULL_E2E_API_PORT:-19600}"
      local shared_dashboard_port="${AGENTCODER_UI_FULL_E2E_DASHBOARD_PORT:-19700}"
  ...
  if [ "$ui_full_audit_parallel" = "1" ]; then
    ...
    if [ -n "${AGENTCODER_E2E_HOST:-}" ]; then
      parallel_cmd+=(--host "${AGENTCODER_E2E_HOST}")
    fi
    if [ -n "${AGENTCODER_UI_FULL_E2E_API_PORT:-}" ]; then
      parallel_cmd+=(--api-port "${AGENTCODER_UI_FULL_E2E_API_PORT}")
    fi
    if [ -n "${AGENTCODER_UI_FULL_E2E_DASHBOARD_PORT:-}" ]; then
      parallel_cmd+=(--dashboard-port "${AGENTCODER_UI_FULL_E2E_DASHBOARD_PORT}")
    fi
...
PY
    ...
    if [[ "$serial_mode_break_glass_active" != "1" ]]; then
      echo "❌ [ci] AGENTCODER_CI_UI_FULL_AUDIT_PARALLEL=${ui_full_audit_parallel} is blocked (fail-closed). use parallel mode, or set ${serial_break_glass_var}=1 with reason/ticket."
      return 1
    fi
  ...
  if [ -n "${AGENTCODER_E2E_HOST:-}" ]; then
    cmd+=(--host "${AGENTCODER_E2E_HOST}")
  fi
  if [ -n "${AGENTCODER_UI_FULL_E2E_API_PORT:-}" ]; then
    cmd+=(--api-port "${AGENTCODER_UI_FULL_E2E_API_PORT}")
  fi
  if [ -n "${AGENTCODER_UI_FULL_E2E_DASHBOARD_PORT:-}" ]; then
    cmd+=(--dashboard-port "${AGENTCODER_UI_FULL_E2E_DASHBOARD_PORT}")
  fi
  if [ -n "${AGENTCODER_UI_FULL_E2E_EXTERNAL_API_BASE:-}" ]; then
    cmd+=(--external-api-base "${AGENTCODER_UI_FULL_E2E_EXTERNAL_API_BASE}")
  fi
  if [ -n "${AGENTCODER_UI_FULL_E2E_EXTERNAL_DASHBOARD_BASE:-}" ]; then
    cmd+=(--external-dashboard-base "${AGENTCODER_UI_FULL_E2E_EXTERNAL_DASHBOARD_BASE}")
  fi
  if [ -n "${AGENTCODER_E2E_API_TOKEN:-}" ]; then
    cmd+=(--api-token "${AGENTCODER_E2E_API_TOKEN}")
  fi
