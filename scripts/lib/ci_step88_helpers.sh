...
run_ci_step88_ui_strict_click_gate() {
  UI_TRUTH_BATCH_RUN_ID="${CODEFLOW_CI_UI_TRUTH_BATCH_RUN_ID:-}"
  ...
  UI_STRICT_CLICK_GATE="${CODEFLOW_CI_UI_STRICT_CLICK_GATE:-1}"
  UI_STRICT_CLICK_REQUIRED="${CODEFLOW_CI_UI_STRICT_CLICK_REQUIRED:-1}"
  UI_STRICT_REPORT_ROOT="${CODEFLOW_CI_UI_STRICT_REPORT_ROOT:-.runtime-cache/test_output/ui_full_gemini_audit}"
  UI_STRICT_REPORT_PATH="${CODEFLOW_CI_UI_STRICT_REPORT_PATH:-}"
  UI_STRICT_REPORT_REQUIRE_COMPAT="${CODEFLOW_CI_UI_STRICT_REPORT_REQUIRE_COMPAT:-1}"
  UI_STRICT_REPORT_MAX_AGE_SEC="${CODEFLOW_CI_UI_STRICT_REPORT_MAX_AGE_SEC:-172800}"
  # Default mode is full Gemini verdict strict (fail on warn/fail verdicts).
  # Set CODEFLOW_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT=0 only when explicitly opting into click-only strict mode.
  UI_STRICT_REQUIRE_GEMINI_VERDICT="${CODEFLOW_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT:-1}"
  UI_TRUTH_CLICK_INVENTORY_REQUIRED="${CODEFLOW_CI_UI_CLICK_INVENTORY_REQUIRED:-$UI_STRICT_CLICK_REQUIRED}"
  UI_TRUTH_CLICK_INVENTORY_REPORT="${CODEFLOW_CI_UI_CLICK_INVENTORY_REPORT:-}"
  UI_STRICT_BREAK_GLASS_ACTIVE="$(resolve_ci_break_glass \
    "ui_strict_downgrade" \
    "CODEFLOW_CI_UI_STRICT_BREAK_GLASS" \
    "CODEFLOW_CI_UI_STRICT_BREAK_GLASS_REASON" \
    "CODEFLOW_CI_UI_STRICT_BREAK_GLASS_TICKET")"

  if [ "$UI_STRICT_CLICK_GATE" != "1" ] && [ "$UI_STRICT_BREAK_GLASS_ACTIVE" != "1" ]; then
    echo "❌ [ci] CODEFLOW_CI_UI_STRICT_CLICK_GATE=0 is blocked (fail-closed)."
    exit 1
  fi
  if [ "$UI_STRICT_CLICK_REQUIRED" != "1" ] && [ "$UI_STRICT_BREAK_GLASS_ACTIVE" != "1" ]; then
    echo "❌ [ci] CODEFLOW_CI_UI_STRICT_CLICK_REQUIRED!=1 is blocked (fail-closed)."
    exit 1
  fi
  if [ "$UI_STRICT_REQUIRE_GEMINI_VERDICT" != "0" ] && [ "$UI_STRICT_REQUIRE_GEMINI_VERDICT" != "1" ]; then
    echo "❌ [ci] CODEFLOW_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT must be 0 or 1."
    exit 1
  fi
  if [ "$UI_STRICT_REQUIRE_GEMINI_VERDICT" = "0" ] && [ "$UI_STRICT_BREAK_GLASS_ACTIVE" != "1" ]; then
    echo "❌ [ci] CODEFLOW_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT=0 is blocked (fail-closed). set CODEFLOW_CI_UI_STRICT_BREAK_GLASS=1 with CODEFLOW_CI_UI_STRICT_BREAK_GLASS_REASON and CODEFLOW_CI_UI_STRICT_BREAK_GLASS_TICKET."
    exit 1
  fi
  if [ "$UI_STRICT_CLICK_GATE" = "1" ]; then
    ...
    if [ -n "$UI_STRICT_REPORT_PATH" ]; then
      ...
      if ! CODEFLOW_UI_STRICT_BREAK_GLASS="$UI_STRICT_BREAK_GLASS_ACTIVE" \
        CODEFLOW_UI_STRICT_BREAK_GLASS_REASON="${CODEFLOW_CI_UI_STRICT_BREAK_GLASS_REASON:-}" \
        CODEFLOW_UI_STRICT_BREAK_GLASS_TICKET="${CODEFLOW_CI_UI_STRICT_BREAK_GLASS_TICKET:-}" \
    ...
    echo "⚠️ [WARN] CODEFLOW_CI_UI_STRICT_CLICK_GATE=0 with break-glass"
  fi
