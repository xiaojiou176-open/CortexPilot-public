...
run_ci_step88_ui_strict_click_gate() {
  UI_TRUTH_BATCH_RUN_ID="${AGENTCODER_CI_UI_TRUTH_BATCH_RUN_ID:-}"
  ...
  UI_STRICT_CLICK_GATE="${AGENTCODER_CI_UI_STRICT_CLICK_GATE:-1}"
  UI_STRICT_CLICK_REQUIRED="${AGENTCODER_CI_UI_STRICT_CLICK_REQUIRED:-1}"
  UI_STRICT_REPORT_ROOT="${AGENTCODER_CI_UI_STRICT_REPORT_ROOT:-.runtime-cache/test_output/ui_full_gemini_audit}"
  UI_STRICT_REPORT_PATH="${AGENTCODER_CI_UI_STRICT_REPORT_PATH:-}"
  UI_STRICT_REPORT_REQUIRE_COMPAT="${AGENTCODER_CI_UI_STRICT_REPORT_REQUIRE_COMPAT:-1}"
  UI_STRICT_REPORT_MAX_AGE_SEC="${AGENTCODER_CI_UI_STRICT_REPORT_MAX_AGE_SEC:-172800}"
  # Default mode is full Gemini verdict strict (fail on warn/fail verdicts).
  # Set AGENTCODER_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT=0 only when explicitly opting into click-only strict mode.
  UI_STRICT_REQUIRE_GEMINI_VERDICT="${AGENTCODER_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT:-1}"
  UI_TRUTH_CLICK_INVENTORY_REQUIRED="${AGENTCODER_CI_UI_CLICK_INVENTORY_REQUIRED:-$UI_STRICT_CLICK_REQUIRED}"
  UI_TRUTH_CLICK_INVENTORY_REPORT="${AGENTCODER_CI_UI_CLICK_INVENTORY_REPORT:-}"
  UI_STRICT_BREAK_GLASS_ACTIVE="$(resolve_ci_break_glass \
    "ui_strict_downgrade" \
    "AGENTCODER_CI_UI_STRICT_BREAK_GLASS" \
    "AGENTCODER_CI_UI_STRICT_BREAK_GLASS_REASON" \
    "AGENTCODER_CI_UI_STRICT_BREAK_GLASS_TICKET")"

  if [ "$UI_STRICT_CLICK_GATE" != "1" ] && [ "$UI_STRICT_BREAK_GLASS_ACTIVE" != "1" ]; then
    echo "❌ [ci] AGENTCODER_CI_UI_STRICT_CLICK_GATE=0 is blocked (fail-closed)."
    exit 1
  fi
  if [ "$UI_STRICT_CLICK_REQUIRED" != "1" ] && [ "$UI_STRICT_BREAK_GLASS_ACTIVE" != "1" ]; then
    echo "❌ [ci] AGENTCODER_CI_UI_STRICT_CLICK_REQUIRED!=1 is blocked (fail-closed)."
    exit 1
  fi
  if [ "$UI_STRICT_REQUIRE_GEMINI_VERDICT" != "0" ] && [ "$UI_STRICT_REQUIRE_GEMINI_VERDICT" != "1" ]; then
    echo "❌ [ci] AGENTCODER_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT must be 0 or 1."
    exit 1
  fi
  if [ "$UI_STRICT_REQUIRE_GEMINI_VERDICT" = "0" ] && [ "$UI_STRICT_BREAK_GLASS_ACTIVE" != "1" ]; then
    echo "❌ [ci] AGENTCODER_CI_UI_STRICT_REQUIRE_GEMINI_VERDICT=0 is blocked (fail-closed). set AGENTCODER_CI_UI_STRICT_BREAK_GLASS=1 with AGENTCODER_CI_UI_STRICT_BREAK_GLASS_REASON and AGENTCODER_CI_UI_STRICT_BREAK_GLASS_TICKET."
    exit 1
  fi
  if [ "$UI_STRICT_CLICK_GATE" = "1" ]; then
    ...
    if [ -n "$UI_STRICT_REPORT_PATH" ]; then
      ...
      if ! AGENTCODER_UI_STRICT_BREAK_GLASS="$UI_STRICT_BREAK_GLASS_ACTIVE" \
        AGENTCODER_UI_STRICT_BREAK_GLASS_REASON="${AGENTCODER_CI_UI_STRICT_BREAK_GLASS_REASON:-}" \
        AGENTCODER_UI_STRICT_BREAK_GLASS_TICKET="${AGENTCODER_CI_UI_STRICT_BREAK_GLASS_TICKET:-}" \
    ...
    echo "⚠️ [WARN] AGENTCODER_CI_UI_STRICT_CLICK_GATE=0 with break-glass"
  fi
