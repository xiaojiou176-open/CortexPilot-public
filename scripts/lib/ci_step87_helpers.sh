...
PY
  ...
  if [ "${CODEFLOW_CI_UI_REGRESSION_FLAKE_GATE:-1}" = "1" ]; then
    ...
    UI_FLAKE_PARALLEL="${CODEFLOW_CI_UI_FLAKE_PARALLEL:-0}"
    P0_FLAKE_ITERATIONS="${CODEFLOW_CI_UI_FLAKE_P0_ITER:-$default_p0_iter}"
    P0_FLAKE_THRESHOLD="${CODEFLOW_CI_UI_FLAKE_P0_THRESHOLD:-$default_p0_threshold}"
    P0_FLAKE_MAX_THRESHOLD_POLICY="${CODEFLOW_CI_UI_FLAKE_P0_MAX_THRESHOLD_POLICY:-$default_p0_threshold}"
    P0_FLAKE_MIN_ITER_POLICY="${CODEFLOW_CI_UI_FLAKE_P0_MIN_ITER_POLICY:-$default_p0_iter}"
    P0_COMMANDS_FILE="${CODEFLOW_CI_UI_REGRESSION_P0_COMMANDS_FILE:-scripts/ui_regression_p0.commands}"
    ...
    P1_ENABLED=0
    if [ "${CODEFLOW_CI_NIGHTLY_FULL:-0}" = "1" ] || [ "${CODEFLOW_CI_UI_FLAKE_RUN_P1:-1}" = "1" ]; then
      P1_ENABLED=1
      P1_FLAKE_ITERATIONS="${CODEFLOW_CI_UI_FLAKE_P1_ITER:-$default_p1_iter}"
      P1_FLAKE_THRESHOLD="${CODEFLOW_CI_UI_FLAKE_P1_THRESHOLD:-$default_p1_threshold}"
      P1_FLAKE_MAX_THRESHOLD_POLICY="${CODEFLOW_CI_UI_FLAKE_P1_MAX_THRESHOLD_POLICY:-$default_p1_threshold}"
      P1_FLAKE_MIN_ITER_POLICY="${CODEFLOW_CI_UI_FLAKE_P1_MIN_ITER_POLICY:-$default_p1_iter}"
      P1_COMMANDS_FILE="${CODEFLOW_CI_UI_REGRESSION_P1_COMMANDS_FILE:-scripts/ui_regression_p1.commands}"
      ...
      echo "ℹ️ [ci] skip P1 flake gate (enable by CODEFLOW_CI_NIGHTLY_FULL=1 or CODEFLOW_CI_UI_FLAKE_RUN_P1=1)"
    ...
    UI_FLAKE_BREAK_GLASS_ACTIVE="$(resolve_ci_break_glass \
      "ui_flake_gate_skip" \
      "CODEFLOW_CI_UI_FLAKE_BREAK_GLASS" \
      "CODEFLOW_CI_UI_FLAKE_BREAK_GLASS_REASON" \
      "CODEFLOW_CI_UI_FLAKE_BREAK_GLASS_TICKET")"
    if [[ "$UI_FLAKE_BREAK_GLASS_ACTIVE" != "1" ]]; then
      echo "❌ [ci] CODEFLOW_CI_UI_REGRESSION_FLAKE_GATE=0 is blocked (fail-closed). Use explicit break-glass vars."
      exit 1
    fi
    echo "⚠️ [WARN] CODEFLOW_CI_UI_REGRESSION_FLAKE_GATE=0 with break-glass, require explicit flake reports (no auto-latest reuse)"
    if [[ -z "${CODEFLOW_UI_P0_REPORT:-}" || -z "${CODEFLOW_UI_P1_REPORT:-}" ]]; then
      echo "❌ [ci] flake gate skipped but explicit reports missing: require CODEFLOW_UI_P0_REPORT and CODEFLOW_UI_P1_REPORT"
      exit 1
    fi
    P0_REPORT_PATH="${CODEFLOW_UI_P0_REPORT}"
    P1_REPORT_PATH="${CODEFLOW_UI_P1_REPORT}"
