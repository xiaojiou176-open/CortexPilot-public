...
agentcoder_machine_cache_auto_prune_interval_sec() {
  local raw="${AGENTCODER_MACHINE_CACHE_AUTO_PRUNE_INTERVAL_SEC:-1800}"
  if [[ "$raw" =~ ^[0-9]+$ ]]; then
    printf '%s\n' "$raw"
    return 0
  fi
...
agentcoder_machine_cache_auto_prune_enabled() {
  local raw="${AGENTCODER_MACHINE_CACHE_AUTO_PRUNE:-1}"
  local normalized
...
agentcoder_machine_cache_auto_prune_state_dir() {
...
agentcoder_maybe_auto_prune_machine_cache() {
  local root_dir="${1:?root_dir required}"
  local reason="${2:-auto}"

  if ! agentcoder_machine_cache_auto_prune_enabled; then
    return 0
  fi
  if [[ "${AGENTCODER_MACHINE_CACHE_AUTO_PRUNE_RUNNING:-0}" == "1" ]]; then
    return 0
  fi

  local interval_sec
  interval_sec="$(agentcoder_machine_cache_auto_prune_interval_sec)"
  if [[ ! "$interval_sec" =~ ^[0-9]+$ ]] || (( interval_sec <= 0 )); then
    return 0
  fi

  local state_dir
  state_dir="$(agentcoder_machine_cache_auto_prune_state_dir "$root_dir")"
...
EOF
  ...
  local python_bin=""
  python_bin="$(agentcoder_python_bin "$root_dir" 2>/dev/null || true)"
  if [[ ! -x "$python_bin" ]]; then
    prune_status="skip"
    note="managed python toolchain missing; defer auto-prune until bootstrap finishes"
  elif ! (
    cd "$root_dir"
    export AGENTCODER_MACHINE_CACHE_AUTO_PRUNE_RUNNING=1
    export AGENTCODER_CLEANUP_CONFIRM=YES
    export AGENTCODER_CLEANUP_ROOT_NOISE=0
    export AGENTCODER_CLEANUP_CONTRACT_ARTIFACTS=0
