...
run_ci_step6_ui_audit() {
  ...
  if [ "${CODEFLOW_CI_UI_AUDIT_GATE:-1}" = "1" ]; then
    npm run ui:audit
  else
    require_skip_gate_break_glass_or_fail "CODEFLOW_CI_UI_AUDIT_GATE" "ui_audit_gate_skip"
    echo "⚠️ [WARN] CODEFLOW_CI_UI_AUDIT_GATE=0, skip ui audit gate (break-glass)"
  fi
...
run_ci_step7_dependency_audit() {
  ...
  if [ "${CODEFLOW_SOFT_AUDIT:-0}" = "1" ]; then
    local soft_audit_break_glass_active=""
    if ! soft_audit_break_glass_active="$(
      resolve_ci_break_glass \
        "dependency_audit_soft_mode" \
        "CODEFLOW_SOFT_AUDIT" \
        "CODEFLOW_SOFT_AUDIT_REASON" \
        "CODEFLOW_SOFT_AUDIT_TICKET"
    )"; then
      echo "❌ [ci] CODEFLOW_SOFT_AUDIT=1 requires break-glass audit fields: CODEFLOW_SOFT_AUDIT_REASON + CODEFLOW_SOFT_AUDIT_TICKET"
      exit 1
    fi
