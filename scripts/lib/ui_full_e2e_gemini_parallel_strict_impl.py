...
def main() -> int:
    ...
    parser.add_argument(
        "--shards",
        type=int,
        default=int(os.environ.get("CODEFLOW_CI_UI_FULL_AUDIT_SHARDS", "6")),
        help="Total shard count. Default: 6.",
    )
    parser.add_argument(
        "--model",
        default=(
            os.environ.get("CODEFLOW_CI_UI_FULL_AUDIT_MODEL")
            or os.environ.get("CODEFLOW_UI_GEMINI_MODEL")
            or "gemini-3.0-flash"
        ),
    )
    ...
    parser.add_argument(
        "--dashboard-port",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_DASHBOARD_PORT", "19700")),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--shard-timeout-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_PARALLEL_SHARD_TIMEOUT_SEC", "2400")),
        help="Hard timeout per shard process in seconds. 0 disables timeout.",
    )
    parser.add_argument(
        "--heartbeat-interval-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_PARALLEL_HEARTBEAT_INTERVAL_SEC", "20")),
        help="Heartbeat interval for shard liveness logs.",
    )
    parser.add_argument(
        "--snapshot-tail-lines",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_PARALLEL_SNAPSHOT_TAIL_LINES", "80")),
    ...
    parser.add_argument(
        "--budget-profile",
        default=os.environ.get("CODEFLOW_CI_UI_FULL_AUDIT_BUDGET_PROFILE", "unknown"),
        help="Budget profile label used by CI (for example: pr/nightly_full).",
    )
    parser.add_argument(
        "--run-profile",
        default=os.environ.get("CODEFLOW_UI_FULL_E2E_RUN_PROFILE", "default"),
        help="Experiment profile label for A/B attribution.",
    )
    parser.add_argument(
        "--run-label",
        default=os.environ.get("CODEFLOW_UI_FULL_E2E_RUN_LABEL", "single"),
        help="Experiment label for this execution.",
    )
    parser.add_argument(
        "--max-pages",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_PAGES", "0")),
        help="Forward global route budget; 0 disables cap.",
    )
    parser.add_argument(
        "--max-interactions",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_INTERACTIONS", "0")),
    ...
    parser.add_argument(
        "--route-sampling-mode",
        default=os.environ.get("CODEFLOW_UI_FULL_E2E_ROUTE_SAMPLING_MODE", "off"),
    ...
    parser.add_argument(
        "--route-priority-file",
        default=os.environ.get("CODEFLOW_UI_FULL_E2E_ROUTE_PRIORITY_FILE", ""),
        help="Optional JSON file providing per-route priority/risk overrides.",
    )
    parser.add_argument(
        "--route-priority-profile",
        default=os.environ.get("CODEFLOW_UI_FULL_E2E_ROUTE_PRIORITY_PROFILE", ""),
        help="Priority profile label for filtering route-priority-file entries.",
    )
    parser.add_argument(
        "--route-sample-size",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_ROUTE_SAMPLE_SIZE", "0")),
    ...
    parser.add_argument(
        "--ab-matrix",
        action="store_true",
        default=_env_flag("CODEFLOW_UI_FULL_E2E_AB_MATRIX", False),
        help="Run A/B experiment matrix and auto-summary (p50/p95/fail_rate ranking).",
    )
    parser.add_argument(
        "--ab-profiles",
        default=os.environ.get("CODEFLOW_UI_FULL_E2E_AB_PROFILES", ""),
        help=(
    ...
    parser.add_argument(
        "--ab-iterations",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_AB_ITERATIONS", "3")),
