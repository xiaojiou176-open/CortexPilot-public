...
run_ci_step84_ui_button_matrix_gate() {
  ...
  if [[ -f "configs/ui_button_coverage_matrix.md" ]]; then
    UI_MATRIX_SNAPSHOT_FILE="$(mktemp "${TMPDIR:-/tmp}/codeflow-ui-matrix.XXXXXX")"
  ...
  if [ "${CODEFLOW_CI_UI_BUTTON_MATRIX_GATE:-1}" = "1" ]; then
    MATRIX_MODE="${CODEFLOW_CI_UI_BUTTON_MATRIX_MODE:-full}"
    MATRIX_REQUIRED_TIERS="${CODEFLOW_CI_UI_BUTTON_MATRIX_REQUIRED_TIERS:-P0,P1}"
    ...
    if [ "${MATRIX_MODE}" = "changed" ]; then
      MATRIX_BASE_REF="${CODEFLOW_CI_UI_BUTTON_MATRIX_BASE_REF:-origin/main}"
    ...
    UI_MATRIX_TODO_P1_GATE="${CODEFLOW_CI_UI_MATRIX_TODO_P1_GATE:-1}"
    if [ "${UI_MATRIX_TODO_P1_GATE}" = "1" ]; then
      ...
      UI_MATRIX_TODO_P1_BREAK_GLASS_ACTIVE="$(resolve_ci_break_glass \
        ...
        "CODEFLOW_CI_UI_MATRIX_TODO_P1_BREAK_GLASS" \
        "CODEFLOW_CI_UI_MATRIX_TODO_P1_BREAK_GLASS_REASON" \
        "CODEFLOW_CI_UI_MATRIX_TODO_P1_BREAK_GLASS_TICKET")" || {
        echo "❌ [ci] CODEFLOW_CI_UI_MATRIX_TODO_P1_GATE=0 requires valid break-glass metadata"
        exit 1
      }
      if [[ "$UI_MATRIX_TODO_P1_BREAK_GLASS_ACTIVE" != "1" ]]; then
        echo "❌ [ci] CODEFLOW_CI_UI_MATRIX_TODO_P1_GATE=0 is blocked (fail-closed)."
        echo "❌ [ci] set CODEFLOW_CI_UI_MATRIX_TODO_P1_BREAK_GLASS=1 with reason/ticket."
        exit 1
      fi
      run_with_timeout_heartbeat_and_cleanup \
        "ci.sh:step8.4:check_ui_matrix_todo_p0" \
        "${STEP8_4_TODO_TIMEOUT_SEC}" \
        python3 scripts/check_ui_matrix_todo_gate.py --tiers P0
      echo "⚠️ [WARN] CODEFLOW_CI_UI_MATRIX_TODO_P1_GATE=0, skip P1 TODO gate (break-glass)"
    fi
  else
    require_skip_gate_break_glass_or_fail "CODEFLOW_CI_UI_BUTTON_MATRIX_GATE" "ui_button_matrix_gate_skip"
    echo "⚠️ [WARN] CODEFLOW_CI_UI_BUTTON_MATRIX_GATE=0, skip ui button matrix gate (break-glass)"
  fi
