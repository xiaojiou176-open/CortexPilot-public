...
if [[ "${CODEFLOW_CI_PM_CHAT_DISABLE_ZSH_ENV:-0}" == "1" ]]; then
  export CODEFLOW_DISABLE_ZSH_ENV_FALLBACK=1
fi
if [[ "${CODEFLOW_CI_PM_CHAT_DISABLE_DOTENV:-0}" == "1" && -z "${CODEFLOW_DEFAULT_ENV_ROOT:-}" ]]; then
  export CODEFLOW_DEFAULT_ENV_ROOT="${TMPDIR:-/tmp}/codeflow-empty-env-root"
  mkdir -p "${CODEFLOW_DEFAULT_ENV_ROOT}"
fi

source "$ROOT_DIR/scripts/lib/env.sh"

codex_config_path="${CODEFLOW_CODEX_CONFIG_PATH:-$HOME/.codex/config.toml}"
codex_cfg_base_url=""
codex_cfg_provider=""
codex_cfg_model=""
codex_cfg_has_key="0"
codex_cfg_key_source="none"
PYTHON_BIN="${CODEFLOW_CI_PM_CHAT_PYTHON_BIN:-}"
...
load_env_var_from_dotenv_if_missing() {
  ...
  if [[ "${CODEFLOW_CI_PM_CHAT_DISABLE_DOTENV:-0}" == "1" ]]; then
    return 0
  fi
  if [[ -n "${!key:-}" ]]; then
    return 0
...
if [[ -z "$PYTHON_BIN" ]]; then
  if [[ -n "${CODEFLOW_PYTHON:-}" ]] && [[ -x "${CODEFLOW_PYTHON}" ]] && "${CODEFLOW_PYTHON}" -c "pass" >/dev/null 2>&1; then
...
if [[ -n "$PYTHON_BIN" ]] && [[ "${CODEFLOW_CI_PM_CHAT_DISABLE_CODEX_CONFIG:-0}" != "1" ]] && ! is_mainline_context && [[ -f "$codex_config_path" ]]; then
  codex_cfg_rows="$(
...
PROVIDER_ENV_CHAIN=(
  CODEFLOW_CI_PM_CHAT_PROVIDER
  CODEFLOW_E2E_CODEX_PROVIDER
  CODEFLOW_PROVIDER
)
RUNTIME_PROVIDER_ENV_CHAIN=(
  CODEFLOW_CI_PM_CHAT_RUNTIME_OPTIONS_PROVIDER
  CODEFLOW_RUNTIME_OPTIONS_PROVIDER
  CODEFLOW_E2E_RUNTIME_OPTIONS_PROVIDER
)
BASE_URL_ENV_CHAIN=(
  CODEFLOW_CI_PM_CHAT_BASE_URL
  CODEFLOW_E2E_CODEX_BASE_URL
)
MODEL_ENV_CHAIN=(
  CODEFLOW_CI_PM_CHAT_MODEL
  CODEFLOW_E2E_CODEX_MODEL
  CODEFLOW_MODEL
)
...
if ! is_mainline_context && [[ "$has_llm_key" != "1" ]] && [[ -z "${CODEFLOW_CI_PM_CHAT_MODE:-}" ]]; then
  pm_chat_mode_default="mock"
fi

pm_chat_mode="${CODEFLOW_CI_PM_CHAT_MODE:-$pm_chat_mode_default}"
pm_chat_runner="${CODEFLOW_CI_PM_CHAT_RUNNER:-agents}"
pm_chat_web_mode="${CODEFLOW_CI_PM_CHAT_WEB_MODE:-prod}"
pm_chat_runtime_provider="${ENV_RUNTIME_PROVIDER_NORM:-$RESOLVED_PROVIDER}"

pm_chat_requires_key="0"
pm_chat_requires_gemini_key="0"
if [[ "$pm_chat_mode" == "real" ]] && [[ "$has_llm_key" != "1" ]] && [[ "${CODEFLOW_CI_PM_CHAT_ALLOW_MISSING_KEY:-0}" != "1" ]]; then
  pm_chat_requires_key="1"
fi
if [[ "$pm_chat_mode" == "real" ]] && [[ "$pm_chat_runtime_provider" == "gemini" ]] && [[ "$has_gemini_key" != "1" ]] && [[ "${CODEFLOW_CI_PM_CHAT_ALLOW_MISSING_KEY:-0}" != "1" ]]; then
  pm_chat_requires_key="1"
