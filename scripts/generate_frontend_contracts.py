...
OPENAPI_PATH = ROOT / "docs" / "api" / "openapi.codeflow.json"
...
def _load_contract_extension() -> dict:
    ...
    ext = payload.get("x-codeflow-frontend-contract")
    if not isinstance(ext, dict):
        raise SystemExit(f"missing x-codeflow-frontend-contract in {OPENAPI_PATH}")
    return ext
...
def main() -> int:
    ...
    print("generated frontend api contract artifacts from docs/api/openapi.codeflow.json")
    return 0
