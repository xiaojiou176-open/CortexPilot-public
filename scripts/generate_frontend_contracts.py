...
OPENAPI_PATH = ROOT / "docs" / "api" / "openapi.agentcoder.json"
...
def _load_contract_extension() -> dict:
    ...
    ext = payload.get("x-agentcoder-frontend-contract")
    if not isinstance(ext, dict):
        raise SystemExit(f"missing x-agentcoder-frontend-contract in {OPENAPI_PATH}")
    return ext
...
def main() -> int:
    ...
    print("generated frontend api contract artifacts from docs/api/openapi.agentcoder.json")
    return 0
