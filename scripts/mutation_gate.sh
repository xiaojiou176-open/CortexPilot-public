...
PROFILE_MODE="${AGENTCODER_MUTATION_PROFILE_MODE:-}"
if [[ -z "$PROFILE_MODE" ]]; then
  if [[ -n "${AGENTCODER_MUTATION_TARGET_FILE+x}" || -n "${AGENTCODER_MUTATION_CONFIG_FILE+x}" || -n "${AGENTCODER_MUTATION_TEST_CMD+x}" ]]; then
    PROFILE_MODE="single"
  else
    PROFILE_MODE="all"
  fi
...
if [[ "$PROFILE_MODE" == "all" ]]; then
  ...
  run_profile() {
    ...
    if ! AGENTCODER_MUTATION_PROFILE_MODE="single" \
      AGENTCODER_MUTATION_TARGET_FILE="$target" \
      AGENTCODER_MUTATION_CONFIG_FILE="$config" \
      AGENTCODER_MUTATION_REPORT_PATH="$report" \
      AGENTCODER_MUTATION_MIN_MUTANTS="$min_mutants" \
      AGENTCODER_MUTATION_MIN_KILL_RATE="$min_kill_rate" \
      AGENTCODER_MUTATION_TEST_CMD="$test_cmd" \
  ...
  run_profile \
    "tests_gate" \
    "$ROOT_DIR/apps/orchestrator/src/agentcoder_orch/gates/tests_gate.py" \
    "$ROOT_DIR/configs/mutation/tests_gate_mutants.json" \
    "$ROOT_DIR/.runtime-cache/test_output/mutation/mutation_gate_report.json" \
    "${AGENTCODER_MUTATION_MIN_MUTANTS:-20}" \
    "${AGENTCODER_MUTATION_MIN_KILL_RATE:-0.75}" \
  ...
  run_profile \
    ...
    "$ROOT_DIR/apps/orchestrator/src/agentcoder_orch/api/routes_runs.py" \
  ...
  run_profile \
    ...
    "${AGENTCODER_MUTATION_ROUTES_ADMIN_MIN_MUTANTS:-3}" \
    "${AGENTCODER_MUTATION_ROUTES_ADMIN_MIN_KILL_RATE:-0.75}" \
    "$PYTHON_BIN_ESCAPED -m pytest apps/orchestrator/tests/self_heal/test_cov_apps_orchestrator_src_agentcoder_orch_api_routes_admin.py apps/orchestrator/tests/test_api_main.py::test_api_god_mode_approve_requires_role_and_pending -q"

  run_profile \
    "routes_intake_write_paths" \
    "$ROOT_DIR/apps/orchestrator/src/agentcoder_orch/api/routes_intake.py" \
    ...
    "${AGENTCODER_MUTATION_ROUTES_INTAKE_MIN_MUTANTS:-3}" \
    "${AGENTCODER_MUTATION_ROUTES_INTAKE_MIN_KILL_RATE:-0.66}" \
...
TARGET_FILE="${AGENTCODER_MUTATION_TARGET_FILE:-$ROOT_DIR/apps/orchestrator/src/agentcoder_orch/gates/tests_gate.py}"
CONFIG_FILE="${AGENTCODER_MUTATION_CONFIG_FILE:-$ROOT_DIR/configs/mutation/tests_gate_mutants.json}"
REPORT_PATH="${AGENTCODER_MUTATION_REPORT_PATH:-$ROOT_DIR/.runtime-cache/test_output/mutation/mutation_gate_report.json}"
MIN_MUTANTS="${AGENTCODER_MUTATION_MIN_MUTANTS:-20}"
MIN_KILL_RATE="${AGENTCODER_MUTATION_MIN_KILL_RATE:-0.75}"
...
TEST_CMD="${AGENTCODER_MUTATION_TEST_CMD:-$DEFAULT_TEST_CMD}"
...
BACKUP_FILE="$(mktemp "${TMPDIR:-/tmp}/agentcoder-mutation-gate.XXXXXX")"
...
if min_mutants < 1:
    raise SystemExit("❌ [mutation-gate] invalid AGENTCODER_MUTATION_MIN_MUTANTS (must be >= 1)")
if min_kill_rate <= 0 or min_kill_rate > 1:
    raise SystemExit("❌ [mutation-gate] invalid AGENTCODER_MUTATION_MIN_KILL_RATE (must be in (0, 1])")
if not test_cmd:
