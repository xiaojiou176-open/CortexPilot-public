...
PROFILE_MODE="${CODEFLOW_MUTATION_PROFILE_MODE:-}"
if [[ -z "$PROFILE_MODE" ]]; then
  if [[ -n "${CODEFLOW_MUTATION_TARGET_FILE+x}" || -n "${CODEFLOW_MUTATION_CONFIG_FILE+x}" || -n "${CODEFLOW_MUTATION_TEST_CMD+x}" ]]; then
    PROFILE_MODE="single"
  else
    PROFILE_MODE="all"
  fi
...
if [[ "$PROFILE_MODE" == "all" ]]; then
  ...
  run_profile() {
    ...
    if ! CODEFLOW_MUTATION_PROFILE_MODE="single" \
      CODEFLOW_MUTATION_TARGET_FILE="$target" \
      CODEFLOW_MUTATION_CONFIG_FILE="$config" \
      CODEFLOW_MUTATION_REPORT_PATH="$report" \
      CODEFLOW_MUTATION_MIN_MUTANTS="$min_mutants" \
      CODEFLOW_MUTATION_MIN_KILL_RATE="$min_kill_rate" \
      CODEFLOW_MUTATION_TEST_CMD="$test_cmd" \
  ...
  run_profile \
    "tests_gate" \
    "$ROOT_DIR/apps/orchestrator/src/codeflow_orch/gates/tests_gate.py" \
    "$ROOT_DIR/configs/mutation/tests_gate_mutants.json" \
    "$ROOT_DIR/.runtime-cache/test_output/mutation/mutation_gate_report.json" \
    "${CODEFLOW_MUTATION_MIN_MUTANTS:-20}" \
    "${CODEFLOW_MUTATION_MIN_KILL_RATE:-0.75}" \
  ...
  run_profile \
    ...
    "$ROOT_DIR/apps/orchestrator/src/codeflow_orch/api/routes_runs.py" \
  ...
  run_profile \
    ...
    "${CODEFLOW_MUTATION_ROUTES_ADMIN_MIN_MUTANTS:-3}" \
    "${CODEFLOW_MUTATION_ROUTES_ADMIN_MIN_KILL_RATE:-0.75}" \
    "$PYTHON_BIN_ESCAPED -m pytest apps/orchestrator/tests/self_heal/test_cov_apps_orchestrator_src_codeflow_orch_api_routes_admin.py apps/orchestrator/tests/test_api_main.py::test_api_god_mode_approve_requires_role_and_pending -q"

  run_profile \
    "routes_intake_write_paths" \
    "$ROOT_DIR/apps/orchestrator/src/codeflow_orch/api/routes_intake.py" \
    ...
    "${CODEFLOW_MUTATION_ROUTES_INTAKE_MIN_MUTANTS:-3}" \
    "${CODEFLOW_MUTATION_ROUTES_INTAKE_MIN_KILL_RATE:-0.66}" \
...
TARGET_FILE="${CODEFLOW_MUTATION_TARGET_FILE:-$ROOT_DIR/apps/orchestrator/src/codeflow_orch/gates/tests_gate.py}"
CONFIG_FILE="${CODEFLOW_MUTATION_CONFIG_FILE:-$ROOT_DIR/configs/mutation/tests_gate_mutants.json}"
REPORT_PATH="${CODEFLOW_MUTATION_REPORT_PATH:-$ROOT_DIR/.runtime-cache/test_output/mutation/mutation_gate_report.json}"
MIN_MUTANTS="${CODEFLOW_MUTATION_MIN_MUTANTS:-20}"
MIN_KILL_RATE="${CODEFLOW_MUTATION_MIN_KILL_RATE:-0.75}"
...
TEST_CMD="${CODEFLOW_MUTATION_TEST_CMD:-$DEFAULT_TEST_CMD}"
...
BACKUP_FILE="$(mktemp "${TMPDIR:-/tmp}/codeflow-mutation-gate.XXXXXX")"
...
if min_mutants < 1:
    raise SystemExit("❌ [mutation-gate] invalid CODEFLOW_MUTATION_MIN_MUTANTS (must be >= 1)")
if min_kill_rate <= 0 or min_kill_rate > 1:
    raise SystemExit("❌ [mutation-gate] invalid CODEFLOW_MUTATION_MIN_KILL_RATE (must be in (0, 1])")
if not test_cmd:
