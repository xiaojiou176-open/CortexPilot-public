...
def _load_env_var_from_zsh_if_missing(key: str) -> None:
    if str(os.environ.get("CODEFLOW_UI_DISABLE_ZSH_ENV", "")).strip().lower() in {"1", "true", "yes", "on"}:
        return
    if os.environ.get(key, "").strip():
        return
    try:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--host", default=os.environ.get("CODEFLOW_E2E_HOST", "127.0.0.1"))
    parser.add_argument("--api-port", type=int, default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_API_PORT", "19600")))
    parser.add_argument(
        "--dashboard-port", type=int, default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_DASHBOARD_PORT", "19700"))
    )
    parser.add_argument("--external-api-base", default=os.environ.get("CODEFLOW_UI_FULL_E2E_EXTERNAL_API_BASE", ""))
    parser.add_argument(
        "--external-dashboard-base", default=os.environ.get("CODEFLOW_UI_FULL_E2E_EXTERNAL_DASHBOARD_BASE", "")
    )
    parser.add_argument(
        "--api-token",
        default=(
            os.environ.get("CODEFLOW_E2E_API_TOKEN")
            or os.environ.get("CODEFLOW_API_TOKEN")
            or "codeflow-e2e-token"
        ),
    )
    parser.add_argument("--gemini-model", default=os.environ.get("CODEFLOW_UI_GEMINI_MODEL", "gemini-3.0-flash"))
    parser.add_argument("--provider", default=os.environ.get("CODEFLOW_UI_AUDIT_PROVIDER", "gemini"))
    parser.add_argument("--provider-base-url", default=os.environ.get("CODEFLOW_UI_PROVIDER_BASE_URL", ""))
    parser.add_argument("--provider-model", default=os.environ.get("CODEFLOW_UI_PROVIDER_MODEL", ""))
    parser.add_argument("--thinking-level", default=os.environ.get("CODEFLOW_UI_GEMINI_THINKING_LEVEL", "high"))
    parser.add_argument("--gemini-key-env", default=os.environ.get("CODEFLOW_UI_GEMINI_KEY_ENV", "GEMINI_API_KEY"))
    parser.add_argument(
        "--gemini-request-timeout-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_GEMINI_REQUEST_TIMEOUT_SEC", "75")),
    )
    parser.add_argument(
        "--gemini-max-attempts",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_GEMINI_MAX_ATTEMPTS", "5")),
    )
    parser.add_argument("--max-pages", type=int, default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_PAGES", "0")))
    parser.add_argument(
        "--max-buttons-per-page",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_BUTTONS_PER_PAGE", "120")),
    )
    parser.add_argument(
        "--max-interactions",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_INTERACTIONS", "504")),
        help="Global interaction budget across all routes; 0 disables cap.",
    )
    parser.add_argument(
        "--max-duplicate-targets",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_DUPLICATE_TARGETS", "3")),
        help="Maximum number of targets retained per semantic signature on each route.",
    )
    parser.add_argument(
        "--navigation-timeout-ms", type=int, default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_NAV_TIMEOUT_MS", "30000"))
    )
    parser.add_argument(
        "--action-timeout-ms", type=int, default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_ACTION_TIMEOUT_MS", "10000"))
    )
    parser.add_argument("--page-settle-ms", type=int, default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_PAGE_SETTLE_MS", "1500")))
    parser.add_argument(
        "--interaction-settle-ms",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_INTERACTION_SETTLE_MS", "1200")),
    )
    parser.add_argument(
        "--max-runtime-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_MAX_RUNTIME_SEC", "0")),
        help="Hard runtime budget for one shard; 0 means no explicit runtime cap.",
    )
    parser.add_argument(
        "--heartbeat-interval-sec",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_HEARTBEAT_INTERVAL_SEC", "20")),
        help="Emit and persist heartbeat every N seconds while this shard is running.",
    )
    parser.add_argument("--run-id", default=os.environ.get("CODEFLOW_UI_FULL_E2E_RUN_ID", "").strip())
    parser.add_argument(
        "--route-shard-total",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_ROUTE_SHARD_TOTAL", "1")),
        help="Total shard count for route-level parallel execution.",
    )
    parser.add_argument(
        "--route-shard-index",
        type=int,
        default=int(os.environ.get("CODEFLOW_UI_FULL_E2E_ROUTE_SHARD_INDEX", "0")),
        help="Current shard index (0-based).",
    )
    parser.add_argument(
        "--headed",
        action="store_true",
        default=_env_flag("CODEFLOW_UI_FULL_E2E_HEADED", False),
...
def main() -> int:
    ...
    try:
        ...
        if use_external_servers:
            ...
            api_env["CODEFLOW_API_AUTH_REQUIRED"] = str(
                os.environ.get("CODEFLOW_UI_FULL_E2E_API_AUTH_REQUIRED", "false")
            ).strip().lower()
            api_env["CODEFLOW_API_TOKEN"] = args.api_token
            api_env["CODEFLOW_DASHBOARD_PORT"] = str(resolved_dashboard_port)
            ...
            dashboard_env["NEXT_PUBLIC_CODEFLOW_API_BASE"] = base_api
            dashboard_env["NEXT_PUBLIC_CODEFLOW_API_TOKEN"] = args.api_token
