...
if [ ! -f "${REPO_ROOT}/apps/orchestrator/src/agentcoder_orch/cli.py" ]; then
  printf '%s\n' "Expected a Agentcoder checkout at ${REPO_ROOT}." >&2
  exit 1
fi

PYTHON_BIN="${AGENTCODER_PYTHON:-$(agentcoder_python_bin "${REPO_ROOT}" || true)}"
if [ -z "${PYTHON_BIN}" ] || [ ! -x "${PYTHON_BIN}" ]; then
  printf '%s\n' "Managed Agentcoder Python toolchain not found." >&2
...
if ! "${PYTHON_BIN}" -c "import typer" >/dev/null 2>&1; then
  printf '%s\n' "Missing Python dependency 'typer' for Agentcoder orchestrator." >&2
  printf '%s\n' "Run 'npm run bootstrap:host' (or install the orchestrator Python deps) before starting the read-only MCP server." >&2
  exit 1
fi

exec "${PYTHON_BIN}" -m agentcoder_orch.cli mcp-readonly-server
