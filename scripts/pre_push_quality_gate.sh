...
resolve_pre_push_probe_provider_mode_or_fail() {
  local mode="${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_API_MODE:-require}"
  case "$mode" in
    require|auto|off)
      ;;
    *)
      echo "❌ [pre-push-quality-gate] unsupported CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_API_MODE=${mode}. expected: require|auto|off" >&2
      exit 1
      ;;
  esac
  ...
  if [[ "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS:-0}" != "1" ]]; then
    echo "❌ [pre-push-quality-gate] probe provider mode downgrade requires break-glass" >&2
    echo "Required env: CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS=1 + CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_REASON + CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_TICKET" >&2
    exit 1
  fi
  if [[ -z "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_REASON:-}" || -z "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_TICKET:-}" ]]; then
  ...
  audit_log_path="$(append_pre_push_break_glass_audit \
    ...
    "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_REASON}" \
    "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_TICKET}" \
    "provider_mode=${mode}")"
  echo "⚠️ [pre-push-quality-gate] probe provider mode downgraded via break-glass: mode=${mode}" >&2
  echo "   reason=${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_REASON}" >&2
  echo "   ticket=${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE_BREAK_GLASS_TICKET}" >&2
...
require_skip_precommit_break_glass_or_fail() {
  if [[ "${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS:-0}" != "1" ]]; then
    echo "❌ [pre-push-quality-gate] explicit CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES=1 requires break-glass" >&2
    echo "Required env: CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS=1 + CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_REASON + CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_TICKET" >&2
    exit 1
  fi
  if [[ -z "${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_REASON:-}" || -z "${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_TICKET:-}" ]]; then
  ...
  audit_log_path="$(append_pre_push_break_glass_audit \
    ...
    "${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_REASON}" \
    "${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_TICKET}" \
    "skip_precommit_gates=1")"
  echo "⚠️ [pre-push-quality-gate] pre-commit lint/doc gates skip approved via break-glass" >&2
  echo "   reason=${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_REASON}" >&2
  echo "   ticket=${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES_BREAK_GLASS_TICKET}" >&2
  echo "   audit_log=${audit_log_path}" >&2
}
...
skip_lint_doc_gates_request="${CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES:-auto}"
...
if [[ "$skip_lint_doc_gates_request" == "auto" ]]; then
  ...
  echo "❌ [pre-push-quality-gate] unsupported CODEFLOW_PRE_PUSH_SKIP_PRECOMMIT_GATES=${skip_lint_doc_gates_request}. expected: auto|1|0" >&2
  exit 1
fi
...
run_local_ci="${CODEFLOW_PRE_PUSH_RUN_CI_DOUBLE_CHECK:-0}"
if [[ "$run_local_ci" == "1" ]]; then
  ...
  test_mode="${CODEFLOW_PRE_PUSH_TEST_MODE:-incremental}"
  ...
  if [[ "$test_mode" == "incremental" ]]; then
    ...
    if incremental_output="$(
      python3 scripts/detect_changed_tests.py \
        --base-ref "${CODEFLOW_PRE_PUSH_BASE_REF:-origin/main}" \
...
EOF
    ...
    if printf '%s\n' "${incremental_lines[@]}" | rg -q '^(__ALL__|apps/orchestrator/tests)$'; then
      ...
      CODEFLOW_TEST_MODE=full bash ./scripts/test.sh
      ...
      CODEFLOW_TEST_MODE=incremental CODEFLOW_TEST_INCREMENTAL_FILES="$incremental_payload" bash ./scripts/test.sh
    fi
  else
  ...
  "$python_bin" scripts/e2e_external_web_probe.py \
    --url "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_URL:-https://example.com}" \
    --provider-api-mode "${PRE_PUSH_EXTERNAL_PROBE_PROVIDER_MODE}" \
    --hard-timeout-sec "${CODEFLOW_PRE_PUSH_EXTERNAL_PROBE_TIMEOUT_SEC:-120}"
  ...
  if [[ "${CODEFLOW_PRE_PUSH_BREAK_GLASS:-0}" != "1" ]]; then
    echo "❌ [pre-push-quality-gate] off mode requires break-glass" >&2
    echo "Set CODEFLOW_PRE_PUSH_BREAK_GLASS=1 with reason/ticket to bypass." >&2
    exit 1
  fi
  if [[ -z "${CODEFLOW_PRE_PUSH_BREAK_GLASS_REASON:-}" || -z "${CODEFLOW_PRE_PUSH_BREAK_GLASS_TICKET:-}" ]]; then
    echo "❌ [pre-push-quality-gate] break-glass requires reason and ticket" >&2
    echo "Required env: CODEFLOW_PRE_PUSH_BREAK_GLASS_REASON, CODEFLOW_PRE_PUSH_BREAK_GLASS_TICKET" >&2
    exit 1
  fi
  audit_log_path="$(append_pre_push_break_glass_audit \
    "pre_push_skip_local_ci_double_check" \
    "${CODEFLOW_PRE_PUSH_BREAK_GLASS_REASON}" \
    "${CODEFLOW_PRE_PUSH_BREAK_GLASS_TICKET}" \
    "run_local_ci=${run_local_ci}")"
  echo "⚠️ [pre-push-quality-gate] break-glass: skip all local verification bundles"
  echo "   reason=${CODEFLOW_PRE_PUSH_BREAK_GLASS_REASON}"
  echo "   ticket=${CODEFLOW_PRE_PUSH_BREAK_GLASS_TICKET}"
  echo "   audit_log=${audit_log_path}"
else
  echo "❌ [pre-push-quality-gate] invalid CODEFLOW_PRE_PUSH_RUN_CI_DOUBLE_CHECK=${run_local_ci} (expected: 0|1|off)" >&2
  exit 1
fi
