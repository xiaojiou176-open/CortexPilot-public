...
def _gemini_request_lock() -> Any:
    ...
    lock_path = Path(
        os.environ.get(
            "CODEFLOW_UI_GEMINI_REQUEST_LOCK",
            ".runtime-cache/codeflow/locks/ui_gemini_request.lock",
        )
    )
...
def _respect_gemini_min_interval() -> None:
    raw = str(os.environ.get("CODEFLOW_UI_GEMINI_MIN_INTERVAL_SEC", "2.5")).strip()
    try:
    ...
    stamp_path = Path(
        os.environ.get(
            "CODEFLOW_UI_GEMINI_REQUEST_STAMP",
            ".runtime-cache/codeflow/locks/ui_gemini_request.timestamp",
        )
    )
...
def _diagnose_extreme_tall_images(image_paths: list[Path]) -> dict[str, Any] | None:
    ratio_threshold = max(
        1.0,
        _parse_float_env("CODEFLOW_UI_GEMINI_EXTREME_TALL_RATIO", _EXTREME_TALL_ASPECT_RATIO),
    )
    min_height_px = max(
        1,
        _parse_int_env("CODEFLOW_UI_GEMINI_EXTREME_TALL_MIN_HEIGHT_PX", _EXTREME_TALL_MIN_HEIGHT_PX),
    )
...
class GeminiAnalyzer:
    ...
    def _cache_enabled(self) -> bool:
        return str(os.environ.get("CODEFLOW_GEMINI_CONTEXT_CACHE", "1")).strip().lower() not in {"0", "false", "no", "off"}

    def _cache_root(self) -> Path:
        raw = str(os.environ.get("CODEFLOW_GEMINI_CONTEXT_CACHE_DIR", "")).strip()
        if raw:
