...
MACHINE_TMP_ROOT="$(codeflow_machine_tmp_root "$ROOT_DIR")"
codeflow_maybe_auto_prune_machine_cache "$ROOT_DIR" "clean_room_recovery"
mkdir -p "$MACHINE_TMP_ROOT"
MACHINE_CACHE_ROOT="$(mktemp -d "$MACHINE_TMP_ROOT/clean-room-machine-cache.XXXXXX")"
export CODEFLOW_MACHINE_CACHE_ROOT="$MACHINE_CACHE_ROOT"
export CODEFLOW_TOOLCHAIN_CACHE_ROOT="$MACHINE_CACHE_ROOT/toolchains"
export CODEFLOW_PNPM_STORE_DIR="$MACHINE_CACHE_ROOT/pnpm-store"
export PLAYWRIGHT_BROWSERS_PATH="$MACHINE_CACHE_ROOT/playwright"
export CODEFLOW_CLEAN_ROOM_MACHINE_TMP_ROOT="$MACHINE_TMP_ROOT"
unset CODEFLOW_PYTHON
unset VIRTUAL_ENV
...
write_clean_room_report() {
  local exit_code="$1"
  python3 - <<'PY' "$CLEAN_ROOM_REPORT_PATH" "$exit_code" "$CLEAN_ROOM_STATUS" "${CODEFLOW_CLEAN_ROOM_MACHINE_TMP_ROOT:-}" "${CODEFLOW_MACHINE_CACHE_ROOT:-}" "${CODEFLOW_CLEAN_ROOM_PRESERVE_ROOT:-}"
import json
import sys
...
export CODEFLOW_CLEAN_ROOM_PRESERVE_ROOT="$CLEAN_ROOM_PRESERVE_ROOT"
...
preserve_runtime_path ".runtime-cache/codeflow/reports/ci"
preserve_runtime_path ".runtime-cache/codeflow/release"
...
PYTHON_BIN="$(bash -lc "source \"$ROOT_DIR/scripts/lib/toolchain_env.sh\" && codeflow_python_bin \"$ROOT_DIR\"")"
...
CODEFLOW_HYGIENE_SKIP_UPSTREAM=1 bash "$ROOT_DIR/scripts/check_repo_hygiene.sh"

bash "$ROOT_DIR/scripts/cleanup_workspace_modules.sh" >/dev/null 2>&1 || true

restore_runtime_path ".runtime-cache/test_output/governance"
restore_runtime_path ".runtime-cache/codeflow/reports/ci"
restore_runtime_path ".runtime-cache/codeflow/release"
