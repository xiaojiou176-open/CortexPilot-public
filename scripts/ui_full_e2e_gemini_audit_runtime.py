...
def _ensure_playwright_tmpdir(scope: str = "ui-full-e2e") -> None:
    ...
    if not temp_root:
        temp_root = str(ROOT / ".runtime-cache" / "codeflow" / "temp")
...
def _playwright_browser_bootstrap_lock() -> Any:
    ...
    lock_path = Path(
            os.environ.get(
                "CODEFLOW_UI_PLAYWRIGHT_BOOTSTRAP_LOCK",
                str(ROOT / ".runtime-cache" / "codeflow" / "locks" / "ui_playwright_browser_bootstrap.lock"),
            )
    )
