...
HYGIENE_SKIP_UPSTREAM="${AGENTCODER_HYGIENE_SKIP_UPSTREAM:-}"
if [[ -z "$HYGIENE_SKIP_UPSTREAM" ]] && { is_truthy "${AGENTCODER_CI_CONTAINER:-0}" || is_truthy "${AGENTCODER_HOST_COMPAT:-0}"; }; then
...
PYTHON_BIN="$(agentcoder_python_bin "$ROOT_DIR")" || {
...
export AGENTCODER_PYTHON="${AGENTCODER_PYTHON:-$PYTHON_BIN}"
...
AGENTCODER_HYGIENE_SKIP_UPSTREAM="$HYGIENE_SKIP_UPSTREAM" bash scripts/check_repo_hygiene.sh
...
TEST_MODE="${AGENTCODER_TEST_MODE:-full}"
INCREMENTAL_FILES="${AGENTCODER_TEST_INCREMENTAL_FILES:-}"
...
EOF
  ...
  SKIP_COVERAGE_GATE="${AGENTCODER_TEST_INCREMENTAL_SKIP_COVERAGE:-1}"
else
...
if ! [[ "$COV_FAIL_UNDER" =~ ^[0-9]+$ ]]; then
  echo "❌ [ERROR] AGENTCODER_COV_FAIL_UNDER must be an integer; current value: $COV_FAIL_UNDER" >&2
  exit 1
fi
if ! [[ "$MIN_DEFAULT_COVERAGE_FLOOR" =~ ^[0-9]+$ ]]; then
  echo "❌ [ERROR] AGENTCODER_DEFAULT_COVERAGE_FLOOR must be an integer; current value: $MIN_DEFAULT_COVERAGE_FLOOR" >&2
  exit 1
fi
if [[ "$MIN_DEFAULT_COVERAGE_FLOOR" -lt "$DEFAULT_COVERAGE_HARD_FLOOR" ]]; then
  echo "❌ [ERROR] AGENTCODER_DEFAULT_COVERAGE_FLOOR cannot be lower than hard floor ${DEFAULT_COVERAGE_HARD_FLOOR}; current value: $MIN_DEFAULT_COVERAGE_FLOOR" >&2
  exit 1
fi
...
if ! [[ "$ORCH_CORE_COV_FAIL_UNDER" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
  echo "❌ [ERROR] AGENTCODER_ORCH_CORE_COV_FAIL_UNDER must be numeric; current value: $ORCH_CORE_COV_FAIL_UNDER" >&2
  exit 1
fi
...
PY
then
  echo "❌ [ERROR] AGENTCODER_ORCH_CORE_COV_FAIL_UNDER cannot be lower than hard floor ${ORCH_CORE_COVERAGE_HARD_FLOOR}; current value: $ORCH_CORE_COV_FAIL_UNDER" >&2
  exit 1
fi
load_orchestrator_critical_modules_config_or_fail
PYTEST_DIST_MODE="${AGENTCODER_PYTEST_DIST_MODE:-loadscope}"
...
PYTEST_RESERVE_CPUS="${AGENTCODER_PYTEST_RESERVE_CPUS:-1}"
PYTEST_MIN_WORKERS="${AGENTCODER_PYTEST_MIN_WORKERS:-1}"
PYTEST_MAX_WORKERS="${AGENTCODER_PYTEST_MAX_WORKERS:-0}"
if [[ ! "$PYTEST_RESERVE_CPUS" =~ ^[0-9]+$ ]]; then
  echo "❌ [ERROR] AGENTCODER_PYTEST_RESERVE_CPUS must be a non-negative integer; current value: $PYTEST_RESERVE_CPUS" >&2
  exit 1
fi
if [[ ! "$PYTEST_MIN_WORKERS" =~ ^[1-9][0-9]*$ ]]; then
  echo "❌ [ERROR] AGENTCODER_PYTEST_MIN_WORKERS must be a positive integer; current value: $PYTEST_MIN_WORKERS" >&2
  exit 1
fi
if [[ "$PYTEST_MAX_WORKERS" =~ ^[1-9][0-9]*$ ]] && [[ "$PYTEST_MAX_WORKERS" -lt "$PYTEST_MIN_WORKERS" ]]; then
  echo "❌ [ERROR] AGENTCODER_PYTEST_MAX_WORKERS cannot be smaller than AGENTCODER_PYTEST_MIN_WORKERS; current value: max=$PYTEST_MAX_WORKERS min=$PYTEST_MIN_WORKERS" >&2
  exit 1
fi
...
PYTEST_WORKERS="${AGENTCODER_PYTEST_WORKERS:-$PYTEST_AUTO_WORKERS}"
if [[ ! "$PYTEST_WORKERS" =~ ^[1-9][0-9]*$ ]]; then
  echo "❌ [ERROR] AGENTCODER_PYTEST_WORKERS must be a positive integer; current value: $PYTEST_WORKERS" >&2
  exit 1
fi
if [[ "$PYTEST_MAX_WORKERS" =~ ^[1-9][0-9]*$ ]] && [[ "$PYTEST_WORKERS" -gt "$PYTEST_MAX_WORKERS" ]]; then
  echo "❌ [ERROR] AGENTCODER_PYTEST_WORKERS exceeds AGENTCODER_PYTEST_MAX_WORKERS; current value: workers=$PYTEST_WORKERS max=$PYTEST_MAX_WORKERS" >&2
  exit 1
fi
PYTEST_PARALLEL_ARGS="${AGENTCODER_PYTEST_PARALLEL_ARGS:--n $PYTEST_WORKERS --dist $PYTEST_DIST_MODE}"
...
set +e
heartbeat_pid=""
start_heartbeat "test.sh:orchestrator-pytest-parallel" "${AGENTCODER_TEST_HEARTBEAT_INTERVAL_SEC:-45}" heartbeat_pid

# Build pytest command based on test mode
if [[ "$SKIP_COVERAGE_GATE" == "1" ]]; then
  echo "ℹ️ [INFO][COVERAGE] skipped in incremental mode (set AGENTCODER_TEST_INCREMENTAL_SKIP_COVERAGE=0 to enable)"
...
if [[ "$parallel_status" -ne 0 ]]; then
  ...
  if [[ "$should_serial_recheck" -ne 1 ]]; then
    echo "❌ [ERROR][category:$parallel_failure_category] ${parallel_failure_reason}; skipping serial recheck (set AGENTCODER_FORCE_SERIAL_RECHECK=1 to force it)" >&2
  ...
  set +e
  heartbeat_pid=""
  start_heartbeat "test.sh:orchestrator-pytest-serial-recheck" "${AGENTCODER_TEST_HEARTBEAT_INTERVAL_SEC:-45}" heartbeat_pid
