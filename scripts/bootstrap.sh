...
codeflow_maybe_auto_prune_machine_cache "$ROOT_DIR" "bootstrap:${MODE}"

resolve_pnpm_store_dir() {
  codeflow_pnpm_store_dir "$ROOT_DIR"
}
...
PLAYWRIGHT_BROWSERS_PATH_RESOLVED="$(codeflow_playwright_browsers_path "$ROOT_DIR")"
...
if [[ "$need_python_venv" -eq 1 ]]; then
  local_venv_root="$(codeflow_python_venv_root "$ROOT_DIR")"
  mkdir -p "$(dirname "$local_venv_root")"
  if python_venv_needs_rebuild "$local_venv_root"; then
    rm -rf "$local_venv_root"
  fi
  bootstrap_python_bin="$(codeflow_bootstrap_python_bin)" || {
    echo "❌ bootstrap requires python3 (or CODEFLOW_BOOTSTRAP_PYTHON) to create the managed toolchain" >&2
    exit 1
  }
