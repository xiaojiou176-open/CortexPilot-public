...
def _parse_strict_allowlist(path: Path) -> list[str]:
    ...
    match = re.search(r"STRICT_CI_AGENTCODER_ENV_ALLOWLIST=\((.*?)\)", text, re.S)
    if not match:
        return []
    items = []
...
def main() -> int:
    ...
    _require("AGENTCODER_CI_RUNNER_CLASS: self_hosted" not in workflow_text, errors, "legacy self_hosted runner_class env still present in workflow")
    ...
    expected_allowlist = strict_env.get("allowlisted_agentcoder_env") or []
    parsed_allowlist = _parse_strict_allowlist(docker_ci_path)
    _require(parsed_allowlist == expected_allowlist, errors, "strict_env_contract.allowlisted_agentcoder_env drift vs scripts/docker_ci.sh")
    if strict_env.get("forbid_dotenv_fallback") is True:
        ...
        _require('append_strict_ci_agentcoder_allowlist' in docker_ci_text, errors, "docker_ci strict allowlist hook missing")
    ...
    _require("AGENTCODER_CI_ROUTE_ID" in docker_ci_text, errors, "docker_ci must pass route metadata into strict container path")
