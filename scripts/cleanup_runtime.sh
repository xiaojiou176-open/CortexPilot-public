...
PROFILE="${AGENTCODER_CLEANUP_PROFILE:-default}"
MODE_INPUT="${1:-}"
...
CLEAN_ROOT_NOISE="${AGENTCODER_CLEANUP_ROOT_NOISE:-1}"
CLEAN_CONTRACT_ARTIFACTS="${AGENTCODER_CLEANUP_CONTRACT_ARTIFACTS:-0}"
CONFIRM_APPLY="${AGENTCODER_CLEANUP_CONFIRM:-}"
...
validate_toggle_01 "AGENTCODER_CLEANUP_ROOT_NOISE" "$CLEAN_ROOT_NOISE"
validate_toggle_01 "AGENTCODER_CLEANUP_CONTRACT_ARTIFACTS" "$CLEAN_CONTRACT_ARTIFACTS"
...
if [[ "$MODE" == "apply" ]]; then
  if [[ "$PROFILE" == "nightly" ]]; then
    ...
    echo "❌ [cleanup] apply mode requires AGENTCODER_CLEANUP_CONFIRM=YES" >&2
    exit 2
  fi
fi
...
root = Path(".runtime-cache/agentcoder/reports/ci")
if not root.exists():
    raise SystemExit(0)
...
for path in sorted(root.rglob("*.json")):
    ...
    if not report_type.startswith("agentcoder_ci_"):
        continue
    if report_type in {"agentcoder_ci_current_run_source_manifest", "agentcoder_ci_route_report"}:
        continue
...
if [[ "$MODE" == "apply" ]]; then
  PYTHONPATH=apps/orchestrator/src "$PYTHON" -m agentcoder_orch.cli cleanup runtime --apply
else
  PYTHONPATH=apps/orchestrator/src "$PYTHON" -m agentcoder_orch.cli cleanup runtime --dry-run
fi
...
if [[ "$CLEAN_ROOT_NOISE" == "1" ]]; then
  ...
  echo "ℹ️ [cleanup] skip root noise cleanup (AGENTCODER_CLEANUP_ROOT_NOISE=0)"
fi
...
PY
	    ...
	    for base in \
	      ...
	      ".runtime-cache/agentcoder/contracts/results" ".runtime-cache/agentcoder/contracts/reviews" ".runtime-cache/agentcoder/contracts/tasks"; do
      [[ -d "$base" ]] || continue
      find "$base" -maxdepth 1 \( -type d -name 'run_*' -o -type f -name 'task-*.json' \) -print
    done | LC_ALL=C sort -u
  )
else
  echo "ℹ️ [cleanup] skip contract artifacts cleanup (set AGENTCODER_CLEANUP_CONTRACT_ARTIFACTS=1 to enable)"
fi
