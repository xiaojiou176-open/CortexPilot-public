...
PROFILE="${CODEFLOW_CLEANUP_PROFILE:-default}"
MODE_INPUT="${1:-}"
...
CLEAN_ROOT_NOISE="${CODEFLOW_CLEANUP_ROOT_NOISE:-1}"
CLEAN_CONTRACT_ARTIFACTS="${CODEFLOW_CLEANUP_CONTRACT_ARTIFACTS:-0}"
CONFIRM_APPLY="${CODEFLOW_CLEANUP_CONFIRM:-}"
...
validate_toggle_01 "CODEFLOW_CLEANUP_ROOT_NOISE" "$CLEAN_ROOT_NOISE"
validate_toggle_01 "CODEFLOW_CLEANUP_CONTRACT_ARTIFACTS" "$CLEAN_CONTRACT_ARTIFACTS"
...
if [[ "$MODE" == "apply" ]]; then
  if [[ "$PROFILE" == "nightly" ]]; then
    ...
    echo "❌ [cleanup] apply mode requires CODEFLOW_CLEANUP_CONFIRM=YES" >&2
    exit 2
  fi
fi
...
root = Path(".runtime-cache/codeflow/reports/ci")
if not root.exists():
    raise SystemExit(0)
...
for path in sorted(root.rglob("*.json")):
    ...
    if not report_type.startswith("codeflow_ci_"):
        continue
    if report_type in {"codeflow_ci_current_run_source_manifest", "codeflow_ci_route_report"}:
        continue
...
if [[ "$MODE" == "apply" ]]; then
  PYTHONPATH=apps/orchestrator/src "$PYTHON" -m codeflow_orch.cli cleanup runtime --apply
else
  PYTHONPATH=apps/orchestrator/src "$PYTHON" -m codeflow_orch.cli cleanup runtime --dry-run
fi
...
if [[ "$CLEAN_ROOT_NOISE" == "1" ]]; then
  ...
  echo "ℹ️ [cleanup] skip root noise cleanup (CODEFLOW_CLEANUP_ROOT_NOISE=0)"
fi
...
PY
	    ...
	    for base in \
	      ...
	      ".runtime-cache/codeflow/contracts/results" ".runtime-cache/codeflow/contracts/reviews" ".runtime-cache/codeflow/contracts/tasks"; do
      [[ -d "$base" ]] || continue
      find "$base" -maxdepth 1 \( -type d -name 'run_*' -o -type f -name 'task-*.json' \) -print
    done | LC_ALL=C sort -u
  )
else
  echo "ℹ️ [cleanup] skip contract artifacts cleanup (set CODEFLOW_CLEANUP_CONTRACT_ARTIFACTS=1 to enable)"
fi
