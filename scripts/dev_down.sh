...
PID_FILE="$ROOT_DIR/.runtime-cache/agentcoder/temp/agentcoder-dev-api.pid"
HOST="${AGENTCODER_DEV_HOST:-127.0.0.1}"
DASHBOARD_PORT="${AGENTCODER_DASHBOARD_PORT:-3100}"
TAURI_DEV_PORT="${AGENTCODER_TAURI_DEV_PORT:-1420}"
...
stop_port_listener_if_repo_process() {
  ...
  for pid in $pids; do
    ...
    if [[ "$cmd" == *"$ROOT_DIR/apps/dashboard"* || "$cmd" == *"$ROOT_DIR/apps/desktop"* || ( -n "$PYTHON_BIN" && "$cmd" == *"$PYTHON_BIN"* ) || "$cmd" == *"agentcoder_orch.cli serve"* ]]; then
...
stop_repo_dev_processes_by_pattern "pnpm --dir apps/dashboard dev|agentcoder-dashboard@0\\.1\\.0 dev" "Dashboard pnpm wrapper"
stop_repo_dev_processes_by_pattern "npm --prefix apps/desktop run tauri:dev|agentcoder-desktop@0\\.1\\.0 tauri:dev" "Desktop tauri npm wrapper"
...
for pid in $(ps -Ao pid,command | rg "${PYTHON_BIN:-$ROOT_DIR/.runtime-cache/cache/toolchains/python/current/bin/python} -m agentcoder_orch\\.cli serve --host $HOST --port 19[0-9]{3}" | awk '{print $1}'); do
