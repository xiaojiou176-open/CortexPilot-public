...
STORE_DIR="${CODEFLOW_PNPM_STORE_DIR:-$(codeflow_pnpm_store_dir "$ROOT_DIR")/desktop}"
INSTALL_NODE_LINKER="${CODEFLOW_DESKTOP_PNPM_NODE_LINKER:-hoisted}"
INSTALL_PACKAGE_IMPORT_METHOD="${CODEFLOW_DESKTOP_PNPM_IMPORT_METHOD:-copy}"
INSTALL_SHAMEFULLY_HOIST="${CODEFLOW_DESKTOP_PNPM_SHAMEFULLY_HOIST:-1}"
...
LOCK_DIR="$ROOT_DIR/.runtime-cache/codeflow/locks/install-desktop-deps.lock"
LOCK_OWNER_FILE="$LOCK_DIR/owner"
LOCK_HELD=0
MIN_ENOSPC_RECOVERY_HEADROOM_GIB="${CODEFLOW_DESKTOP_ENOSPC_MIN_HEADROOM_GIB:-3}"
WORKSPACE_RETRY_STORE_ACTIVE=0

codeflow_maybe_auto_prune_machine_cache "$ROOT_DIR" "install_desktop_deps"
...
resolve_writable_store_dir() {
  ...
  codeflow_pnpm_local_retry_dir "$ROOT_DIR" "desktop"
}
...
cleanup_stale_retry_stores() {
  local retry_prefix
  retry_prefix="$(codeflow_pnpm_local_retry_prefix "$ROOT_DIR" "desktop")"
  local candidate=""
  shopt -s nullglob
...
recover_with_workspace_store() {
  ...
  INSTALL_PACKAGE_IMPORT_METHOD="${CODEFLOW_DESKTOP_ENOSPC_IMPORT_METHOD:-hardlink}"
...
reset_app_node_modules() {
  ...
  local quarantine_root="$ROOT_DIR/.runtime-cache/codeflow/temp/install-desktop-deps"
