...
These examples package the current truthful Codeflow adoption paths for
...
- keep ecosystem reality separate from Codeflow publication state
...
examples/coding-agents/
  ...
  claude-code/
    README.md
    project.mcp.json
    .claude/commands/codeflow-proof.md
    .claude/agents/codeflow-reviewer.md
  mcp/readonly.mcp.json.example
  openclaw/README.md
  openclaw/codeflow-server.json
  openclaw/config.openclaw.example.toml
  plugin-bundles/codeflow-coding-agent-bundle/
    .codex-plugin/plugin.json
    .claude-plugin/plugin.json
    .mcp.json
    README.md
    skills/codeflow-adoption-router/SKILL.md
    skills/codeflow-adoption-router/manifest.yaml
```
...
2. Replace `__CODEFLOW_REPO_ROOT__` with the absolute path to your
   Codeflow checkout.
...
`plugin-bundles/codeflow-coding-agent-bundle/` when you want the smallest
local plugin-bundle installation that ships Codeflow skills and the same
...
registry-shaped `manifest.yaml` for the bundled `codeflow-adoption-router`
...
- one bundle-shipped `manifest.yaml` for the shared `codeflow-adoption-router`
  skill

## OpenClaw

OpenClaw can consume the same compatible bundle used for the Codex local plugin
example. Start with `openclaw/README.md`, `openclaw/codeflow-server.json`,
and `openclaw/config.openclaw.example.toml`.

The truthful story is:

- OpenClaw has native plugin and skills surfaces
- Codeflow currently ships a compatible local bundle example, while the
...
- that bundle now includes a registry-shaped `manifest.yaml` for the shared
  `codeflow-adoption-router` skill
