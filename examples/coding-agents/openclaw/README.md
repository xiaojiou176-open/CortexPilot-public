...
truthful Agentcoder position inside that ecosystem.
...
1. Use the compatible bundle at:
   ...
   ```text
   examples/coding-agents/plugin-bundles/agentcoder-coding-agent-bundle/
   ```

   That bundle now includes
   `skills/agentcoder-adoption-router/manifest.yaml`, so the same repo-owned
...
2. Pair it with one of the tracked OpenClaw MCP/config examples:
   ...
   ```text
   examples/coding-agents/openclaw/agentcoder-server.json
   examples/coding-agents/openclaw/config.openclaw.example.toml
   ```

3. Keep Agentcoder on the proof / replay / read-only integration side unless a
   native published OpenClaw path is explicitly shipped and tested.

## Expected success

- the OpenClaw config resolves to your local Agentcoder checkout and the same
...
- It helps OpenClaw workflows reuse Agentcoder skills and read-only MCP.
- It does not upgrade Agentcoder into a hosted operator or write-capable MCP
  product.
