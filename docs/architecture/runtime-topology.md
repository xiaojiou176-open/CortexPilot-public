...
- Retention reports now also expose `test_output_visibility` plus `cleanup_scope.observed_roots.test_output`, so `.runtime-cache/test_output/` is no longer a blind spot even before any heavier cleanup policy is promoted.
...
- Space-governance reports write repo/external/shared-layer audit snapshots under `.runtime-cache/cortexpilot/reports/space_governance/`, while cleanup gate receipts live under `.runtime-cache/test_output/space_governance/`.
...
- JS runtime machine state is app-local and explicit: only `apps/dashboard/node_modules`, `apps/dashboard/.next`, `apps/desktop/node_modules`, `apps/desktop/dist`, and `apps/desktop/tsconfig.tsbuildinfo` are allowed repo-local machine-managed surfaces; root `node_modules` remains forbidden.
