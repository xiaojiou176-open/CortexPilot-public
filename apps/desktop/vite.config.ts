...
const coverageRunId = process.env.CODEFLOW_DESKTOP_COVERAGE_RUN_ID ?? `${process.pid}`;
const coverageReportsDirectory = process.env.CODEFLOW_DESKTOP_COVERAGE_DIR ?? path.join("coverage", `run-${coverageRunId}`);
const shouldEmitHtmlCoverage = !process.env.CI || process.env.CODEFLOW_COVERAGE_HTML === "1";
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "@codeflow/frontend-shared/uiCopy": path.resolve(process.cwd(), "../../packages/frontend-shared/uiCopy.ts"),
      "@codeflow/frontend-shared/uiLocale": path.resolve(process.cwd(), "../../packages/frontend-shared/uiLocale.ts"),
    },
  },
  build: {
    // Keep debuggability without exposing full source maps by default.
    sourcemap: process.env.CODEFLOW_DESKTOP_SOURCEMAP === "full" ? true : "hidden",
    rollupOptions: {
      output: {
