...
describe("Desktop command center shell", { timeout: 15000 }, () => {
  ...
  it("toggles desktop chrome locale and persists the preference", async () => {
    ...
    expect(window.localStorage.getItem("codeflow.ui.locale")).toBe("zh-CN");
  });
  ...
  it("keeps page-level Alt+Shift shortcuts on command tower without triggering app-level Alt routing", async () => {
    ...
    await screen.findByText(
      /Codeflow 的桌面端聚焦执行与操作决策；更深的治理分析仍留给 Web 视图。|Codeflow on desktop stays focused on execution and operator decisions/,
    );
  ...
  it("restores recoverable draft from localStorage", async () => {
    ...
    window.localStorage.setItem(
      "codeflow.desktop.draft:codeflow-main:pm-live-1",
      "这是未提交的草稿"
    );
    render(<App />);
    await navigateToPmEntry(user);
  ...
  it("discards recoverable draft and removes prompt", async () => {
    const user = userEvent.setup();
    const draftKey = "codeflow.desktop.draft:codeflow-main:pm-live-1";
  ...
  it("shows onboarding banner once and allows dismiss", async () => {
    ...
    expect(window.localStorage.getItem("codeflow.desktop.onboarding.dismissed")).toBe("1");
  ...
  it("renders Command Tower three primary actions", async () => {
    ...
    await screen.findByText(/Codeflow 的桌面端聚焦执行与操作决策；更深的治理分析仍留给 Web 视图。|Codeflow on desktop stays focused on execution and operator decisions/);
