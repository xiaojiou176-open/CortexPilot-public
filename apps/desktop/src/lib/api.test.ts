...
describe("desktop api client", () => {
  ...
  it("uses bearer header for SSE stream when desktop api token is present", async () => {
    const previousToken = process.env.VITE_CODEFLOW_API_TOKEN;
    process.env.VITE_CODEFLOW_API_TOKEN = "desktop-token";
    vi.resetModules();
    ...
    if (previousToken === undefined) delete process.env.VITE_CODEFLOW_API_TOKEN;
    else process.env.VITE_CODEFLOW_API_TOKEN = previousToken;
  });

  it("uses refreshed bearer token after env rotation within same module instance", async () => {
    const previousToken = process.env.VITE_CODEFLOW_API_TOKEN;
    process.env.VITE_CODEFLOW_API_TOKEN = "desktop-token-old";
    vi.resetModules();
    ...
    process.env.VITE_CODEFLOW_API_TOKEN = "desktop-token-new";
  ...
  it("uses contract-backed role-config paths and desktop operator role for mutations", async () => {
    const previousRole = process.env.VITE_CODEFLOW_OPERATOR_ROLE;
    process.env.VITE_CODEFLOW_OPERATOR_ROLE = "ops";
    vi.resetModules();
    ...
    expect((calls[1][1]?.headers as Record<string, string>)["x-codeflow-role"]).toBe("OPS");
    expect((calls[2][1]?.headers as Record<string, string>)["x-codeflow-role"]).toBe("OPS");
    expect(api.mutationExecutionCapability()).toEqual({ executable: true, operatorRole: "OPS" });

    if (previousRole === undefined) delete process.env.VITE_CODEFLOW_OPERATOR_ROLE;
    else process.env.VITE_CODEFLOW_OPERATOR_ROLE = previousRole;
  });
  ...
  it("uses EventSource branch when token is absent and keeps query params", () => {
    const previousToken = process.env.VITE_CODEFLOW_API_TOKEN;
    delete process.env.VITE_CODEFLOW_API_TOKEN;
    vi.resetModules();
    class MockEventSource {
      url: string;
      constructor(url: string) {
