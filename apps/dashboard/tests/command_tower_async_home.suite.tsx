...
describe("command tower async hardening (home)", () => {
  ...
  it("applies status/project/sort filters into live polling query", async () => {
    ...
    fireEvent.click(failedCheckbox);
    fireEvent.click(failedCheckbox);
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. agentcoder|agentcoder/i), {
      target: { value: "agentcoder" },
    });
    ...
    await waitFor(() => {
      ...
      expect(lastCall?.projectKey).toBe("agentcoder");
  ...
  it("hydrates filters/live/focus from URL search params on first load", async () => {
    ...
    try {
      window.history.pushState(
        {},
        "",
        "/command-tower?status[]=failed&project_key=agentcoder&sort=failed_desc&focus=blocked&live=0",
      );
  ...
  it("resets applied filters through drawer reset action", async () => {
    ...
    const projectInput = screen.getByPlaceholderText(/e\.g\. agentcoder|agentcoder/i);
    fireEvent.change(projectInput, { target: { value: "agentcoder" } });
  ...
  it("supports keyboard shortcuts and live toggle aria state", async () => {
    ...
    expect(screen.getByPlaceholderText(/例如：agentcoder|agentcoder/)).toHaveValue("");
  });
  ...
  it("covers home alerts fallback fields and generated_at fallback", async () => {
    ...
    await ensureDrawerOpen();
    fireEvent.keyDown(screen.getByPlaceholderText(/e\.g\. agentcoder|agentcoder/i), { key: "x" });
