...
describe("command tower async hardening (home)", () => {
  ...
  it("applies status/project/sort filters into live polling query", async () => {
    ...
    fireEvent.click(failedCheckbox);
    fireEvent.click(failedCheckbox);
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. codeflow|codeflow/i), {
      target: { value: "codeflow" },
    });
    ...
    await waitFor(() => {
      ...
      expect(lastCall?.projectKey).toBe("codeflow");
  ...
  it("hydrates filters/live/focus from URL search params on first load", async () => {
    ...
    try {
      window.history.pushState(
        {},
        "",
        "/command-tower?status[]=failed&project_key=codeflow&sort=failed_desc&focus=blocked&live=0",
      );
  ...
  it("resets applied filters through drawer reset action", async () => {
    ...
    const projectInput = screen.getByPlaceholderText(/e\.g\. codeflow|codeflow/i);
    fireEvent.change(projectInput, { target: { value: "codeflow" } });
  ...
  it("supports keyboard shortcuts and live toggle aria state", async () => {
    ...
    expect(screen.getByPlaceholderText(/例如：codeflow|codeflow/)).toHaveValue("");
  });
  ...
  it("covers home alerts fallback fields and generated_at fallback", async () => {
    ...
    await ensureDrawerOpen();
    fireEvent.keyDown(screen.getByPlaceholderText(/e\.g\. codeflow|codeflow/i), { key: "x" });
