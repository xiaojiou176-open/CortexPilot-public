  ...
  it("covers all GET wrappers with expected routes", async () => {
    ...
    const getCases: Array<{
      ...
      },
      {
        ...
        path: "/api/pm/sessions?status%5B%5D=active&status=active&status%5B%5D=failed&status=failed&project_key=codeflow&sort=blocked_desc",
        invoke: () => fetchPmSessions({ status: ["active", "failed"], projectKey: "codeflow", sort: "blocked_desc" }),
      },
