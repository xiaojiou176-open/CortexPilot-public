...
describe("pm page chat-driven flow", () => {
  ...
  it("merges local chat bubbles with remote timeline messages", async () => {
    ...
    expect(within(remoteBubble as HTMLElement).getByText("Codeflow Command Tower")).toBeInTheDocument();
  });
    ...
    await waitFor(() => {
      expect(mockCreateIntake).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          constraints: ["workspace=apps/dashboard", "repo=codeflow", "safe", "deterministic"],
