...
type DashboardPublicEnvKey =
  ...
  | "NEXT_PUBLIC_AGENTCODER_PUBLIC_DOCS_BASE_URL"
  | "NEXT_PUBLIC_AGENTCODER_OPERATOR_ROLE"
  | "NEXT_PUBLIC_API_BASE"
  | "NEXT_PUBLIC_PM_COPY_VARIANT";

const DEFAULT_DASHBOARD_PUBLIC_DOCS_BASE_URL = "https://xiaojiou176-open.github.io/agentcoder";
...
function readPublicEnv(key: DashboardPublicEnvKey): string {
  switch (key) {
    case "NEXT_PUBLIC_AGENTCODER_API_BASE":
      return String(process.env.NEXT_PUBLIC_AGENTCODER_API_BASE || "").trim();
    case "NEXT_PUBLIC_AGENTCODER_PUBLIC_DOCS_BASE_URL":
      return String(process.env.NEXT_PUBLIC_AGENTCODER_PUBLIC_DOCS_BASE_URL || "").trim();
    case "NEXT_PUBLIC_AGENTCODER_OPERATOR_ROLE":
      return String(process.env.NEXT_PUBLIC_AGENTCODER_OPERATOR_ROLE || "").trim();
...
export function resolveDashboardApiBase(): string {
  const candidates = [
    readPublicEnv("NEXT_PUBLIC_AGENTCODER_API_BASE"),
...
export function resolveDashboardPublicDocsBaseUrl(): string {
  const configuredBase = readPublicEnv("NEXT_PUBLIC_AGENTCODER_PUBLIC_DOCS_BASE_URL");
  if (configuredBase) {
    if (configuredBase === "/") {
      return "";
    }
...
export function resolveDashboardOperatorRoleEnv(): string {
  const candidates = [readPublicEnv("NEXT_PUBLIC_AGENTCODER_OPERATOR_ROLE")];
  for (const candidate of candidates) {
    if (candidate) {
