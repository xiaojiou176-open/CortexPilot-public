...
from codeflow_orch.api.main import app as api_app
...
from codeflow_orch.mcp_queue_pilot_server import serve_queue_pilot_mcp
from codeflow_orch.mcp_readonly_server import serve_readonly_mcp
from codeflow_orch.planning.coverage_chain import (
...
from codeflow_orch import cli_command_helpers, cli_coverage_helpers, cli_runtime_helpers
...
@app.command()
def doctor() -> None:
    table = Table(title="codeflow doctor")
...
def run(
    ...
    prev_force_unlock = os.environ.get("CODEFLOW_FORCE_UNLOCK")
    prev_runner = os.environ.get("CODEFLOW_RUNNER")
    try:
        if force_unlock:
            os.environ["CODEFLOW_FORCE_UNLOCK"] = "1"
        ...
        if runner:
            os.environ["CODEFLOW_RUNNER"] = runner
        ...
        if prev_force_unlock is None:
            ...
            os.environ["CODEFLOW_FORCE_UNLOCK"] = prev_force_unlock
        if runner:
            if prev_runner is None:
                os.environ.pop("CODEFLOW_RUNNER", None)
            else:
                os.environ["CODEFLOW_RUNNER"] = prev_runner
