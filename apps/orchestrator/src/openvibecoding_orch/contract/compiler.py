...
from codeflow_orch.contract.validator import ContractValidator, _resolve_ref_fragment, resolve_agent_registry_path
...
def _runtime_binding_sources(contract: dict[str, Any], runtime_binding: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "model": "env.CODEFLOW_CODEX_MODEL"
        if os.getenv("CODEFLOW_CODEX_MODEL", "").strip()
        else (
            "env.CODEFLOW_PROVIDER_MODEL"
            if os.getenv("CODEFLOW_PROVIDER_MODEL", "").strip()
