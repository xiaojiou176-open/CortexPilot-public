...
from codeflow_orch.runners import agents_events
...
async def run_worker_execution(
    *,
    ...
    class _CodeflowMCPServerStdio(mcp_server_stdio_cls):
        def __init__(
            self,
            params: Any,
            *,
    ...
    env["CODEFLOW_PROVIDER"] = provider
    codex_base_url = resolve_codex_base_url()
    if codex_base_url:
        env["CODEFLOW_PROVIDER_BASE_URL"] = codex_base_url
    ...
    if resolved_api_key:
        if normalized_provider == "gemini":
            ...
            env.setdefault("CODEFLOW_EQUILIBRIUM_API_KEY", resolved_api_key)
    server = _CodeflowMCPServerStdio(
