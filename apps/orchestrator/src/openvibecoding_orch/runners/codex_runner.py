...
from codeflow_orch.observability.codex_event_parser import parse_codex_event_line
...
def _resolve_model() -> str | None:
    model = os.getenv("CODEFLOW_CODEX_MODEL", "").strip()
    if model:
        return model
    return None
...
def _use_output_schema() -> bool:
    raw = os.getenv("CODEFLOW_CODEX_USE_OUTPUT_SCHEMA", "").strip().lower()
    return raw in {"1", "true", "yes", "on"}


def _exec_timeout_sec() -> int:
    raw = os.getenv("CODEFLOW_CODEX_EXEC_TIMEOUT_SEC", "300").strip()
    try:
        value = int(raw)
    except ValueError:
        value = 300
...
class CodexRunner:
    ...
    def run_contract(
        ...
        mcp_only = os.getenv("CODEFLOW_MCP_ONLY", "1").strip().lower() in {"1", "true", "yes"}
        allow_codex = os.getenv("CODEFLOW_ALLOW_CODEX_EXEC", "").strip().lower() in {"1", "true", "yes"}
            ...
            self._store.append_event(
                run_id,
                {
                    ...
                    "meta": {
                        ...
                        "env": "CODEFLOW_CODEX_USE_OUTPUT_SCHEMA",
                    },
                },
            )
        if model:
        ...
        codex_version = os.getenv("CODEFLOW_CODEX_VERSION", "").strip() or None
