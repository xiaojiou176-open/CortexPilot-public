...
from codeflow_orch.runners import agents_events, agents_handoff
from codeflow_orch.store.run_store import RunStore


def _handoff_timeout_fail_open_enabled() -> bool:
    raw = os.getenv("CODEFLOW_AGENTS_HANDOFF_TIMEOUT_FAIL_OPEN", "").strip().lower()
    ...
    if chain_roles and len(chain_roles) > 1:
        ...
        async def _run_chain() -> list[dict[str, Any]]:
            ...
            for idx in range(len(chain_roles) - 1):
                ...
                agent = agent_cls(
                    name=f"CodeflowHandoff_{from_role}",
