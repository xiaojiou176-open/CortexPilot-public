...
def _skip_role_prompt_requested() -> bool:
    raw = os.getenv("CODEFLOW_SKIP_ROLE_PROMPT", "").strip().lower()
...
def decorate_instruction(
    ...
    inline_output_schema = os.getenv("CODEFLOW_INLINE_OUTPUT_SCHEMA", "true").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def agent_instructions(
    ...
    timebox_raw = os.getenv("CODEFLOW_CODEX_TIMEBOX_SEC", "").strip()
    timebox_note = ""
    if timebox_raw:
        timebox_note = (
            "\nTimebox: "
