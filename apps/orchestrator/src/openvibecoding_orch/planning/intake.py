...
from codeflow_orch.contract.compiler import compile_plan, sync_role_contract
from codeflow_orch.contract.role_config_registry import build_runtime_capability_summary
from codeflow_orch.contract.validator import ContractValidator
from codeflow_orch.config import get_runner_config
from codeflow_orch.scheduler import approval_flow
...
_PROVIDER_API_KEY_ENV_HINTS = {
    ...
    "equilibrium": "CODEFLOW_EQUILIBRIUM_API_KEY",
    "codex_equilibrium": "CODEFLOW_EQUILIBRIUM_API_KEY",
}
...
def _agents_available() -> bool:
    smoke_flags = (
        os.getenv("CODEFLOW_ORCHESTRATION_SMOKE_MODE", "").strip().lower(),
        os.getenv("CODEFLOW_E2E_ORCHESTRATION_SMOKE_MODE", "").strip().lower(),
    )
...
def _resolve_agents_store() -> bool:
    raw = os.getenv("CODEFLOW_AGENTS_STORE", "").strip().lower()
...
def _run_agent(prompt: str, instructions: str) -> dict[str, Any]:
    ...
    if compat_client is not None:
        try:
            ...
            pass
    agent = Agent(name="CodeflowPlanner", instructions=instructions, mcp_servers=[])
        ...
        result = Runner.run_streamed(
            ...
            run_config=RunConfig(
                ...
                model_settings=ModelSettings(
                    extra_headers={"x-codeflow-intake": "plan_bundle"},
    ...
    def answer(self, intake_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if task_chain:
            ...
            if auto_run_chain:
                ...
                try:
                    if not os.getenv("CODEFLOW_RUNNER"):
