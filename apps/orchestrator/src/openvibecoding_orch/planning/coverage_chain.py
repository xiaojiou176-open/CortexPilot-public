...
def load_coverage_targets(
    ...
    threshold: float,
    max_workers: int,
    include_prefix: str = "apps/orchestrator/src/codeflow_orch/",
...
def _preferred_worker_python() -> str:
    override = os.getenv("CODEFLOW_PYTHON", "").strip()
    if override:
        return override
...
def _worker_timeout_sec() -> int:
    raw = os.getenv("CODEFLOW_COVERAGE_WORKER_TIMEOUT_SEC", "300").strip()
    try:
        value = int(raw)
    except ValueError:
        value = 300
...
def _default_shell_permission() -> str:
    runner_name = os.getenv("CODEFLOW_RUNNER", "").strip().lower()
...
def _worker_contract(
    ...
    payload = {
        ...
        "inputs": {
            "spec": (
                "You are the Codeflow coverage self-heal worker. "
...
def build_coverage_self_heal_chain(
    ...
    resolved_chain_id = chain_id or f"task_chain_codeflow_self_heal_coverage_{_now_tag()}"
