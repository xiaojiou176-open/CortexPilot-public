...
def _ci_or_container_env() -> bool:
    return (
        ...
        or os.getenv("CODEFLOW_CI_CONTAINER", "").strip().lower() in {"1", "true", "yes", "on"}
    )
...
def _env_default_policy() -> dict[str, Any]:
    ...
    profile_mode = _normalize_profile_mode(
        _non_empty_text(
            os.getenv("CODEFLOW_BROWSER_PROFILE_MODE", ""),
            default=default_profile_mode,
        ),
        default_profile_mode,
    )
    profile_dir = _non_empty_text(
        os.getenv("CODEFLOW_BROWSER_PROFILE_DIR", ""),
        default="",
    )
    profile_name = _non_empty_text(
        os.getenv("CODEFLOW_BROWSER_PROFILE_NAME", ""),
        default=_default_profile_name(profile_mode),
    )
    cookie_path = _non_empty_text(
        os.getenv("CODEFLOW_BROWSER_COOKIE_PATH", ""),
        default="",
    )
    stealth_mode = _normalize_stealth_mode(
        _non_empty_text(
            os.getenv("CODEFLOW_BROWSER_STEALTH_MODE", ""),
            default="none",
        ),
        "none",
    )
    behavior_enabled = _raw_to_bool(os.getenv("CODEFLOW_BROWSER_HUMAN_BEHAVIOR", ""), default=False)
    behavior_level = _normalize_behavior_level(
        _non_empty_text(os.getenv("CODEFLOW_BROWSER_HUMAN_BEHAVIOR_LEVEL", ""), default="low"),
        "low",
    )
    plugin_optional = _raw_to_bool(os.getenv("CODEFLOW_BROWSER_PLUGIN_OPTIONAL", ""), default=True)
    return {
...
def _local_profile_defaults_enabled() -> bool:
    if _ci_or_container_env():
        return False
    if _non_empty_text(
        os.getenv("CODEFLOW_CLEAN_ROOM_MACHINE_TMP_ROOT", ""),
        os.getenv("CODEFLOW_CLEAN_ROOM_PRESERVE_ROOT", ""),
        default="",
    ):
        return False
    return True
...
def _explicit_profile_env_overrides() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    profile_mode = _non_empty_text(
        os.getenv("CODEFLOW_BROWSER_PROFILE_MODE", ""),
        os.getenv("CODEFLOW_WEB_PROFILE_MODE", ""),
        default="",
    )
    if profile_mode:
    ...
    profile_dir = _non_empty_text(
        os.getenv("CODEFLOW_BROWSER_PROFILE_DIR", ""),
        os.getenv("CODEFLOW_WEB_PROFILE_DIR", ""),
        default="",
    )
    if profile_dir:
    ...
    profile_name = _non_empty_text(
        os.getenv("CODEFLOW_BROWSER_PROFILE_NAME", ""),
        os.getenv("CODEFLOW_WEB_PROFILE_NAME", ""),
        default="",
    )
    if profile_name:
...
def _profile_allowlist_roots() -> list[Path]:
    configured = _non_empty_text(os.getenv("CODEFLOW_BROWSER_PROFILE_ALLOWLIST", ""), default="")
    if configured:
...
def resolve_browser_policy(
    *,
    ...
    break_glass = _raw_to_bool(os.getenv("CODEFLOW_BROWSER_BREAK_GLASS", ""), default=False)
