...
def _resolve_running_stale_sec() -> int:
    raw = os.getenv("CODEFLOW_RUNNING_LOCK_STALE_SEC", "").strip()
    if raw:
        try:
...
def _force_unlock_enabled() -> bool:
    return _truthy(os.getenv("CODEFLOW_FORCE_UNLOCK", ""))


def _owner_token() -> str:
    explicit = os.getenv("CODEFLOW_LOCK_OWNER_TOKEN", "").strip()
    if explicit:
        return explicit
    run_id = os.getenv("CODEFLOW_RUN_ID", "").strip()
    if run_id:
...
def _resolve_lock_ttl(auto_cleanup: bool) -> tuple[int, str]:
    ...
    if not auto_cleanup:
        return 0, "disabled"
    default_raw = os.getenv("CODEFLOW_LOCK_TTL_SEC_DEFAULT", "").strip()
    if default_raw != "":
        try:
...
def acquire_lock(allowed_paths: Iterable[str], reclaim_stale: bool = True) -> bool:
    ...
    run_id = os.getenv("CODEFLOW_RUN_ID", "unknown")
    paths = _sanitize_allowed_paths(allowed_paths)
    if not paths:
        return False
...
def release_lock(allowed_paths: Iterable[str]) -> None:
    ...
    explicit_owner = os.getenv("CODEFLOW_LOCK_OWNER_TOKEN", "").strip()
