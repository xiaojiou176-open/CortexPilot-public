...
_CONFIG_CACHE: CodeflowConfig | None = None
_ENV_OVERRIDE_ORDER = {
    "provider": ("CODEFLOW_PROVIDER",),
    "base_url": ("CODEFLOW_PROVIDER_BASE_URL",),
    "model": ("CODEFLOW_PROVIDER_MODEL",),
}
...
def _explicit_env_candidates(repo_root: Path) -> list[Path]:
    explicit_path = os.getenv("CODEFLOW_ENV_FILE", "").strip()
    if explicit_path:
        return [Path(explicit_path)]
    env_root = Path(os.getenv("CODEFLOW_DEFAULT_ENV_ROOT", "~/.config/codeflow")).expanduser()
    return [
        env_root / ".env.local",
        env_root / ".env",
    ]
...
def _assert_no_shadowed_env_values() -> None:
    env_values = {
        "CODEFLOW_PROVIDER": os.getenv("CODEFLOW_PROVIDER"),
        "CODEFLOW_PROVIDER_BASE_URL": os.getenv("CODEFLOW_PROVIDER_BASE_URL"),
        "CODEFLOW_PROVIDER_MODEL": os.getenv("CODEFLOW_PROVIDER_MODEL"),
    }
...
def _load_explicit_env_files() -> None:
    ...
    with _ENV_LOAD_LOCK:
        if _ENV_LOADED:
            return
        explicit_path = _normalize_env_value(os.getenv("CODEFLOW_ENV_FILE"))
        if explicit_path:
            ...
            if not explicit_candidate.exists() or not explicit_candidate.is_file():
                raise RuntimeError(f"env effective-chain breakpoint: CODEFLOW_ENV_FILE not found: {explicit_candidate}")
...
def _default_machine_cache_root() -> Path:
    explicit = os.getenv("CODEFLOW_MACHINE_CACHE_ROOT", "").strip()
    if explicit:
    ...
    if xdg_cache_home:
        return Path(xdg_cache_home) / "codeflow"
...
def load_config() -> CodeflowConfig:
    _load_explicit_env_files()

    repo_root = _resolve_repo_relative_path(
        Path(_env_value(os.getenv("CODEFLOW_REPO_ROOT"), str(_repo_root()))),
        _repo_root(),
    )
    runtime_root = _resolve_repo_relative_path(
        Path(
            _env_value(
                os.getenv("CODEFLOW_RUNTIME_ROOT"),
                ".runtime-cache/codeflow",
            )
        ),
        repo_root,
    )
    schema_root, _ = _resolve_runtime_path("CODEFLOW_SCHEMA_ROOT", "schemas", repo_root)
    contract_root, contract_root_explicit = _resolve_runtime_path(
        "CODEFLOW_CONTRACT_ROOT", "contracts", repo_root
    )
    runtime_contract_root = _resolve_repo_relative_path(
        Path(os.getenv("CODEFLOW_RUNTIME_CONTRACT_ROOT", ".runtime-cache/codeflow/contracts")),
        repo_root,
    )
    machine_cache_root = _default_machine_cache_root()
    toolchain_cache_root = Path(os.getenv("CODEFLOW_TOOLCHAIN_CACHE_ROOT", str(machine_cache_root / "toolchains")))
    worktree_root = _resolve_repo_relative_path(
        Path(os.getenv("CODEFLOW_WORKTREE_ROOT", ".runtime-cache/codeflow/worktrees")),
        repo_root,
    )
    runs_root = _resolve_repo_relative_path(
        Path(os.getenv("CODEFLOW_RUNS_ROOT", ".runtime-cache/codeflow/runs")),
        repo_root,
    )
    logs_root = _resolve_repo_relative_path(
        Path(
            _env_value(
                os.getenv("CODEFLOW_LOGS_ROOT"),
                ".runtime-cache/logs",
            )
        ),
        repo_root,
    ...
    security = SecurityConfig(
        api_auth_required=_env_flag("CODEFLOW_API_AUTH_REQUIRED", True),
        api_token=_env_value(os.getenv("CODEFLOW_API_TOKEN")),
    )

    retention = RetentionConfig(
        run_days=_env_int("CODEFLOW_RETENTION_RUN_DAYS", 7, min_value=1),
        max_runs=_env_int("CODEFLOW_RETENTION_MAX_RUNS", 200, min_value=1),
        log_days=_env_int("CODEFLOW_RETENTION_LOG_DAYS", 7, min_value=1),
        worktree_days=_env_int("CODEFLOW_RETENTION_WORKTREE_DAYS", 2, min_value=1),
        log_max_files=_env_int("CODEFLOW_RETENTION_LOG_MAX_FILES", 5, min_value=1),
        cache_hours=_env_int("CODEFLOW_RETENTION_CACHE_HOURS", 24, min_value=1),
        codex_home_days=_env_int("CODEFLOW_RETENTION_CODEX_HOME_DAYS", 3, min_value=1),
        max_codex_homes=_env_int("CODEFLOW_RETENTION_MAX_CODEX_HOMES", 500, min_value=1),
        intake_days=_env_int("CODEFLOW_RETENTION_INTAKE_DAYS", 7, min_value=1),
        max_intakes=_env_int("CODEFLOW_RETENTION_MAX_INTAKES", 500, min_value=1),
        machine_cache_cap_bytes=_env_int(
            "CODEFLOW_RETENTION_MACHINE_CACHE_CAP_BYTES",
    ...
    tracing = TracingConfig(
        enabled=_env_flag("CODEFLOW_TRACING_ENABLED", True),
        endpoint=os.getenv("CODEFLOW_OTLP_ENDPOINT", "").strip(),
        headers=os.getenv("CODEFLOW_OTLP_HEADERS", "").strip(),
        protocol=os.getenv("CODEFLOW_OTLP_PROTOCOL", "grpc").strip().lower() or "grpc",
        console_enabled=_env_flag("CODEFLOW_ENABLE_CONSOLE_TRACE", False),
        required=_env_flag("CODEFLOW_OTEL_REQUIRED", False),
    )

    logging = LoggingConfig(
        max_bytes=_env_int("CODEFLOW_LOG_MAX_BYTES", 100 * 1024 * 1024, min_value=1024),
        backup_count=_env_int("CODEFLOW_LOG_BACKUP_COUNT", 5, min_value=1),
        trace_id=os.getenv("CODEFLOW_TRACE_ID", "").strip(),
        schema_version=os.getenv("CODEFLOW_LOG_SCHEMA_VERSION", "log_event.v2").strip() or "log_event.v2",
    )

    api_runtime = ApiRuntimeConfig(
        dashboard_port=(
            _env_value(
                os.getenv("CODEFLOW_DASHBOARD_PORT"),
                "3100",
            )
            or "3100"
        ),
        canary_percent=_env_percent("CODEFLOW_CANARY_PERCENT", 0.0),
        allowed_origins=_env_csv("CODEFLOW_API_ALLOWED_ORIGINS"),
    )
    ...
    runner = RunnerConfig(
        ...
        agents_api=os.getenv("CODEFLOW_AGENTS_API", "").strip(),
        agents_model=runtime_model,
        codex_model=os.getenv("CODEFLOW_CODEX_MODEL", "").strip() or runtime_model,
        agents_store=_env_flag("CODEFLOW_AGENTS_STORE", False),
        ...
        mcp_timeout_seconds=_env_float("CODEFLOW_MCP_SERVER_TIMEOUT_SEC", 30.0, min_value=0.1),
        mcp_connect_timeout_seconds=_env_float("CODEFLOW_MCP_SERVER_CONNECT_TIMEOUT_SEC", 10.0, min_value=0.1),
    )
...
def get_cached_config(force_reload: bool = False) -> CodeflowConfig:
