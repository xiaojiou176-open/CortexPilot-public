...
from codeflow_orch.gates.path_match import is_allowed_path, normalize_path
...
def _default_protected_paths() -> list[str]:
    raw = os.getenv("CODEFLOW_PROTECTED_PATHS", "")
    if raw.strip():
        return [item.strip() for item in raw.split(",") if item.strip()]
    return [".git/", ".gitignore", "pytest.ini", ".runtime-cache/codeflow/", ".env*"]
