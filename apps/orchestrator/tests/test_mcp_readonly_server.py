...
from agentcoder_orch.mcp_readonly_server import AgentcoderReadonlyMcpServer
...
def test_mcp_readonly_server_lists_tools_and_calls_with_structured_content() -> None:
    ...
    server = AgentcoderReadonlyMcpServer(read_service=_FakeReadService())  # type: ignore[arg-type]
    ...
    assert init_response["result"]["serverInfo"]["name"] == "agentcoder-readonly"
...
def test_mcp_readonly_server_cli_supports_runs_reports_and_keeps_read_only(tmp_path: Path) -> None:
    ...
    env["AGENTCODER_RUNTIME_ROOT"] = str(runtime_root)
    env["AGENTCODER_RUNS_ROOT"] = str(runs_root)
