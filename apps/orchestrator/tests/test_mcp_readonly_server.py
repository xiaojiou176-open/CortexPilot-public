...
from codeflow_orch.mcp_readonly_server import CodeflowReadonlyMcpServer
from codeflow_orch.transport.mcp_jsonl import JsonlStream, send_json
...
def test_mcp_readonly_server_lists_tools_and_calls_with_structured_content() -> None:
    ...
    server = CodeflowReadonlyMcpServer(read_service=_FakeReadService())  # type: ignore[arg-type]
    ...
    assert init_response["result"]["serverInfo"]["name"] == "codeflow-readonly"
...
def test_mcp_readonly_server_cli_supports_runs_reports_and_keeps_read_only(tmp_path: Path) -> None:
    ...
    env["CODEFLOW_RUNTIME_ROOT"] = str(runtime_root)
    env["CODEFLOW_RUNS_ROOT"] = str(runs_root)
