...
def test_cli_queue_and_session_alias(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_SCHEMA_ROOT", str(schema_root))
    monkeypatch.setenv("CODEFLOW_CONTRACT_ROOT", str(contracts_root))
    monkeypatch.setenv("CODEFLOW_SESSION_ALIAS_PATH", str(alias_path))
...
def test_cleanup_runtime_cli(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_RETENTION_RUN_DAYS", "0")
    monkeypatch.setenv("CODEFLOW_RETENTION_MAX_RUNS", "1")
    monkeypatch.setenv("CODEFLOW_RETENTION_LOG_DAYS", "0")
    monkeypatch.setenv("CODEFLOW_RETENTION_WORKTREE_DAYS", "0")
