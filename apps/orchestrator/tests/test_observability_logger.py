...
from agentcoder_orch.observability import logger as logger_module
...
def test_json_line_formatter_includes_request_id_from_meta() -> None:
    ...
    assert payload["service"] == "agentcoder-orchestrator"
...
def test_resolve_log_level_defaults_to_debug(monkeypatch) -> None:
    monkeypatch.delenv("AGENTCODER_LOG_LEVEL", raising=False)
...
def test_level_and_access_filters_cover_accept_and_reject_paths() -> None:
    ...
    low_record = logging.LogRecord("agentcoder", logging.INFO, __file__, 1, "msg", (), None)
    high_record = logging.LogRecord("agentcoder", logging.ERROR, __file__, 1, "msg", (), None)
    ...
    api_record = logging.LogRecord("agentcoder", logging.INFO, __file__, 1, "msg", (), None)
    api_record.component = "api"
    assert access_filter.filter(api_record) is True

    http_event = logging.LogRecord("agentcoder", logging.INFO, __file__, 1, "msg", (), None)
    http_event.event = "HTTP_REQUEST"
    assert access_filter.filter(http_event) is True

    other = logging.LogRecord("agentcoder", logging.INFO, __file__, 1, "msg", (), None)
...
def test_gzip_rotating_file_handler_compresses_rotated_files(monkeypatch, tmp_path: Path) -> None:
    ...
    log_path = tmp_path / "agentcoder.log"
    rotated = tmp_path / "agentcoder.log.1"
    ...
    assert (tmp_path / "agentcoder.log.1.gz").exists() is True
...
def test_json_line_formatter_rejects_invalid_surface() -> None:
    formatter = logger_module.JsonLineFormatter()
    record = logging.LogRecord("agentcoder", logging.INFO, __file__, 1, "evt", (), None)
    record.component = "api"
    record.event = "TEST_EVENT"
...
def test_resolve_logs_root_and_targets(monkeypatch, tmp_path: Path) -> None:
    ...
    assert targets["runtime"].name == "agentcoder-runtime.jsonl"
    assert targets["error"].name == "agentcoder-error.jsonl"
    assert targets["access"].name == "agentcoder-access.jsonl"
    assert targets["e2e"].name == "agentcoder-e2e.jsonl"


def test_gzip_rotating_file_handler_skips_already_compressed(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(logger_module.RotatingFileHandler, "doRollover", lambda self: None)
    log_path = tmp_path / "agentcoder.log"
    compressed = tmp_path / "agentcoder.log.1.gz"
