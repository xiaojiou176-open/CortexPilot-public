...
from codeflow_orch.transport import codex_profile_pool as profile_pool
from codeflow_orch.transport import mcp_jsonl
...
def test_profile_pool_parse_and_state_roundtrip(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path / "runtime"))
    monkeypatch.setenv("CODEFLOW_CODEX_PROFILE_POOL", "A,a,B")
...
def test_profile_pool_pick_profile_rotation(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path / "runtime"))
    monkeypatch.setenv("CODEFLOW_CODEX_PROFILE_POOL", "alpha,beta")
    ...
    monkeypatch.delenv("CODEFLOW_CODEX_PROFILE_POOL", raising=False)
...
def test_profile_pool_lock_self_heal_on_stale_ttl(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path / "runtime"))
    monkeypatch.setenv("CODEFLOW_CODEX_PROFILE_POOL_LOCK_TTL_SEC", "1")
    lock_path = profile_pool._pool_lock_path()
    lock_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_profile_pool_lock_does_not_preempt_active_lock(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path / "runtime"))
    monkeypatch.setenv("CODEFLOW_CODEX_PROFILE_POOL_LOCK_TTL_SEC", "120")
    lock_path = profile_pool._pool_lock_path()
    lock_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_profile_pool_error_and_parse_branches(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path / "runtime"))
    monkeypatch.setenv("CODEFLOW_CODEX_PROFILE_POOL", "p1,p2")
