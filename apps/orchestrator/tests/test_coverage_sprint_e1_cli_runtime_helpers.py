...
from agentcoder_orch import cli, cli_command_helpers
...
def test_cli_force_unlock_fail_closed_and_env_restore(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_RUNNER", "previous-runner")
    monkeypatch.setenv("AGENTCODER_FORCE_UNLOCK", "previous-unlock")
    ...
    assert os.getenv("AGENTCODER_RUNNER") == "previous-runner"
    assert os.getenv("AGENTCODER_FORCE_UNLOCK") == "previous-unlock"
...
def test_cli_run_chain_restores_existing_force_unlock_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli, "Orchestrator", _FakeOrchestrator)
    monkeypatch.setenv("AGENTCODER_FORCE_UNLOCK", "keep-me")
    result = runner.invoke(app, ["run-chain", str(chain_path), "--mock"])
    assert result.exit_code == 0
    assert os.getenv("AGENTCODER_FORCE_UNLOCK") == "keep-me"
