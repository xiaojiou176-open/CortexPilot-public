...
def _write_active_contract(store: RunStore, run_id: str, mcp_tools: list[str], monkeypatch) -> None:
    monkeypatch.setenv("AGENTCODER_RUNTIME_ROOT", str(store._runs_root.parent))
    contract = {
        "task_id": "tool_task",
...
def test_tool_runner_browser_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.BrowserRunner", DummyBrowser)
...
def test_tool_runner_search_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.search_verify", _raise)

    runner = ToolRunner(run_id, store)
    result = runner.run_search("agentcoder", provider="duckduckgo")
    assert result["ok"] is False
...
def test_tool_runner_search_result_ok_false_logs_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.search_verify", _fake)

    runner = ToolRunner(run_id, store)
    result = runner.run_search("agentcoder", provider="chatgpt_web")
    assert result["ok"] is False
...
def test_tool_runner_browser_ddg_fail_closed_logs_explicit_browser_error(tmp_path: Path, monkeypatch) -> None:
    ...
    runner = ToolRunner(run_id, store)
    result = runner.run_search("agentcoder", provider="browser")
    assert result["ok"] is False
...
def test_tool_runner_mcp_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.mcp_adapter.record_mcp_call", _raise)
...
def test_tool_runner_sampling_requires_approval(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.delenv("AGENTCODER_SAMPLING_APPROVED", raising=False)
...
def test_tool_runner_sampling_approved(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_SAMPLING_APPROVED", "true")
...
def test_tool_runner_browser_init_typeerror_policy_compat_fallback(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.BrowserRunner", CompatBrowser)
...
def test_tool_runner_search_typeerror_policy_compat_fallback(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.search_verify", _search)

    runner = ToolRunner(run_id, store)
    result = runner.run_search("agentcoder", provider="codex_web", browser_policy={"profile_mode": "ephemeral"})
    assert result["ok"] is True
...
def test_tool_runner_module_import_keeps_sys_path_when_root_already_present(monkeypatch) -> None:
    module_name = "agentcoder_orch.runners.tool_runner"
