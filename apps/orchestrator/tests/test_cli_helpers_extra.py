...
def test_cli_wait_run_and_manifest_and_hooks(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_HOOKS_AUTO_INSTALL", "yes")
    assert cli._hooks_auto_install_enabled() is True
    monkeypatch.setenv("AGENTCODER_HOOKS_AUTO_INSTALL", "0")
...
def test_cli_temporal_worker_and_serve(monkeypatch) -> None:
    ...
    monkeypatch.setitem(__import__("sys").modules, "agentcoder_orch.temporal.worker", _FakeTemporalWorkerModule)
