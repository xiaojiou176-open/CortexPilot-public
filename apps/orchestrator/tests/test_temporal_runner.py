...
from codeflow_orch.temporal.runner import run_workflow


def test_temporal_runner_disabled(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("CODEFLOW_TEMPORAL_WORKFLOW", raising=False)
...
def test_temporal_runner_missing_dependency(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_TEMPORAL_WORKFLOW", "1")
...
def test_temporal_runner_helpers_and_required(monkeypatch) -> None:
    from codeflow_orch.temporal import runner as temporal_runner
    ...
    monkeypatch.setenv("CODEFLOW_TEMPORAL_ADDRESS", "host:7233")
    monkeypatch.setenv("CODEFLOW_TEMPORAL_NAMESPACE", "ns")
    monkeypatch.setenv("CODEFLOW_TEMPORAL_TASK_QUEUE", "q")
    assert temporal_runner._config() == ("host:7233", "ns", "q")

    workflow_id = temporal_runner._workflow_id("task")
    assert workflow_id.startswith("codeflow-task-")

    monkeypatch.setenv("CODEFLOW_TEMPORAL_REQUIRED", "1")
    with pytest.raises(RuntimeError, match="boom"):
        temporal_runner.temporal_required_or_raise("boom")

    monkeypatch.setenv("CODEFLOW_TEMPORAL_REQUIRED", "0")
...
def test_temporal_runner_success_and_missing_run_id(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_TEMPORAL_NAMESPACE", "default")
    monkeypatch.setenv("CODEFLOW_TEMPORAL_TASK_QUEUE", "codeflow-orch")
    ...
    workflow_mod = types.ModuleType("codeflow_orch.temporal.workflows")
    workflow_mod.RunRequest = _FakeRunRequest
    workflow_mod.CodeflowRunWorkflow = _FakeWorkflow
    monkeypatch.setitem(sys.modules, "codeflow_orch.temporal.workflows", workflow_mod)
