...
def test_temporal_runner_missing_dependency(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("AGENTCODER_TEMPORAL_WORKFLOW", "1")
...
def test_temporal_runner_helpers_and_required(monkeypatch) -> None:
    from agentcoder_orch.temporal import runner as temporal_runner
    ...
    monkeypatch.setenv("AGENTCODER_TEMPORAL_ADDRESS", "host:7233")
    monkeypatch.setenv("AGENTCODER_TEMPORAL_NAMESPACE", "ns")
    monkeypatch.setenv("AGENTCODER_TEMPORAL_TASK_QUEUE", "q")
    assert temporal_runner._config() == ("host:7233", "ns", "q")

    workflow_id = temporal_runner._workflow_id("task")
    assert workflow_id.startswith("agentcoder-task-")

    monkeypatch.setenv("AGENTCODER_TEMPORAL_REQUIRED", "1")
    with pytest.raises(RuntimeError, match="boom"):
        temporal_runner.temporal_required_or_raise("boom")

    monkeypatch.setenv("AGENTCODER_TEMPORAL_REQUIRED", "0")
...
def test_temporal_runner_success_and_missing_run_id(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_TEMPORAL_NAMESPACE", "default")
    monkeypatch.setenv("AGENTCODER_TEMPORAL_TASK_QUEUE", "agentcoder-orch")
    ...
    workflow_mod = types.ModuleType("agentcoder_orch.temporal.workflows")
    workflow_mod.RunRequest = _FakeRunRequest
    workflow_mod.AgentcoderRunWorkflow = _FakeWorkflow
    monkeypatch.setitem(sys.modules, "agentcoder_orch.temporal.workflows", workflow_mod)
