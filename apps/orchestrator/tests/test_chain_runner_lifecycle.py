...
def _disable_strict_nontrivial(monkeypatch) -> None:
    monkeypatch.setenv("AGENTCODER_ACCEPTANCE_STRICT_NONTRIVIAL", "0")
...
def test_chain_runner_lifecycle_mock_mode_ignores_subprocess_exec_mode(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("AGENTCODER_CHAIN_EXEC_MODE", "subprocess")
    monkeypatch.setenv("AGENTCODER_CHAIN_SUBPROCESS_TIMEOUT_SEC", "1")
