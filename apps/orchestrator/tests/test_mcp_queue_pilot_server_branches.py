...
from codeflow_orch import mcp_queue_pilot_server as queue_pilot_module


def test_mcp_queue_pilot_helpers_and_protocol_edges(monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_APPROVAL_ALLOWED_ROLES", " owner , ops ")
...
def test_mcp_queue_pilot_server_covers_default_constructor_unknown_methods_and_stream(monkeypatch) -> None:
    ...
    server = queue_pilot_module.CodeflowQueuePilotMcpServer()
    ...
    assert init_response["result"]["serverInfo"]["name"] == "codeflow-queue-pilot"
    ...
    monkeypatch.setattr(queue_pilot_module, "CodeflowQueuePilotMcpServer", _FakeServer)
