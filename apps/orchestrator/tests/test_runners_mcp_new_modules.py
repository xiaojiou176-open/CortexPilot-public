...
from codeflow_orch.runners import mcp_logging, mcp_server_lifecycle, mcp_streaming
...
def test_mcp_server_lifecycle_env_resolution(monkeypatch) -> None:
    monkeypatch.delenv("CODEFLOW_MCP_TIMEOUT_SEC", raising=False)
    monkeypatch.delenv("CODEFLOW_MCP_CONNECT_TIMEOUT_SEC", raising=False)
    monkeypatch.delenv("CODEFLOW_MCP_CLEANUP_TIMEOUT_SEC", raising=False)
    monkeypatch.delenv("CODEFLOW_MCP_TOOL_TIMEOUT_SEC", raising=False)
    ...
    monkeypatch.setenv("CODEFLOW_MCP_TIMEOUT_SEC", "bad")
    monkeypatch.setenv("CODEFLOW_MCP_CONNECT_TIMEOUT_SEC", "bad")
    monkeypatch.setenv("CODEFLOW_MCP_CLEANUP_TIMEOUT_SEC", "bad")
    monkeypatch.setenv("CODEFLOW_MCP_TOOL_TIMEOUT_SEC", "bad")
    assert mcp_server_lifecycle.resolve_mcp_timeout_seconds() == 600.0
    assert mcp_server_lifecycle.resolve_mcp_connect_timeout_sec() == 20.0
    assert mcp_server_lifecycle.resolve_mcp_cleanup_timeout_sec() == 5.0
    assert mcp_server_lifecycle.resolve_mcp_tool_timeout_sec() is None

    monkeypatch.setenv("CODEFLOW_MCP_TIMEOUT_SEC", "0")
    monkeypatch.setenv("CODEFLOW_MCP_CONNECT_TIMEOUT_SEC", "0")
    monkeypatch.setenv("CODEFLOW_MCP_CLEANUP_TIMEOUT_SEC", "0")
    monkeypatch.setenv("CODEFLOW_MCP_TOOL_TIMEOUT_SEC", "3")
    ...
    monkeypatch.delenv("CODEFLOW_MCP_CONNECT_TIMEOUT_SEC", raising=False)
    monkeypatch.delenv("CODEFLOW_MCP_CLEANUP_TIMEOUT_SEC", raising=False)
    monkeypatch.setenv("CODEFLOW_MCP_SERVER_TIMEOUT_SEC", "777")
    monkeypatch.setenv("CODEFLOW_MCP_SERVER_CONNECT_TIMEOUT_SEC", "44")
    monkeypatch.setenv("CODEFLOW_MCP_SERVER_CLEANUP_TIMEOUT_SEC", "9")
    assert mcp_server_lifecycle.resolve_mcp_timeout_seconds() == 600.0
    assert mcp_server_lifecycle.resolve_mcp_connect_timeout_sec() == 20.0
    assert mcp_server_lifecycle.resolve_mcp_cleanup_timeout_sec() == 5.0
...
def test_mcp_streaming_summaries_and_log_every(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_STREAM_LOG_EVERY", "")
    assert mcp_streaming.resolve_stream_log_every() == 0
    monkeypatch.setenv("CODEFLOW_STREAM_LOG_EVERY", "bad")
    assert mcp_streaming.resolve_stream_log_every() == 0
    monkeypatch.setenv("CODEFLOW_STREAM_LOG_EVERY", "4")
