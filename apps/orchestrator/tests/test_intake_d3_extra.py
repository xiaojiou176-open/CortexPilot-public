...
from codeflow_orch.runners.provider_resolution import ProviderCredentials
...
def test_intake_execute_chain_uses_repo_root_and_orchestrator(monkeypatch, tmp_path: Path) -> None:
    ...
    scheduler_mod = types.ModuleType("codeflow_orch.scheduler.scheduler")
    ...
    monkeypatch.setitem(sys.modules, "codeflow_orch.scheduler.scheduler", scheduler_mod)
...
def test_intake_service_answer_payload_and_contract_edge_branches(monkeypatch, tmp_path: Path) -> None:
    ...
    assert os.getenv("CODEFLOW_RUNNER") == "codex"
