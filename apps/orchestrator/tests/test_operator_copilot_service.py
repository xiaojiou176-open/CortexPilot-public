...
from codeflow_orch.services.operator_copilot import (
...
def test_generate_operator_copilot_brief_returns_structured_report(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codeflow_orch.services.operator_copilot.get_runner_config",
    ...
    monkeypatch.setattr("codeflow_orch.services.operator_copilot.resolve_runtime_provider_from_env", lambda: "gemini")
    monkeypatch.setattr(
        "codeflow_orch.services.operator_copilot.resolve_provider_credentials",
        lambda: types.SimpleNamespace(gemini_api_key="gemini-key", openai_api_key="", anthropic_api_key="", equilibrium_api_key=""),
    )
    monkeypatch.setattr("codeflow_orch.services.operator_copilot.build_llm_compat_client", lambda **_kwargs: object())
...
def test_generate_operator_copilot_brief_fails_closed_without_agents(monkeypatch) -> None:
    monkeypatch.setattr("codeflow_orch.services.operator_copilot._agents_available", lambda: False)
...
def test_operator_copilot_switchyard_runtime_forces_chat_mode_and_placeholder_key(monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.services.operator_copilot.resolve_runtime_provider_from_env", lambda: "openai")
    monkeypatch.setattr(
        "codeflow_orch.services.operator_copilot.resolve_provider_credentials",
        lambda: types.SimpleNamespace(gemini_api_key="", openai_api_key="", anthropic_api_key="", equilibrium_api_key=""),
    )
    monkeypatch.setattr("codeflow_orch.services.operator_copilot.build_llm_compat_client", _build_client)
