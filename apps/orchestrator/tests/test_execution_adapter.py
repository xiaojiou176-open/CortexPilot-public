...
from agentcoder_orch.runners import execution_adapter as adapter
...
def test_claude_adapter_injects_anthropic_via_contract_without_mutating_process_env(
    ...
    class FakeAgentsRunner:
        ...
        def run_contract(self, contract, worktree_path, schema_path, mock_mode=False):
            captured["provider"] = os.environ.get("AGENTCODER_PROVIDER")
            captured["provider_model"] = os.environ.get("AGENTCODER_PROVIDER_MODEL")
            captured["provider_base_url"] = os.environ.get("AGENTCODER_PROVIDER_BASE_URL")
    ...
    monkeypatch.setenv("AGENTCODER_PROVIDER_MODEL", "gemini-3.1-pro-preview")
    ...
    assert os.environ.get("AGENTCODER_PROVIDER") == "openai"
    assert os.environ.get("AGENTCODER_PROVIDER_MODEL") == "gemini-3.1-pro-preview"
    assert "AGENTCODER_PROVIDER_BASE_URL" not in os.environ
...
def test_claude_adapter_restores_process_env_even_when_runner_raises(
    ...
    monkeypatch.setenv("AGENTCODER_PROVIDER", "openai")
    monkeypatch.setenv("AGENTCODER_PROVIDER_MODEL", "gemini-3.1-pro-preview")
    monkeypatch.setenv("AGENTCODER_PROVIDER_BASE_URL", "http://localhost:1456/v1")
    ...
    assert os.environ.get("AGENTCODER_PROVIDER") == "openai"
    assert os.environ.get("AGENTCODER_PROVIDER_MODEL") == "gemini-3.1-pro-preview"
    assert os.environ.get("AGENTCODER_PROVIDER_BASE_URL") == "http://localhost:1456/v1"
