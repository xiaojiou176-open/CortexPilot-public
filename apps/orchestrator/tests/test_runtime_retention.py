...
from agentcoder_orch.runtime.retention import apply_retention_plan, build_retention_plan, write_retention_report
...
def test_retention_dry_plan_and_apply(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_RETENTION_RUN_DAYS", "1")
    monkeypatch.setenv("AGENTCODER_RETENTION_MAX_RUNS", "1")
    monkeypatch.setenv("AGENTCODER_RETENTION_LOG_DAYS", "1")
    monkeypatch.setenv("AGENTCODER_RETENTION_WORKTREE_DAYS", "1")
    monkeypatch.setenv("AGENTCODER_RETENTION_LOG_MAX_FILES", "1")
    monkeypatch.setenv("AGENTCODER_RETENTION_CACHE_HOURS", "1")
...
def test_machine_cache_summary_excludes_repo_browser_root_from_cap_pressure(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_REPO_ROOT", str(repo_root))
    monkeypatch.setenv("AGENTCODER_RUNTIME_ROOT", str(runtime_root))
    monkeypatch.setenv("AGENTCODER_MACHINE_CACHE_ROOT", str(machine_root))
    monkeypatch.setenv("AGENTCODER_RETENTION_MACHINE_CACHE_CAP_BYTES", "50")
        ...
        json.dumps(
            {
                ...
                "apply_gate_max_age_minutes": 15,
                    "machine_cache_retention_policy": {
                        ...
                        "protected_prefixes": [
                            "${AGENTCODER_MACHINE_CACHE_ROOT}/browser",
                        ],
