...
def test_scheduler_mcp_only_block_branch(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_ALLOW_CODEX_EXEC", "0")
...
def test_scheduler_temporal_notify_required_failure_and_diagnostic_root(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_RUNS_ROOT", str(runs_root))
    monkeypatch.setenv("AGENTCODER_WORKTREE_ROOT", str(worktree_root))
    monkeypatch.setenv("AGENTCODER_DIAGNOSTIC_RUNS_ROOT", str(diagnostic_root))
...
def test_scheduler_lock_auto_cleanup_released_event(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_LOCK_AUTO_CLEANUP", "1")
    monkeypatch.setenv("AGENTCODER_LOCK_TTL_SEC", "30")
...
def test_scheduler_search_pipeline_failed_branch(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ({"queries": ["agentcoder"], "verify": False}, None),
    )
