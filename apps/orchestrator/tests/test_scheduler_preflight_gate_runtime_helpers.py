...
from agentcoder_orch.scheduler import preflight_gate_runtime_helpers as helpers
...
def test_preflight_success_with_sampling_and_force_unlock(monkeypatch, tmp_path: Path) -> None:
    ...
    assert os.getenv("AGENTCODER_NETWORK_APPROVED") == "1"
...
def test_preflight_mcp_concurrency_required_failure(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("AGENTCODER_MCP_CONCURRENCY_REQUIRED", "true")
    contract = _base_contract()
    ops = _build_ops(
