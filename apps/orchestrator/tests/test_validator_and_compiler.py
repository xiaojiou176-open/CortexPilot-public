...
def test_contract_validator_import_does_not_eagerly_import_compiler() -> None:
    saved_modules = {
        ...
        if name == "agentcoder_orch.contract" or name.startswith("agentcoder_orch.contract.")
    }
    for name in list(saved_modules):
        sys.modules.pop(name, None)

    try:
        validator_module = importlib.import_module("agentcoder_orch.contract.validator")
        assert validator_module.ContractValidator is not None
        assert "agentcoder_orch.contract.compiler" not in sys.modules
    finally:
...
def test_compile_plan_missing_task_id(monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.contract.compiler.ContractValidator.validate_report", lambda *args, **kwargs: None)
...
def test_compile_plan_inherits_forbidden_actions(monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.contract.compiler._load_forbidden_actions", lambda: ["rm -rf"])
    contract = compile_plan(plan)
...
def test_compile_plan_rejects_missing_mcp_bundle_fragment(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_AGENT_REGISTRY", str(registry_path))
