...
from codeflow_orch.scheduler import approval_flow, evidence_pipeline, task_build_pipeline, test_pipeline
...
def test_approval_requires_human_approval_modes(monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_GOD_MODE_REQUIRED", "1")
    ...
    monkeypatch.setenv("CODEFLOW_GOD_MODE_REQUIRED", "0")
    monkeypatch.setenv("CODEFLOW_GOD_MODE_ON_REQUEST", "0")
    assert approval_flow.requires_human_approval(
        requires_network=False,
    ...
    monkeypatch.setenv("CODEFLOW_GOD_MODE_ON_REQUEST", "1")
    assert approval_flow.requires_human_approval(
...
def test_approval_helpers_flags(monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_FORCE_UNLOCK", "true")
    assert approval_flow.force_unlock_requested()

    monkeypatch.setenv("CODEFLOW_LOCK_AUTO_CLEANUP", "")
    monkeypatch.setenv("CODEFLOW_LOCK_TTL_SEC", "120")
    assert approval_flow.auto_lock_cleanup_requested()

    monkeypatch.setenv("CODEFLOW_GOD_MODE_TIMEOUT_SEC", "15")
    assert approval_flow.god_mode_timeout_sec() == 15
    monkeypatch.setenv("CODEFLOW_GOD_MODE_TIMEOUT_SEC", "bad")
...
def test_await_human_approval_success(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_GOD_MODE_TIMEOUT_SEC", "3")
...
def test_await_human_approval_timeout(tmp_path: Path, monkeypatch) -> None:
    ...
    store = _ApprovalStore(events)
    monkeypatch.setenv("CODEFLOW_GOD_MODE_TIMEOUT_SEC", "1")
...
def test_approval_extra_branches_and_invalid_json(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_LOCK_AUTO_CLEANUP", "true")
    assert approval_flow.auto_lock_cleanup_requested()
    ...
    monkeypatch.setenv("CODEFLOW_GOD_MODE_TIMEOUT_SEC", "2")
