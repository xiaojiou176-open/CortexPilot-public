...
def test_pm_chat_e2e_real_mode_requires_ui_intake_by_default() -> None:
    text = _runner_text()
    assert 'CODEFLOW_E2E_REQUIRE_UI_INTAKE", "1" if run_mode == "real" else "0"' in text
...
def test_pm_chat_e2e_config_api_key_import_requires_explicit_opt_in() -> None:
    text = _script_text()
    assert (
        'IMPORT_CONFIG_API_KEY="${CODEFLOW_E2E_IMPORT_CONFIG_API_KEY:-0}"' in text
        or 'IMPORT_CONFIG_API_KEY="$(codeflow_env_get CODEFLOW_E2E_IMPORT_CONFIG_API_KEY "0")"' in text
    )
...
def test_pm_chat_e2e_default_acceptance_command_avoids_bash_prefix_for_tool_gate() -> None:
    text = _script_text()
    assert 'ACCEPTANCE_CMD="$(codeflow_env_get CODEFLOW_E2E_ACCEPTANCE_CMD "")"' in text
...
def test_pm_chat_e2e_strict_acceptance_defaults_to_real_mode_and_supports_override() -> None:
    text = _runner_text()
    assert 'strict_acceptance_env = str(os.getenv("CODEFLOW_E2E_STRICT_ACCEPTANCE", "")).strip().lower()' in text
...
def test_pm_chat_e2e_objective_can_be_overridden_from_env() -> None:
    text = _runner_text()
    assert "CODEFLOW_E2E_OBJECTIVE" in text


def test_pm_chat_e2e_api_start_exports_allowed_origins_for_dashboard_port() -> None:
    text = _script_text()
    assert "CODEFLOW_API_ALLOWED_ORIGINS" in text
...
def test_pm_chat_e2e_browser_flow_defaults_api_auth_off() -> None:
    text = _script_text()
    assert 'CODEFLOW_E2E_API_AUTH_REQUIRED' in text or 'API_AUTH_REQUIRED="false"' in text


def test_pm_chat_e2e_deterministic_template_payload_is_injected_for_pm_intake() -> None:
    text = _runner_text()
    assert "CODEFLOW_E2E_TASK_TEMPLATE" in text
    assert "CODEFLOW_E2E_TEMPLATE_PAYLOAD_JSON" in text
