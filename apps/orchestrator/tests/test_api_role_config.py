    ...
    return {
        ...
        "agents": [
            ...
            },
            {
                "agent_id": "agent-1",
                "role": "TECH_LEAD",
                "codex_home": "$HOME/.codex-homes/agentcoder-techlead",
                "defaults": {
                    "sandbox": "read-only",
                    "approval_policy": "never",
                    "network": "deny",
...
def test_role_config_routes_preview_and_apply(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_AGENT_REGISTRY", str(repo_root / "policies" / "agent_registry.json"))
    monkeypatch.setenv("AGENTCODER_ROLE_CONFIG_REGISTRY", str(role_config_path))
    monkeypatch.delenv("AGENTCODER_PROVIDER_BASE_URL", raising=False)
    monkeypatch.delenv("AGENTCODER_CODEX_MODEL", raising=False)
...
def test_role_config_routes_fail_closed_on_invalid_provider(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_AGENT_REGISTRY", str(repo_root / "policies" / "agent_registry.json"))
    monkeypatch.setenv("AGENTCODER_ROLE_CONFIG_REGISTRY", str(repo_root / "policies" / "role_config_registry.json"))
...
def test_role_config_runtime_capability_import_stays_lightweight(tmp_path: Path) -> None:
    ...
    probe.write_text(
        textwrap.dedent(
            ...
            module = importlib.import_module("agentcoder_orch.contract.role_config_registry")
