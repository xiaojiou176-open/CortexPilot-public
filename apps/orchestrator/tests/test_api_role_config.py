...
def test_role_config_routes_preview_and_apply(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_AGENT_REGISTRY", str(repo_root / "policies" / "agent_registry.json"))
    monkeypatch.setenv("CODEFLOW_ROLE_CONFIG_REGISTRY", str(role_config_path))
    monkeypatch.delenv("CODEFLOW_PROVIDER_BASE_URL", raising=False)
    monkeypatch.delenv("CODEFLOW_CODEX_MODEL", raising=False)
...
def test_role_config_routes_fail_closed_on_invalid_provider(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_AGENT_REGISTRY", str(repo_root / "policies" / "agent_registry.json"))
    monkeypatch.setenv("CODEFLOW_ROLE_CONFIG_REGISTRY", str(repo_root / "policies" / "role_config_registry.json"))
...
def test_role_config_runtime_capability_import_stays_lightweight(tmp_path: Path) -> None:
    ...
    probe.write_text(
        textwrap.dedent(
            ...
            module = importlib.import_module("codeflow_orch.contract.role_config_registry")
