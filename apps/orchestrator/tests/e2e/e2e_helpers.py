...
def _acquire_ui_lock(repo_root: Path):
    lock_dir = repo_root / ".runtime-cache" / "codeflow" / "locks"
...
def _repo_root_from_env(env: dict[str, str]) -> Path:
    schema_root = Path(env["CODEFLOW_SCHEMA_ROOT"]).resolve()
...
def build_env(repo_root: Path, runtime_root: Path, runs_root: Path, worktree_root: Path) -> dict[str, str]:
    ...
    base_env["CODEFLOW_RUNTIME_ROOT"] = str(runtime_root)
    base_env["CODEFLOW_RUNS_ROOT"] = str(runs_root)
    base_env["CODEFLOW_WORKTREE_ROOT"] = str(worktree_root)
    base_env["CODEFLOW_SCHEMA_ROOT"] = str(repo_root / "schemas")
    base_env["CODEFLOW_CONTRACT_ROOT"] = str(runtime_root / "contracts")
    base_env["CODEFLOW_TOOL_REGISTRY"] = str(repo_root / "tooling" / "registry.json")
    base_env["CODEFLOW_AGENT_REGISTRY"] = str(repo_root / "policies" / "agent_registry.json")
...
def start_ui(repo_root: Path, env: dict[str, str], api_port: int, log_path: Path) -> tuple[ManagedProcess, int]:
    ui_port = free_port()
    ui_env = dict(env)
    ui_env["NEXT_PUBLIC_CODEFLOW_API_BASE"] = f"http://127.0.0.1:{api_port}"
