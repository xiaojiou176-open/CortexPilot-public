...
def _require_mcp_e2e_or_skip() -> None:
    ...
    if not enabled:
        if require:
            pytest.fail("mcp protocol drift e2e is required but disabled (set AGENTCODER_ENABLE_MCP_E2E=1)")
...
def test_mcp_protocol_drift_detection() -> None:
    ...
    try:
        resp = initialize_with_protocol(handle, "1900-01-01", request_id=1, client_name="agentcoder_drift")
