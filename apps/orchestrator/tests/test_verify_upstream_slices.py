...
def test_pm_chat_real_e2e_smoke_entrypoint_keeps_fast_gate_enabled() -> None:
    ...
    assert "CODEFLOW_E2E_SKIP_FAST_GATE=1" not in smoke_entrypoint
    assert "CODEFLOW_E2E_RUNNER=agents" in smoke_entrypoint
    assert "CODEFLOW_E2E_WEB_MODE=prod" in smoke_entrypoint
...
def test_verify_upstream_slices_handles_spacey_python_path_via_governance_wrapper(tmp_path: Path) -> None:
    ...
    env = {
        **os.environ,
        "CODEFLOW_PYTHON": str(spaced_python),
    }
    proc = subprocess.run(
