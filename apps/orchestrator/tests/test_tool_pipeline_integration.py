...
from agentcoder_orch.config import reset_cached_config
from agentcoder_orch.scheduler import scheduler as scheduler_module
from agentcoder_orch.scheduler import scheduler_bridge
...
def test_tool_pipeline_network_gate_denied(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_SEARCH_MODE", "mock")
    monkeypatch.setenv("AGENTCODER_CHAIN_EXEC_MODE", "inline")

    search_path = repo / "search_requests.json"
    search_body = _write_json(search_path, {"queries": ["agentcoder"]})
    artifacts = [
        {
...
def test_tool_pipeline_search_browser_tampermonkey(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.BrowserRunner", _BrowserOk)

    search_path = repo / "search_requests.json"
    search_body = _write_json(search_path, {"queries": ["agentcoder"], "repeat": 2, "parallel": 2})
    ...
    _pin_tool_requests(
        monkeypatch,
        search_request={"queries": ["agentcoder"], "repeat": 2, "parallel": 2},
...
def test_tool_pipeline_searcher_short_circuits_before_codex_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_MCP_ONLY", "1")
    monkeypatch.setenv("AGENTCODER_ALLOW_CODEX_EXEC", "1")

    search_path = repo / "search_requests.json"
    search_body = _write_json(
        search_path,
        {"queries": ["agentcoder"], "providers": ["chatgpt_web", "grok_web"], "repeat": 2, "parallel": 2},
    )
    _pin_tool_requests(
        monkeypatch,
        search_request={"queries": ["agentcoder"], "providers": ["chatgpt_web", "grok_web"], "repeat": 2, "parallel": 2},
    )
    ...
    monkeypatch.setattr(
        "agentcoder_orch.runners.codex_runner.CodexRunner.run_contract",
...
def test_tool_pipeline_page_brief_searcher_short_circuits_before_codex_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.BrowserRunner", _PageBriefBrowserOk)
    monkeypatch.setenv("AGENTCODER_CHAIN_EXEC_MODE", "inline")
    monkeypatch.setenv("AGENTCODER_RUNNER", "codex")
    monkeypatch.setenv("AGENTCODER_MCP_ONLY", "1")
    monkeypatch.setenv("AGENTCODER_ALLOW_CODEX_EXEC", "1")
...
def test_tool_pipeline_browser_failure_marks_run_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.tool_runner.BrowserRunner", _BrowserFail)
...
def test_tool_pipeline_tampermonkey_failure_marks_run_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.scheduler.scheduler.run_tampermonkey", _fail_tamper)
...
def test_tool_pipeline_tampermonkey_exec_failure_marks_run_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.scheduler.scheduler.run_tampermonkey", _exec_fail)
