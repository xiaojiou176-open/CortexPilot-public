...
from codeflow_orch.config import reset_cached_config
from codeflow_orch.scheduler import scheduler as scheduler_module
from codeflow_orch.scheduler import scheduler_bridge
...
def test_tool_pipeline_network_gate_denied(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_SEARCH_MODE", "mock")
    monkeypatch.setenv("CODEFLOW_CHAIN_EXEC_MODE", "inline")

    search_path = repo / "search_requests.json"
    search_body = _write_json(search_path, {"queries": ["codeflow"]})
    artifacts = [
        {
...
def test_tool_pipeline_search_browser_tampermonkey(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.runners.tool_runner.BrowserRunner", _BrowserOk)

    search_path = repo / "search_requests.json"
    search_body = _write_json(search_path, {"queries": ["codeflow"], "repeat": 2, "parallel": 2})
    ...
    _pin_tool_requests(
        monkeypatch,
        search_request={"queries": ["codeflow"], "repeat": 2, "parallel": 2},
...
def test_tool_pipeline_searcher_short_circuits_before_codex_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_MCP_ONLY", "1")
    monkeypatch.setenv("CODEFLOW_ALLOW_CODEX_EXEC", "1")

    search_path = repo / "search_requests.json"
    search_body = _write_json(
        search_path,
        {"queries": ["codeflow"], "providers": ["chatgpt_web", "grok_web"], "repeat": 2, "parallel": 2},
    )
    _pin_tool_requests(
        monkeypatch,
        search_request={"queries": ["codeflow"], "providers": ["chatgpt_web", "grok_web"], "repeat": 2, "parallel": 2},
    )
    ...
    monkeypatch.setattr(
        "codeflow_orch.runners.codex_runner.CodexRunner.run_contract",
...
def test_tool_pipeline_page_brief_searcher_short_circuits_before_codex_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.runners.tool_runner.BrowserRunner", _PageBriefBrowserOk)
    monkeypatch.setenv("CODEFLOW_CHAIN_EXEC_MODE", "inline")
    monkeypatch.setenv("CODEFLOW_RUNNER", "codex")
    monkeypatch.setenv("CODEFLOW_MCP_ONLY", "1")
    monkeypatch.setenv("CODEFLOW_ALLOW_CODEX_EXEC", "1")
...
def test_tool_pipeline_browser_failure_marks_run_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.runners.tool_runner.BrowserRunner", _BrowserFail)
...
def test_tool_pipeline_tampermonkey_failure_marks_run_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.scheduler.scheduler.run_tampermonkey", _fail_tamper)
...
def test_tool_pipeline_tampermonkey_exec_failure_marks_run_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.scheduler.scheduler.run_tampermonkey", _exec_fail)
