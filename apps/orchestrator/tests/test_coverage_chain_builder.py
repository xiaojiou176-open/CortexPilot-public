...
from codeflow_orch.cli import app
from codeflow_orch.contract.validator import ContractValidator
from codeflow_orch.planning.coverage_chain import CoverageTarget, _assigned_role, build_coverage_self_heal_chain, load_coverage_targets
...
def _write_coverage_json(path: Path) -> None:
    payload = {
        "meta": {"version": "1"},
        "files": {
            "apps/orchestrator/src/codeflow_orch/runners/agents_runner.py": {
                "summary": {
            ...
            "apps/orchestrator/src/codeflow_orch/scheduler/scheduler.py": {
                "summary": {
            ...
            "apps/orchestrator/src/codeflow_orch/runners/codex_runner.py": {
                "summary": {
...
def _write_coverage_json_many(path: Path) -> None:
    payload = {
        ...
        "files": {
            ...
            "apps/orchestrator/src/codeflow_orch/chain/runner.py": {
                "summary": {
            ...
            "apps/orchestrator/src/codeflow_orch/planning/intake.py": {
                "summary": {
...
def test_assigned_role_prefers_worker_for_coverage_self_heal_targets() -> None:
    assert _assigned_role("apps/orchestrator/src/codeflow_orch/api/routes_admin.py") == "WORKER"
    assert _assigned_role("apps/orchestrator/src/codeflow_orch/scheduler/scheduler.py") == "WORKER"


def test_worker_timeout_uses_env_override(monkeypatch) -> None:
    targets = [
        CoverageTarget(
            module_path="apps/orchestrator/src/codeflow_orch/api/routes_admin.py",
            module_name="codeflow_orch.api.routes_admin",
            coverage=50.0,
        )
    ]

    monkeypatch.setenv("CODEFLOW_COVERAGE_WORKER_TIMEOUT_SEC", "123")
    ...
    monkeypatch.setenv("CODEFLOW_COVERAGE_WORKER_TIMEOUT_SEC", "5")
...
def test_coverage_targets_and_chain_schema_validation(tmp_path: Path) -> None:
    ...
    assert [item.module_name for item in targets] == [
        "codeflow_orch.runners.agents_runner",
        "codeflow_orch.scheduler.scheduler",
    ]
    ...
    assert [item.module_name for item in branch_targets] == ["codeflow_orch.runners.agents_runner"]
    ...
    assert "test_cov_apps_orchestrator_src_codeflow_orch_runners_agents_runner.py" in gate_cmd
    assert "test_cov_apps_orchestrator_src_codeflow_orch_scheduler_scheduler.py" in gate_cmd
...
def test_shell_permissions_switch_to_on_request_for_codex_runner(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_RUNNER", "codex")
    chain = build_coverage_self_heal_chain(
        targets,
...
def test_cli_coverage_self_heal_chain_execute_fallback_to_equilibrium(tmp_path: Path, monkeypatch) -> None:
    ...
    class _FakeOrchestrator:
        ...
        def execute_chain(self, chain_path: Path, mock_mode: bool = False) -> dict:
            calls["execute_chain"] = {"path": str(chain_path), "mock": mock_mode}
            calls["runner"] = os.getenv("CODEFLOW_RUNNER", "")
            calls["base_url"] = os.getenv("CODEFLOW_PROVIDER_BASE_URL", "")
            calls["lock_auto_cleanup"] = os.getenv("CODEFLOW_LOCK_AUTO_CLEANUP", "")
            calls["chain_exec_mode"] = os.getenv("CODEFLOW_CHAIN_EXEC_MODE", "")
            calls["chain_subprocess_timeout"] = os.getenv("CODEFLOW_CHAIN_SUBPROCESS_TIMEOUT_SEC", "")
    ...
    monkeypatch.delenv("CODEFLOW_LOCK_AUTO_CLEANUP", raising=False)
    monkeypatch.delenv("CODEFLOW_CHAIN_EXEC_MODE", raising=False)
    monkeypatch.delenv("CODEFLOW_CHAIN_SUBPROCESS_TIMEOUT_SEC", raising=False)
    monkeypatch.delenv("CODEFLOW_ALLOW_CODEX_EXEC", raising=False)
...
def test_build_coverage_self_heal_chain_worker_batching_groups() -> None:
    targets = [
        CoverageTarget(
            module_path=f"apps/orchestrator/src/codeflow_orch/module_{idx}.py",
            module_name=f"codeflow_orch.module_{idx}",
            coverage=70.0 + idx,
        )
...
def test_cli_coverage_self_heal_chain_execute_with_codex_fallback(tmp_path: Path, monkeypatch) -> None:
    ...
    class _FakeOrchestrator:
        ...
        def execute_chain(self, chain_path: Path, mock_mode: bool = False) -> dict:
            ...
            calls["chain_exec_mode"] = os.getenv("CODEFLOW_CHAIN_EXEC_MODE", "")
            calls["chain_subprocess_timeout"] = os.getenv("CODEFLOW_CHAIN_SUBPROCESS_TIMEOUT_SEC", "")
            calls["allow_codex_exec"] = os.getenv("CODEFLOW_ALLOW_CODEX_EXEC", "")
...
def test_cli_coverage_self_heal_chain_execute_restores_temporary_env(monkeypatch, tmp_path: Path) -> None:
    ...
    class _FakeOrchestrator:
        ...
        def execute_chain(self, chain_path: Path, mock_mode: bool = False) -> dict:
            ...
            observed["runner_during_execute"] = os.getenv("CODEFLOW_RUNNER", "")
    ...
    def _fake_print(*_args, **_kwargs) -> None:
        observed["runner_at_print"] = os.getenv("CODEFLOW_RUNNER", "")
