...
from codeflow_orch.runners.agents_runner import (
    AgentsRunner,
    _build_codex_payload,
    _extract_binding_from_result,
    _resolve_session_binding,
)
from codeflow_orch.runners.codex_runner import (
    CodexRunner,
    _codex_allowed,
    _codex_flags,
    _extract_session_id,
...
def test_agents_runner_missing_api_key(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("CODEFLOW_AGENTS_BASE_URL", raising=False)
    monkeypatch.setattr("codeflow_orch.runners.agents_runner._equilibrium_healthcheck", lambda *_args, **_kwargs: False)
...
def test_agents_runner_handoff_chain(monkeypatch, tmp_path: Path) -> None:
    ...
    class DummyRunner:
        @staticmethod
        def run_streamed(agent, prompt, **kwargs):
            if agent.name.startswith("CodeflowHandoff_") or agent.name == "CodeflowOwner":
                payload = {
