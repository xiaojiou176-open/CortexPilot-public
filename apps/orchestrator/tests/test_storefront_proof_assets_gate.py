...
def _load_gate_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location("codeflow_storefront_proof_assets_gate", script_path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    return module
...
def _write_fixture(root: Path) -> None:
    ...
    (root / "configs" / "public_proof" / "storefront" / "live-capture-requirements.json").write_text(
        json.dumps(
            {
                "artifact_type": "codeflow_storefront_live_capture_requirements",
