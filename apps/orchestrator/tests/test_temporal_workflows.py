...
from codeflow_orch.temporal import workflows
...
def test_temporal_workflows_fallback_activity_raises() -> None:
    ...
    if hasattr(workflows.CodeflowRunWorkflow, "run"):
    ...
    assert workflows.CodeflowRunWorkflow is not None
...
def test_temporal_workflows_activity_and_workflow_paths(monkeypatch, tmp_path):
    ...
    wf = workflows.CodeflowRunWorkflow()
...
def test_temporal_workflows_reload_activity_and_workflow_paths(monkeypatch, tmp_path) -> None:
    ...
    from codeflow_orch.temporal import workflows as workflows_module
    ...
    assert "CODEFLOW_TEMPORAL_ACTIVITY" not in os.environ
    assert "CODEFLOW_TEMPORAL_WORKFLOW" not in os.environ
    assert "CODEFLOW_TEMPORAL_WORKFLOW_ID" not in os.environ

    monkeypatch.setenv("CODEFLOW_TEMPORAL_ACTIVITY_TIMEOUT", "10")
    workflow_result = asyncio.run(reloaded.CodeflowRunWorkflow().run(request))
