...
def test_reexecute_soft_and_hard_diffs(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.replay.replayer.run_acceptance_tests", _fake_tests)
    monkeypatch.setattr("codeflow_orch.replay.replayer.worktree_manager.create_worktree", lambda *args, **kwargs: repo)
    monkeypatch.setattr("codeflow_orch.replay.replayer.worktree_manager.remove_worktree", lambda *args, **kwargs: None)
...
def test_reexecute_allows_public_searcher_tests_skip_to_pass(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.replay.replayer.run_acceptance_tests", lambda *args, **kwargs: {"ok": True, "reports": []})
    monkeypatch.setattr("codeflow_orch.replay.replayer.worktree_manager.create_worktree", lambda *args, **kwargs: repo)
    monkeypatch.setattr("codeflow_orch.replay.replayer.worktree_manager.remove_worktree", lambda *args, **kwargs: None)
