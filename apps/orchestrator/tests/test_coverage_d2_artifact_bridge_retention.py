...
from codeflow_orch.runtime import retention as retention_module
from codeflow_orch.runtime.retention import RetentionPlan, apply_retention_plan, build_retention_plan
from codeflow_orch.scheduler import artifact_pipeline, scheduler_bridge_finalize as bridge_finalize
...
def test_retention_d2_namespace_contract_overflow_and_apply_edges(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_RETENTION_RUN_DAYS", "30")
    monkeypatch.setenv("CODEFLOW_RETENTION_MAX_RUNS", "10")
    monkeypatch.setenv("CODEFLOW_RETENTION_LOG_DAYS", "30")
    monkeypatch.setenv("CODEFLOW_RETENTION_WORKTREE_DAYS", "30")
    monkeypatch.setenv("CODEFLOW_RETENTION_LOG_MAX_FILES", "10")
    monkeypatch.setenv("CODEFLOW_RETENTION_CACHE_HOURS", "48")
    monkeypatch.setenv("CODEFLOW_RETENTION_CODEX_HOME_DAYS", "30")
    monkeypatch.setenv("CODEFLOW_RETENTION_MAX_CODEX_HOMES", "1")
    monkeypatch.setenv("CODEFLOW_RETENTION_INTAKE_DAYS", "30")
    monkeypatch.setenv("CODEFLOW_RETENTION_MAX_INTAKES", "1")
    monkeypatch.delenv("CODEFLOW_ENV_FILE", raising=False)
    monkeypatch.delenv("CODEFLOW_PROVIDER", raising=False)
