...
from codeflow_orch.services.control_plane_read_service import (
...
def test_control_plane_read_service_from_api_main_builds_workflows_and_queue_filters(monkeypatch) -> None:
    ...
    api_main = ModuleType("codeflow_orch.api.main")
    ...
    main_state_store_helpers = ModuleType("codeflow_orch.api.main_state_store_helpers")
    main_state_store_helpers.collect_workflows = lambda **_: workflows

    queue_module = ModuleType("codeflow_orch.queue")
    queue_module.QueueStore = _FakeQueueStore

    monkeypatch.setitem(sys.modules, "codeflow_orch.api.main", api_main)
    monkeypatch.setitem(sys.modules, "codeflow_orch.api.main_state_store_helpers", main_state_store_helpers)
    monkeypatch.setitem(sys.modules, "codeflow_orch.queue", queue_module)
...
def test_control_plane_read_service_from_runtime_builds_runtime_views_and_pending_approvals(
    ...
    config_module = ModuleType("codeflow_orch.config")
    ...
    main_run_views_helpers = ModuleType("codeflow_orch.api.main_run_views_helpers")
    main_run_views_helpers.list_diff_gate = lambda **_: [
        {"run_id": "run-a", "status": "FAILED"},
        {"run_id": "run-b", "status": "PASS"},
    ]

    compiler_module = ModuleType("codeflow_orch.contract.compiler")
    ...
    monkeypatch.setitem(sys.modules, "codeflow_orch.config", config_module)
    monkeypatch.setitem(sys.modules, "codeflow_orch.api.main_state_store_helpers", main_state_store_helpers)
    monkeypatch.setitem(sys.modules, "codeflow_orch.api.main_run_views_helpers", main_run_views_helpers)
    monkeypatch.setitem(sys.modules, "codeflow_orch.contract.compiler", compiler_module)
    monkeypatch.setitem(sys.modules, "codeflow_orch.queue", queue_module)
