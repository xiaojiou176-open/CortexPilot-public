...
from codeflow_orch.gates import integrated_gate
from codeflow_orch.runners import agents_handoff, agents_mcp_config, agents_payload
from codeflow_orch.scheduler import artifact_refs, runtime_utils
...
def test_orchestration_service_delegates_all_entrypoints(monkeypatch, tmp_path: Path) -> None:
    import codeflow_orch.services.orchestration_service as service_module
...
def test_integrated_gate_load_registry_branches(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_TOOL_REGISTRY", "configs/registry.json")
    ...
    monkeypatch.setenv("CODEFLOW_TOOL_REGISTRY", "configs/missing.json")
    fallback = integrated_gate.validate_integrated_tools(repo_root, ["codex"])
    assert fallback["ok"] is True
    assert fallback["integrated"] == ["codex"]

    monkeypatch.delenv("CODEFLOW_TOOL_REGISTRY", raising=False)
...
def test_tracer_http_fallback_console_and_required(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_OTLP_ENDPOINT", "http://127.0.0.1:4318/v1/traces")
    monkeypatch.setenv("CODEFLOW_OTLP_PROTOCOL", "http")
    monkeypatch.setenv("CODEFLOW_OTLP_HEADERS", "token=abc, malformed, x = y")
    tracer_module = importlib.reload(importlib.import_module("codeflow_orch.observability.tracer"))
    ...
    monkeypatch.delenv("CODEFLOW_OTLP_ENDPOINT", raising=False)
    monkeypatch.setenv("CODEFLOW_ENABLE_CONSOLE_TRACE", "true")
    tracer_module = importlib.reload(importlib.import_module("codeflow_orch.observability.tracer"))
    assert tracer_module.tracing_status()["console_enabled"] is True

    monkeypatch.setenv("CODEFLOW_OTEL_REQUIRED", "true")
    monkeypatch.delenv("CODEFLOW_ENABLE_CONSOLE_TRACE", raising=False)
...
def test_agents_handoff_branch_matrix(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_AGENTS_FORCE_HANDOFF", "true")
...
def test_temporal_workflows_force_import_fallback(monkeypatch) -> None:
    ...
    monkeypatch.setattr(builtins, "__import__", _raising_import)
    workflows = importlib.reload(importlib.import_module("codeflow_orch.temporal.workflows"))
