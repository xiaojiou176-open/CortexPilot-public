...
def test_scheduler_mcp_concurrency_required(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_RUNS_ROOT", str(runs_root))
    monkeypatch.setenv("CODEFLOW_WORKTREE_ROOT", str(worktree_root))
    monkeypatch.setenv("CODEFLOW_MCP_CONCURRENCY_MODE", "multi")
    monkeypatch.setenv("CODEFLOW_MCP_CONCURRENCY_REQUIRED", "1")
...
def test_scheduler_mcp_only_blocks_codex_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.runners.codex_runner.CodexRunner.run_contract", _boom)
