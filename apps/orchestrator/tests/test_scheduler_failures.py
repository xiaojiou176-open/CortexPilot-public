...
def test_scheduler_mcp_concurrency_required(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_RUNS_ROOT", str(runs_root))
    monkeypatch.setenv("AGENTCODER_WORKTREE_ROOT", str(worktree_root))
    monkeypatch.setenv("AGENTCODER_MCP_CONCURRENCY_MODE", "multi")
    monkeypatch.setenv("AGENTCODER_MCP_CONCURRENCY_REQUIRED", "1")
...
def test_scheduler_mcp_only_blocks_codex_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentcoder_orch.runners.codex_runner.CodexRunner.run_contract", _boom)
