...
from codeflow_orch.api import routes_admin, routes_intake, routes_pm, routes_runs
from codeflow_orch.replay import reexec_flow, verify_flow
from codeflow_orch.runners import agents_contract_flow, agents_stream_flow
from codeflow_orch.scheduler import execute_flow, failure_handling, manifest_lifecycle, runtime_bootstrap
...
def test_runtime_bootstrap_creates_target_dirs(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_LOGS_ROOT", str(tmp_path / "logs"))
    monkeypatch.setenv("CODEFLOW_CACHE_ROOT", str(tmp_path / "cache"))
...
def test_route_dep_missing_returns_structured_503(monkeypatch) -> None:
    ...
    admin_response = admin_client.get("/api/god-mode/pending", headers={"x-codeflow-role": "TECH_LEAD"})
...
def test_route_admin_pending_partial_mapping_returns_structured_503(monkeypatch) -> None:
    ...
    client = TestClient(app)
    response = client.get("/api/god-mode/pending", headers={"x-codeflow-role": "TECH_LEAD"})
    assert response.status_code == 503
    assert response.json()["detail"] == {
        "code": "ROUTE_DEPS_NOT_CONFIGURED",
        "error": "route dependencies not configured",
