...
def test_ci_control_plane_doctor_can_emit_current_run_source_metadata(tmp_path: Path) -> None:
    ...
    env = {
        "CODEFLOW_CI_CONTROL_PLANE_DOCTOR_OUT_DIR": str(out_dir),
        "CODEFLOW_DOCTOR_REQUIRE_DOCKER": "0",
        "CODEFLOW_DOCTOR_REQUIRE_SUDO": "0",
        "RUNNER_TEMP": str(tmp_path / "runner-temp"),
        "CODEFLOW_CI_SOURCE_RUN_ID": "local-run",
        "CODEFLOW_CI_SOURCE_ROUTE": "local_full_ci",
        "CODEFLOW_CI_SOURCE_EVENT": "local",
    }
    proc = subprocess.run(
