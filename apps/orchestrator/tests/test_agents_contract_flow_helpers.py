...
from codeflow_orch.runners import agents_contract_flow
...
def test_contract_helper_primitives(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_LANGGRAPH_CONTRACT_SUBFLOW", "yes")
    assert agents_contract_flow._langgraph_requested({}) == (
        True,
        "env.CODEFLOW_LANGGRAPH_CONTRACT_SUBFLOW",
    )

    monkeypatch.setenv("CODEFLOW_LANGGRAPH_CONTRACT_SUBFLOW", "")
