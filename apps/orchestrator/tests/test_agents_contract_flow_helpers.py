...
from agentcoder_orch.runners import agents_contract_flow
...
def test_contract_helper_primitives(monkeypatch) -> None:
    ...
    monkeypatch.setenv("AGENTCODER_LANGGRAPH_CONTRACT_SUBFLOW", "yes")
    assert agents_contract_flow._langgraph_requested({}) == (
        True,
        "env.AGENTCODER_LANGGRAPH_CONTRACT_SUBFLOW",
    )

    monkeypatch.setenv("AGENTCODER_LANGGRAPH_CONTRACT_SUBFLOW", "")
