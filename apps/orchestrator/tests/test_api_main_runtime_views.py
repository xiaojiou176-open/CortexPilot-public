...
def test_api_queue_list_enqueue_and_run_next(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_manifest(
        run_dir,
        {
            ...
            "task_id": "task_source",
            "status": "SUCCESS",
            "workflow": {"workflow_id": "wf-queue", "task_queue": "codeflow-orch", "namespace": "default", "status": "SUCCESS"},
        },
    )
    _write_contract(
        run_dir,
    ...
    run_next = client.post("/api/queue/run-next", json={"mock": True}, headers={"x-codeflow-role": "OWNER"})
...
def test_api_agents_policies_locks_worktrees(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            ...
            "HEAD abc123",
            "branch refs/heads/codeflow-run-run_lock",
            "locked",
        ],
    )
...
def test_api_search_promote_and_agent_status(tmp_path: Path, monkeypatch) -> None:
    ...
    pending_resp = client.get("/api/god-mode/pending", headers={"x-codeflow-role": "TECH_LEAD"})
