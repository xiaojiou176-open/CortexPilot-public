...
from codeflow_orch.locks.locker import _lock_path, acquire_lock, acquire_lock_with_cleanup, release_lock


def test_locker_acquire_collision(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path))
...
def test_locker_release(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path))
    monkeypatch.setenv("CODEFLOW_RUN_ID", "run_release")
    locks_root = tmp_path / "locks"
    locks_root.mkdir(parents=True, exist_ok=True)
...
def test_locker_release_requires_owner_or_force_unlock(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path))
    monkeypatch.setenv("CODEFLOW_RUN_ID", "run_owner_a")
    monkeypatch.setenv("CODEFLOW_LOCK_OWNER_TOKEN", "owner_a")
    target = ["src/owned.py"]

    assert acquire_lock(target) is True

    monkeypatch.setenv("CODEFLOW_RUN_ID", "run_owner_b")
    monkeypatch.setenv("CODEFLOW_LOCK_OWNER_TOKEN", "owner_b")
    release_lock(target)
    assert acquire_lock(target) is False

    monkeypatch.setenv("CODEFLOW_FORCE_UNLOCK", "1")
    release_lock(target)
    monkeypatch.delenv("CODEFLOW_FORCE_UNLOCK", raising=False)
    assert acquire_lock(target) is True
    release_lock(target)
...
def test_locker_sanitize_allowed_paths_non_string_entries(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path))
    monkeypatch.setenv("CODEFLOW_RUN_ID", "run_sanitize")
...
def test_locker_rejects_traversal_and_absolute_allowed_paths(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path))
    monkeypatch.setenv("CODEFLOW_RUN_ID", "run_path_safety")
...
def test_locker_partial_failure_cleanup_does_not_remove_replaced_foreign_lock(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(tmp_path))
    monkeypatch.setenv("CODEFLOW_RUN_ID", "run-owner-a")
    monkeypatch.setenv("CODEFLOW_LOCK_OWNER_TOKEN", "owner_a")
    locks_root = tmp_path / "locks"
    locks_root.mkdir(parents=True, exist_ok=True)
    ...
    import codeflow_orch.locks.locker as locker_module
