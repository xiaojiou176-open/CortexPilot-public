...
from codeflow_orch.runners import agents_mcp_execution_helpers, agents_mcp_runtime
from codeflow_orch.runners import provider_resolution as provider_resolution_module
from codeflow_orch import config as config_module
from codeflow_orch.runners.provider_resolution import (
    build_llm_compat_client,
    ProviderResolutionError,
    resolve_runtime_provider_from_contract,
)
from codeflow_orch.scheduler import scheduler_bridge_runtime
...
def test_mcp_runtime_materialize_injects_provider_model_key_and_base_url(
    ...
    monkeypatch.setenv("CODEFLOW_CODEX_BASE_HOME", str(base_home))
    monkeypatch.setenv("CODEFLOW_CODEX_MODEL", "openai-model")
...
def test_mcp_execution_env_injects_provider_specific_key_and_base_url(
    ...
    assert captured_env["CODEFLOW_PROVIDER"] == "equilibrium"
    assert captured_env["CODEFLOW_PROVIDER_BASE_URL"] == "http://127.0.0.1:1456/v1"
    assert captured_env["CODEFLOW_EQUILIBRIUM_API_KEY"] == "equ-key"
...
def test_select_runner_prefers_contract_provider_override_over_env(
    ...
    monkeypatch.setenv("CODEFLOW_RUNNER", "agents")
    monkeypatch.setenv("CODEFLOW_PROVIDER", "gemini")
...
def test_select_runner_keeps_runner_resolution_when_provider_override_missing(
    ...
    monkeypatch.setattr(scheduler_bridge_runtime, "import_module", lambda _name: fake_module)
    monkeypatch.setenv("CODEFLOW_RUNNER", "agents")
    monkeypatch.setenv("CODEFLOW_PROVIDER", "openai")
...
def test_build_llm_compat_client_litellm_switch_falls_back_to_openai_client(
    ...
    monkeypatch.setenv("CODEFLOW_PROVIDER_USE_LITELLM", "1")
...
def test_build_llm_compat_client_uses_openai_path_when_switch_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("CODEFLOW_PROVIDER_USE_LITELLM", raising=False)
...
def test_resolve_provider_credentials_reads_equilibrium_key_from_env(
    ...
    monkeypatch.setenv("CODEFLOW_EQUILIBRIUM_API_KEY", "eq-only")
...
def test_resolve_runtime_base_url_uses_canonical_env_key(
    ...
    monkeypatch.setenv("CODEFLOW_PROVIDER_BASE_URL", base_url)
...
def test_agents_mcp_runtime_helper_branches_runtime_root_and_toml(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CODEFLOW_RUNTIME_ROOT", str(override_root))
...
def test_agents_mcp_runtime_patch_initialized_notification_runtime_paths(
    ...
    fake_non_codex = types.SimpleNamespace(
        ...
        _client_info={"name": "codeflow", "version": "0.1"},
