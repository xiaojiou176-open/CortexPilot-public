...
from codeflow_orch.store.run_store_primitives import write_atomic
...
def test_run_store_creation(tmp_path: Path):
    os.environ["CODEFLOW_RUNS_ROOT"] = str(tmp_path)
    store = RunStore()
...
def test_run_store_contract_signature(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CODEFLOW_RUNS_ROOT", str(tmp_path))
    store = RunStore()
    run_id = store.create_run("task_sig")
    contract_path = store.write_contract(run_id, {"task_id": "task_sig"})

    monkeypatch.delenv("CODEFLOW_CONTRACT_HMAC_KEY", raising=False)
    assert store.write_contract_signature(run_id, contract_path) is None

    monkeypatch.setenv("CODEFLOW_CONTRACT_HMAC_KEY", "secret")
...
def test_write_atomic_fsyncs_parent_directory(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeflow_orch.store.run_store_primitives.os.fsync", _tracked_fsync)
