# Agentcoder Distribution Contract

This file is the canonical human-readable source for Agentcoder's current
...
Today Agentcoder officially ships a public repo front door, a GitHub Pages
product front door, one proof-first public workflow baseline, a repo-local
read-only MCP server, the live Agentcoder PyPI package plus the live
Agentcoder Official MCP Registry entry for that same read-only runtime,
...
| --- | --- | --- | --- | --- | --- |
| GitHub repo | `shipped` | Canonical public source, docs, code, and release front door | `https://github.com/xiaojiou176-open/agentcoder` | none | keep sharp and truthful |
| GitHub Pages | `shipped` | Canonical public product front door | `https://xiaojiou176-open.github.io/agentcoder/` | none | keep first screen compressed |
...
| Read-only MCP | `shipped` | Repo-owned stdio JSON-RPC MCP for machine-readable inspection only | bootstrapped repo checkout + `bash __AGENTCODER_REPO_ROOT__/scripts/run_agentcoder_readonly_mcp.sh` or the tracked starter templates | `stdio`, JSON-RPC 2.0, read-only, repo-local, no hosted auth, no OAuth | keep artifactized through `configs/mcp_public_manifest.json` |
| PyPI package (`agentcoder-orchestrator`) | `shipped` | Published package for the public read-only MCP runtime under the Agentcoder name | `https://pypi.org/project/agentcoder-orchestrator/0.1.0a4/` | package install only | treat this as the primary install path and keep all docs aligned to it |
| Official MCP Registry entry (`io.github.xiaojiou176-open/agentcoder-readonly`) | `shipped` | Public MCP discovery entry is live under the Agentcoder identity | `https://registry.modelcontextprotocol.io/v0/servers?search=io.github.xiaojiou176-open/agentcoder-readonly` | registry discovery only, stdio package install | keep this as the canonical registry entry and keep repo-side docs aligned to it |
...
| Cross-tool coding-agent bundle | `bundle-compatible` | Local bundle compatible with Codex local marketplace installs, Claude plugin-dir development, and OpenClaw local plugin loading | `examples/coding-agents/plugin-bundles/agentcoder-coding-agent-bundle/` | local bundle metadata + repo-aware MCP wrapper | keep local-install contract; no published listing claim |
| Repo-owned adoption-router skill | `shipped` | Cross-tool routing skill with `SKILL.md` + `manifest.yaml`, shared between the public skill packet, the repo bundle, and external skill distribution | `public-skills/agentcoder-adoption-router/` | repo-owned skill contract, public skill packet plus local bundle example | keep the public packet, repo bundle, and published skill receipts aligned |
| ClawHub skill (`agentcoder-adoption-router`) | `shipped` | Published OpenClaw skill for honest Agentcoder adoption routing | `https://www.clawhub.ai/skills/agentcoder-adoption-router` | skill registry, no hosted Agentcoder account, no write-capable MCP | keep the skill copy aligned with the repo bundle and public boundary |
...
- The latest live public package for the read-only MCP is now
  `agentcoder-orchestrator==0.1.0a4`.
- The latest live Official MCP Registry entry now points to
  `io.github.xiaojiou176-open/agentcoder-readonly@0.1.0a4`.
...
- Implementation: `apps/orchestrator/src/agentcoder_orch/mcp_readonly_server.py`
- Canonical startup wrapper: `scripts/run_agentcoder_readonly_mcp.sh`
