# Codeflow Distribution Contract

This file is the canonical human-readable source for Codeflow's current
...
Today Codeflow officially ships a public repo front door, a GitHub Pages
product front door, one proof-first public workflow baseline, a repo-local
read-only MCP server, the live Codeflow PyPI package plus the live
Codeflow Official MCP Registry entry for that same read-only runtime,
...
| --- | --- | --- | --- | --- | --- |
| GitHub repo | `shipped` | Canonical public source, docs, code, and release front door | `https://github.com/xiaojiou176-open/codeflow` | none | keep sharp and truthful |
| GitHub Pages | `shipped` | Canonical public product front door | `https://xiaojiou176-open.github.io/codeflow/` | none | keep first screen compressed |
...
| Read-only MCP | `shipped` | Repo-owned stdio JSON-RPC MCP for machine-readable inspection only | bootstrapped repo checkout + `bash __CODEFLOW_REPO_ROOT__/scripts/run_codeflow_readonly_mcp.sh` or the tracked starter templates | `stdio`, JSON-RPC 2.0, read-only, repo-local, no hosted auth, no OAuth | keep artifactized through `configs/mcp_public_manifest.json` |
| PyPI package (`codeflow-orchestrator`) | `shipped` | Published package for the public read-only MCP runtime under the Codeflow name | `https://pypi.org/project/codeflow-orchestrator/0.1.0a4/` | package install only | treat this as the primary install path and keep all docs aligned to it |
| Official MCP Registry entry (`io.github.xiaojiou176-open/codeflow-readonly`) | `shipped` | Public MCP discovery entry is live under the Codeflow identity | `https://registry.modelcontextprotocol.io/v0/servers?search=io.github.xiaojiou176-open/codeflow-readonly` | registry discovery only, stdio package install | keep this as the canonical registry entry and keep repo-side docs aligned to it |
...
| Cross-tool coding-agent bundle | `bundle-compatible` | Local bundle compatible with Codex local marketplace installs, Claude plugin-dir development, and OpenClaw local plugin loading | `examples/coding-agents/plugin-bundles/codeflow-coding-agent-bundle/` | local bundle metadata + repo-aware MCP wrapper | keep local-install contract; no published listing claim |
| Repo-owned adoption-router skill | `shipped` | Cross-tool routing skill with `SKILL.md` + `manifest.yaml`, shared between the public skill packet, the repo bundle, and external skill distribution | `public-skills/codeflow-adoption-router/` | repo-owned skill contract, public skill packet plus local bundle example | keep the public packet, repo bundle, and published skill receipts aligned |
| ClawHub skill (`codeflow-adoption-router`) | `shipped` | Published OpenClaw skill for honest Codeflow adoption routing | `https://www.clawhub.ai/skills/codeflow-adoption-router` | skill registry, no hosted Codeflow account, no write-capable MCP | keep the skill copy aligned with the repo bundle and public boundary |
...
| `@codeflow/frontend-api-contract` | `publish-ready but deferred` | Generated route / query / type boundary for frontend consumers | package metadata + README are publish-ready, but the official install story is still clone / vendor reuse until the first npm release exists | typed contract layer only | publish later only after the first public package release is intentionally cut |
| `@codeflow/frontend-shared` | `not standalone distribution unit` | Repo-owned presentation substrate for dashboard / desktop / future web surfaces | repo-local package only | frontend presentation helpers only | keep repo-owned for now |
...
- The latest live public package for the read-only MCP is now
  `codeflow-orchestrator==0.1.0a4`.
- The latest live Official MCP Registry entry now points to
  `io.github.xiaojiou176-open/codeflow-readonly@0.1.0a4`.
...
- Implementation: `apps/orchestrator/src/codeflow_orch/mcp_readonly_server.py`
- Canonical startup wrapper: `scripts/run_codeflow_readonly_mcp.sh`
